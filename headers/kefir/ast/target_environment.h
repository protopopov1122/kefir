...
typedef struct kefir_ast_target_environment {
    ...
    kefir_result_t (*free_type)(struct kefir_mem *,
                              const struct kefir_ast_target_environment *,
                              kefir_ast_target_environment_opaque_type_t);
    kefir_result_t (*object_info)(struct kefir_mem *,
                                const struct kefir_ast_target_environment *,
                                kefir_ast_target_environment_opaque_type_t,
...
#define KEFIR_AST_TARGET_ENVIRONMENT_GET_TYPE(mem, env, type, info) \
    ((env)->get_type((mem), (env), (type), (info)))
#define KEFIR_AST_TARGET_ENVIRONMENT_FREE_TYPE(mem, env, info) \
