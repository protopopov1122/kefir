    ...
    if (KEFIR_AST_TYPE_IS_ARITHMETIC_TYPE(target_type)) {
        if (target_type->tag != KEFIR_AST_TYPE_SCALAR_BOOL ||
            ...
            REQUIRE(KEFIR_AST_TYPE_IS_ARITHMETIC_TYPE(value_type),
                KEFIR_SET_ERROR(KEFIR_MALFORMED_ARG, "Assignable node shall have arithmetic type"));
        }
        ...
        REQUIRE(KEFIR_AST_TYPE_COMPATIBLE(context->type_traits, target_type, value_type),
            KEFIR_SET_ERROR(KEFIR_MALFORMED_ARG, "Both assignable operands shall be compatible"));
        ...
        if (target_underlying->tag != KEFIR_AST_TYPE_VOID && value_underlying->tag != KEFIR_AST_TYPE_VOID) {
            REQUIRE(KEFIR_AST_TYPE_COMPATIBLE(context->type_traits, target_underlying, value_underlying),
                KEFIR_SET_ERROR(KEFIR_MALFORMED_ARG, "Both assignable operands shall be pointers to compatible types"));
        }
        REQUIRE((!value_qualification.constant || target_qualification.constant) &&
            ...
            KEFIR_SET_ERROR(KEFIR_MALFORMED_ARG, "Left assignable operand shall point to the type with all qualifications from right"));
    } else {
        REQUIRE(target_type->tag == KEFIR_AST_TYPE_SCALAR_POINTER,
            KEFIR_SET_ERROR(KEFIR_MALFORMED_ARG, "Left assignable operand shall be a pointer"));
        REQUIRE(KEFIR_AST_TYPE_IS_INTEGRAL_TYPE(value_type) && node->properties.expression_props.constant_expression,
            KEFIR_SET_ERROR(KEFIR_MALFORMED_ARG, "Right assignable operand shall be NULL pointer"));
    }
    return KEFIR_OK;
}