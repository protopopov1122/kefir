...
static kefir_result_t analyze_enum(struct kefir_mem *mem,
    ...
    if (enum_type->complete) {
        ...
        for (const struct kefir_list_entry *iter = kefir_list_head(&enum_type->enumerators);
            iter != NULL;
            kefir_list_next(&iter)) {
            
            ASSIGN_DECL_CAST(struct kefir_ast_enum_enumerator *, enumerator,
                iter->value);
...
static kefir_result_t analyze_array(struct kefir_mem *mem,
    ...
    REQUIRE((array_type->boundary != KEFIR_AST_ARRAY_BOUNDED_STATIC &&
        ...
        KEFIR_SET_ERROR(KEFIR_MALFORMED_ARG, "Array type qualifications shall appear only for the outermost function parameter type"));
...
static kefir_result_t analyze_structure(struct kefir_mem *mem,
                                      const struct kefir_ast_context *context,
    ...
    if (struct_type->complete) {
        for (const struct kefir_list_entry *iter = kefir_list_head(&struct_type->fields);
            ...
            if (field->type->tag == KEFIR_AST_TYPE_ARRAY) {
                ...
                REQUIRE(!KEFIR_AST_TYPE_IS_INCOMPLETE(field->type) &&
                    field->type->tag != KEFIR_AST_TYPE_FUNCTION,
                    KEFIR_SET_ERROR(KEFIR_MALFORMED_ARG, "Structure/union field shall have complete non-function type"));
            }
        }
    }
    return KEFIR_OK;
...
static kefir_result_t analyze_function(struct kefir_mem *mem,
    ...
    REQUIRE(func_type->return_type->tag != KEFIR_AST_TYPE_FUNCTION &&
        func_type->return_type->tag != KEFIR_AST_TYPE_ARRAY,
    ...
    if (func_type->mode == KEFIR_AST_FUNCTION_TYPE_PARAMETERS) {
        for (const struct kefir_list_entry *iter = kefir_list_head(&func_type->parameters);
            iter != NULL;
            kefir_list_next(&iter)) {
            ...
            REQUIRE_OK(kefir_ast_analyze_type(mem, context, KEFIR_AST_TYPE_ANALYSIS_FUNCTION_PARAMETER, param->adjusted_type));
...
kefir_result_t kefir_ast_analyze_type(struct kefir_mem *mem,
                                  const struct kefir_ast_context *context,
                                  kefir_ast_type_analysis_context_t analysis_context,
                                  const struct kefir_ast_type *type) {
