...
kefir_result_t kefir_ast_analyze_while_statement_node(struct kefir_mem *mem, const struct kefir_ast_context *context,
    ...
    REQUIRE(node != NULL, KEFIR_SET_ERROR(KEFIR_MALFORMED_ARG, "Expected valid AST while statement"));
    ...
    REQUIRE(context->flow_control_tree != NULL,
            KEFIR_SET_ERROR(KEFIR_MALFORMED_ARG, "Unable to use while statement in current context"));
    ...
    REQUIRE(node->body->properties.category == KEFIR_AST_NODE_CATEGORY_STATEMENT,
            KEFIR_SET_ERROR(KEFIR_MALFORMED_ARG, "Expected while statement body to be a statement"));
