...
kefir_result_t kefir_ast_format_declarator_specifier(struct kefir_json_output *json,
    ...
    if (display_source_location) {
        ...
        if (kefir_source_location_get(&specifier->source_location, NULL, NULL, NULL)) {
            ...
            REQUIRE_OK(kefir_json_output_string(json, specifier->source_location.source));
            REQUIRE_OK(kefir_json_output_object_key(json, "line"));
            REQUIRE_OK(kefir_json_output_uinteger(json, specifier->source_location.line));
            REQUIRE_OK(kefir_json_output_object_key(json, "column"));
            REQUIRE_OK(kefir_json_output_uinteger(json, specifier->source_location.column));
            REQUIRE_OK(kefir_json_output_object_end(json));
        } else {
            REQUIRE_OK(kefir_json_output_null(json));
        }
