...
static kefir_result_t context_resolve_scoped_ordinary_identifier(const struct kefir_ast_function_declaration_context *context,
...
static kefir_result_t context_resolve_scoped_tag_identifier(const struct kefir_ast_function_declaration_context *context,
                                                           const char *identifier,
                                                           const struct kefir_ast_scoped_identifier **scoped_identifier) {
    REQUIRE(context != NULL, KEFIR_SET_ERROR(KEFIR_MALFORMED_ARG, "Expected valid AST translatation context"));
    REQUIRE(identifier != NULL, KEFIR_SET_ERROR(KEFIR_MALFORMED_ARG, "Expected valid identifier"));
...
static kefir_result_t scoped_context_define_identifier(struct kefir_mem *mem,
    ...
    if (res == KEFIR_OK) {
        ...
        scoped_id =
            kefir_ast_context_allocate_scoped_object_identifier(mem, type, storage_class, NULL,
                KEFIR_AST_SCOPED_IDENTIFIER_NONE_LINKAGE, false, NULL);
...
static kefir_result_t scoped_context_define_constant(struct kefir_mem *mem,
    ...
    struct kefir_ast_scoped_identifier *scoped_id = NULL;
    kefir_result_t res = kefir_ast_identifier_flat_scope_at(&context->ordinary_scope,
...
static kefir_result_t scoped_context_define_tag(struct kefir_mem *mem,
    ...
    struct kefir_ast_scoped_identifier *scoped_id = NULL;
    kefir_result_t res = kefir_ast_identifier_flat_scope_at(&context->tag_scope,
...
static kefir_result_t context_resolve_ordinary_identifier(const struct kefir_ast_context *context,
    ...
    ASSIGN_DECL_CAST(struct kefir_ast_function_declaration_context *, fn_ctx,
        context->payload);
    ...
    if (is_function) {
        ...
        switch (storage_class) {
            ...
            case KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_AUTO:
                return KEFIR_SET_ERROR(KEFIR_MALFORMED_ARG, "Provided specifier is not permitted in declaration of function");
        }
    } else {
