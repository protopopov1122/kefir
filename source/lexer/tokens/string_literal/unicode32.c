...
kefir_result_t kefir_lexer_next_unicode32_string_literal(struct kefir_mem *mem, struct kefir_lexer *lexer,
    ...
    REQUIRE(kefir_lexer_source_cursor_at(lexer->cursor, 0) == U'U' &&
            ...
            KEFIR_SET_ERROR(KEFIR_NO_MATCH, "Unable to match unicode32 string literal"));
    ...
    REQUIRE_CHAIN(&res, kefir_token_new_string_literal_unicode32(mem, literal, literal_length, token));
