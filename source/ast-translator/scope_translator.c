...
kefir_result_t kefir_ast_translator_declare_global_scope_layout(struct kefir_mem *mem,
    ...
    for (const struct kefir_list_entry *iter = kefir_list_head(&global_scope->external_objects);
        ...
        ASSIGN_DECL_CAST(const struct kefir_ast_translator_scoped_identifier *, scoped_identifier,
            iter->value);
