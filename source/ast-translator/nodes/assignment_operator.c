...
static kefir_result_t translate_simple(struct kefir_mem *mem,
    ...
    if (KEFIR_AST_TYPE_IS_SCALAR_TYPE(result_normalized_type)) {
        REQUIRE_OK(kefir_ast_translate_typeconv(builder, value_normalized_type, result_normalized_type));
    }
...
static kefir_result_t translate_multiplication(struct kefir_mem *mem,
    ...
    REQUIRE_OK(kefir_ast_translator_load_value(target_normalized_type, builder));
    REQUIRE_OK(kefir_ast_translate_typeconv(builder, target_normalized_type, common_type));
    REQUIRE_OK(kefir_ast_translate_expression(mem, node->value, builder, context));
    REQUIRE_OK(kefir_ast_translate_typeconv(builder, value_normalized_type, common_type));
    ...
    REQUIRE_OK(kefir_ast_translate_typeconv(builder, common_type, result_normalized_type));
    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IROPCODE_XCHG, 1));
    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IROPCODE_PICK, 1));
    REQUIRE_OK(kefir_ast_translator_store_value(mem, node->base.properties.type, context, builder));
    return KEFIR_OK;
...
static kefir_result_t translate_add(struct kefir_mem *mem,
    ...
    if (KEFIR_AST_TYPE_IS_ARITHMETIC_TYPE(target_normalized_type) &&
        ...
        REQUIRE_OK(kefir_ast_translator_type_cache_generate_owned_object(mem, target_normalized_type->referenced_type, 0,
