...
static kefir_result_t unary_prologue(const struct kefir_ast_translator_context *context,
                                   struct kefir_irbuilder_block *builder,
                                   const struct kefir_ast_unary_operation *node) {
    REQUIRE_OK(kefir_ast_translate_expression(node->arg, builder, context));
...
static kefir_result_t translate_arithmetic_unary(const struct kefir_ast_translator_context *context,
                                               struct kefir_irbuilder_block *builder,
                                               const struct kefir_ast_unary_operation *node) {
    ...
    REQUIRE_OK(unary_prologue(context, builder, node));
...
static kefir_result_t translate_unary_inversion(const struct kefir_ast_translator_context *context,
                                              struct kefir_irbuilder_block *builder,
                                              const struct kefir_ast_unary_operation *node) {
...
static kefir_result_t translate_logical_not_inversion(const struct kefir_ast_translator_context *context,
                                                    struct kefir_irbuilder_block *builder,
                                                    const struct kefir_ast_unary_operation *node) {
...
kefir_result_t kefir_ast_translate_unary_operation_node(const struct kefir_ast_translator_context *context,
                                                    struct kefir_irbuilder_block *builder,
                                                    const struct kefir_ast_unary_operation *node) {
    ...
    switch (node->type) {
        case KEFIR_AST_OPERATION_PLUS:
        case KEFIR_AST_OPERATION_NEGATE:
            return translate_arithmetic_unary(context, builder, node);

        case KEFIR_AST_OPERATION_INVERT:
            return translate_unary_inversion(context, builder, node);

        case KEFIR_AST_OPERATION_LOGICAL_NEGATE:
            return translate_logical_not_inversion(context, builder, node);
