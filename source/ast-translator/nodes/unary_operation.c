...
static kefir_result_t sizeof_type(struct kefir_mem *mem, struct kefir_ast_translator_context *context,
                                  struct kefir_irbuilder_block *builder, const struct kefir_ast_type *type) {
    if (KEFIR_AST_TYPE_IS_VL_ARRAY(type)) {
        ...
        REQUIRE_OK(sizeof_type(mem, context, builder, type->array_type.element_type));
...
static kefir_result_t translate_sizeof(struct kefir_mem *mem, struct kefir_ast_translator_context *context,
    ...
    if (KEFIR_AST_TYPE_IS_VL_ARRAY(node->arg->properties.type) && node->arg->properties.expression_props.lvalue) {
        ...
        REQUIRE_OK(sizeof_type(mem, context, builder, node->arg->properties.type));
    }
    return KEFIR_OK;
}
...
static kefir_result_t translate_alignof(struct kefir_mem *mem, struct kefir_ast_translator_context *context,
    ...
    REQUIRE_OK(unwrap_vla_type(mem, context->ast_context, node->arg->properties.type, &type));
    REQUIRE_OK(KEFIR_AST_TARGET_ENVIRONMENT_GET_TYPE(mem, &context->environment->target_env, type, &opaque_type));
    kefir_result_t res =
        KEFIR_AST_TARGET_ENVIRONMENT_OBJECT_INFO(mem, &context->environment->target_env, opaque_type, NULL, &type_info);
    REQUIRE_ELSE(res == KEFIR_OK, {
