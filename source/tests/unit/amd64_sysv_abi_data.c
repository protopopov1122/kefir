...
DEFINE_CASE(amd64_sysv_abi_data_test5, "AMD64 System V ABI - unaligned data")
    ...
    ASSERT_OK(kefir_irbuilder_type_append_v(&kft_mem, &type, KEFIR_IR_TYPE_INT8, 1, 0));
    ...
    ASSERT_OK(kefir_irbuilder_type_append_v(&kft_mem, &type, KEFIR_IR_TYPE_WORD, 1, 0));
    ASSERT_OK(kefir_irbuilder_type_append_v(&kft_mem, &type, KEFIR_IR_TYPE_FLOAT64, 1, 0));
    ASSERT_OK(kefir_amd64_sysv_type_layout(&type, &kft_mem, &vector));
...
DEFINE_CASE(amd64_sysv_abi_data_test7, "AMD64 System V ABI - array #2")
    ...
    ASSERT_OK(kefir_irbuilder_type_append_v(&kft_mem, &type, KEFIR_IR_TYPE_ARRAY, 0, 5));
    ASSERT_OK(kefir_irbuilder_type_append_v(&kft_mem, &type, KEFIR_IR_TYPE_STRUCT, 0, 2));
    ASSERT_OK(kefir_irbuilder_type_append_v(&kft_mem, &type, KEFIR_IR_TYPE_ARRAY, 0, 100));
    ASSERT_OK(kefir_irbuilder_type_append_v(&kft_mem, &type, KEFIR_IR_TYPE_CHAR, 0, 0));
    ASSERT_OK(kefir_irbuilder_type_append_v(&kft_mem, &type, KEFIR_IR_TYPE_STRUCT, 0, 3));
    ASSERT_OK(kefir_irbuilder_type_append_v(&kft_mem, &type, KEFIR_IR_TYPE_INT64, 0, 0));
...
DEFINE_CASE(amd64_sysv_abi_data_test8, "AMD64 System V ABI - eightbyte allocation")
    ...
    ASSERT_OK(kefir_ir_type_alloc(&kft_mem, 11, &type));
    ASSERT_OK(kefir_irbuilder_type_append_v(&kft_mem, &type, KEFIR_IR_TYPE_STRUCT, 0, 10));
    ASSERT_OK(kefir_irbuilder_type_append_v(&kft_mem, &type, KEFIR_IR_TYPE_INT8, 0, 0));
    ASSERT_OK(kefir_irbuilder_type_append_v(&kft_mem, &type, KEFIR_IR_TYPE_INT8, 0, 0));
...
DEFINE_CASE(amd64_sysv_abi_data_test9, "AMD64 System V ABI - external structure definitions")
    ...
    ASSERT_OK(kefir_irbuilder_type_append_v(&kft_mem, &type1, KEFIR_IR_TYPE_INT8, 0, 0));
    ...
    ASSERT_OK(kefir_irbuilder_type_append_v(&kft_mem, &type2, KEFIR_IR_TYPE_ARRAY, 0, 2));
    ASSERT_OK(kefir_irbuilder_type_append_e(&kft_mem, &type2, &type1, 0));
...
DEFINE_CASE(amd64_sysv_abi_data_test10, "AMD64 System V ABI - bit-fields")
    ...
    struct kefir_vector vector;
    ASSERT_OK(kefir_ir_type_alloc(&kft_mem, 6, &type));
    ASSERT_OK(kefir_irbuilder_type_append_v(&kft_mem, &type, KEFIR_IR_TYPE_BITS, 0, KEFIR_IR_BITS_PARAM(KEFIR_IR_TYPE_INT, 13)));
    ASSERT_OK(kefir_irbuilder_type_append_v(&kft_mem, &type, KEFIR_IR_TYPE_BITS, 0, KEFIR_IR_BITS_PARAM(KEFIR_IR_TYPE_CHAR, 1)));
    ASSERT_OK(kefir_irbuilder_type_append_v(&kft_mem, &type, KEFIR_IR_TYPE_BITS, 0, KEFIR_IR_BITS_PARAM(KEFIR_IR_TYPE_SHORT, 10)));
    ASSERT_OK(kefir_irbuilder_type_append_v(&kft_mem, &type, KEFIR_IR_TYPE_BITS, 0, KEFIR_IR_BITS_PARAM(KEFIR_IR_TYPE_CHAR, 1)));
    ASSERT_OK(kefir_irbuilder_type_append_v(&kft_mem, &type, KEFIR_IR_TYPE_BITS, 0, KEFIR_IR_BITS_PARAM(KEFIR_IR_TYPE_LONG, 1)));
    ASSERT_OK(kefir_irbuilder_type_append_v(&kft_mem, &type, KEFIR_IR_TYPE_BITS, 0, KEFIR_IR_BITS_PARAM(KEFIR_IR_TYPE_INT, 30)));
    ASSERT_OK(kefir_irbuilder_type_append_v(&kft_mem, &type, KEFIR_IR_TYPE_BITS, 0, KEFIR_IR_BITS_PARAM(KEFIR_IR_TYPE_LONG, 35)));
    ASSERT_OK(kefir_amd64_sysv_type_layout(&type, &kft_mem, &vector));
    ASSERT_DATA_ALLOC(&vector, 0, 4, 1, 0);
    ASSERT_DATA_ALLOC(&vector, 1, 1, 1, 4);
    ASSERT_DATA_ALLOC(&vector, 2, 2, 2, 6);
    ASSERT_DATA_ALLOC(&vector, 3, 1, 1, 8);
    ASSERT_DATA_ALLOC(&vector, 4, 7, 1, 9);
    ASSERT_DATA_ALLOC(&vector, 5, 4, 1, 16);
    ASSERT_DATA_ALLOC(&vector, 6, 8, 8, 24);
    ASSERT_OK(kefir_vector_free(&kft_mem, &vector));
    ASSERT_OK(kefir_ir_type_free(&kft_mem, &type));
END_CASE