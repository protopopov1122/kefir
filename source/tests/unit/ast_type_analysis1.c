...
for (kefir_size_t i = 0; i < TYPES_LEN; i++) {
    ...
    ASSERT(
        KEFIR_AST_TYPE_SAME(kefir_ast_type_qualified(&kft_mem, context->type_bundle, TYPES[i], qualification), type2));
...
ASSERT_OK(kefir_ast_enumeration_type_constant(
    ...
    kefir_ast_new_constant_expression(&kft_mem, KEFIR_AST_NODE_BASE(kefir_ast_new_binary_operation(
...
const struct kefir_ast_type *type3 = kefir_ast_type_incomplete_enumeration(&kft_mem, context->type_bundle, "enum1",
...
const struct kefir_ast_type *type3 = kefir_ast_type_array(&kft_mem, context->type_bundle, kefir_ast_type_signed_short(),
                                                          kefir_ast_constant_expression_integer(&kft_mem, 16), NULL);
ASSERT_OK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type3));
...
const struct kefir_ast_type *type5 = kefir_ast_type_array(&kft_mem, context->type_bundle, kefir_ast_type_signed_short(),
                                                          kefir_ast_constant_expression_integer(&kft_mem, 16),
                                                          &(struct kefir_ast_type_qualification){.constant = true});
ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type5));
ASSERT_OK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_FUNCTION_PARAMETER, type5));
...
const struct kefir_ast_type *type6 = kefir_ast_type_array(
    ...
    kefir_ast_new_constant_expression(&kft_mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 5))), NULL);
...
const struct kefir_ast_type *type7 = kefir_ast_type_array_static(
    &kft_mem, context->type_bundle, kefir_ast_type_signed_short(),
    kefir_ast_new_constant_expression(&kft_mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 15))), NULL);
...
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type3, "field1", kefir_ast_type_signed_char(),
                                      NULL));
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type3, "field2", kefir_ast_type_signed_short(),
...
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type3, "field4",
                                      ...
                                      NULL));
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type3, "field5",
                                      kefir_ast_type_unbounded_array(&kft_mem, context->type_bundle,
...
ASSERT(KEFIR_AST_TYPE_SAME(field5->type, kefir_ast_type_unbounded_array(&kft_mem, context->type_bundle,
                                                                        kefir_ast_type_char(), NULL)));
...
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type5, "a",
                                      kefir_ast_type_unbounded_array(&kft_mem, context->type_bundle,
                                                                     kefir_ast_type_char(), NULL),
                                      NULL));
...
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type6, "union_field_two",
                                      kefir_ast_type_unbounded_array(&kft_mem, context->type_bundle,
                                                                     kefir_ast_type_char(), NULL),
                                      NULL));
...
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type8, "void", kefir_ast_type_void(), NULL));
ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type8));

struct kefir_ast_function_type *func_type9 = NULL;
const struct kefir_ast_type *type9f =
    kefir_ast_type_function(&kft_mem, context->type_bundle, kefir_ast_type_void(), "fn", &func_type9);
...
struct kefir_ast_function_type *func_type2 = NULL;
const struct kefir_ast_type *type2 = kefir_ast_type_function(&kft_mem, context->type_bundle, type1, NULL, &func_type2);
ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type2));

struct kefir_ast_function_type *func_type3 = NULL;
const struct kefir_ast_type *type3 =
    kefir_ast_type_function(&kft_mem, context->type_bundle, kefir_ast_type_void(), NULL, &func_type3);
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type3, NULL, kefir_ast_type_void(),
                                            NULL));
ASSERT_OK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type3));

struct kefir_ast_function_type *func_type4 = NULL;
const struct kefir_ast_type *type4 =
    kefir_ast_type_function(&kft_mem, context->type_bundle, kefir_ast_type_void(), NULL, &func_type4);
...
const struct kefir_ast_type *type5 =
    kefir_ast_type_function(&kft_mem, context->type_bundle, kefir_ast_type_void(), NULL, &func_type5);
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type5, NULL, kefir_ast_type_void(),
                                            NULL));
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type5, NULL,
...
const struct kefir_ast_type *type6 =
    kefir_ast_type_function(&kft_mem, context->type_bundle, kefir_ast_type_void(), NULL, &func_type6);
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type6, NULL, struct_type6, NULL));
ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type6));
...
const struct kefir_ast_type *type7 =
    kefir_ast_type_function(&kft_mem, context->type_bundle, kefir_ast_type_void(), NULL, &func_type7);
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type7, NULL, struct_type7, NULL));
...
const struct kefir_ast_type *type8 =
    kefir_ast_type_function(&kft_mem, context->type_bundle, kefir_ast_type_void(), NULL, &func_type8);
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type8, NULL,
                                            kefir_ast_type_unbounded_array(&kft_mem, context->type_bundle,
                                                                           kefir_ast_type_bool(), NULL),
                                            NULL));
...
struct kefir_ast_function_type *func_type9 = NULL;
const struct kefir_ast_type *type9 =
    kefir_ast_type_function(&kft_mem, context->type_bundle, kefir_ast_type_void(), NULL, &func_type9);
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type9, NULL,
                                            kefir_ast_type_signed_int(),
...
const struct kefir_ast_type *type10 =
    kefir_ast_type_function(&kft_mem, context->type_bundle, kefir_ast_type_void(), NULL, &func_type10);
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type10, NULL,
                                            kefir_ast_type_signed_int(),
                                            &(kefir_ast_scoped_identifier_storage_t){
...
const struct kefir_ast_type *type11 =
    kefir_ast_type_function(&kft_mem, context->type_bundle, kefir_ast_type_void(), NULL, &func_type11);
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type11, NULL,
                                            kefir_ast_type_signed_int(), NULL));
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type11, NULL, kefir_ast_type_float(),
                                            NULL));
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type11, NULL,
                                            kefir_ast_type_pointer(&kft_mem, context->type_bundle,
                                                                   kefir_ast_type_void()),
...
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type1, "field1", kefir_ast_type_signed_short(),
                                      NULL));
...
struct kefir_ast_struct_type *struct_type2 = NULL;
const struct kefir_ast_type *type2 = kefir_ast_type_structure(&kft_mem, context->type_bundle, "struct2", &struct_type2);
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type2, "field1",
                                      kefir_ast_type_unbounded_array(&kft_mem, context->type_bundle,
                                                                     kefir_ast_type_char(), NULL),
                                      NULL));
ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type2));
...
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type3, "field1",
                                      kefir_ast_type_unbounded_array(&kft_mem, context->type_bundle,
                                                                     kefir_ast_type_char(), NULL),
                                      NULL));
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type3, "field2", kefir_ast_type_double(),
                                      NULL));
...
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, union_type4, "field1", kefir_ast_type_unsigned_int(),
                                      NULL));
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, union_type4, "field2",
                                      kefir_ast_type_unbounded_array(&kft_mem, context->type_bundle,
...
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type5, "inner1", kefir_ast_type_signed_short(),
                                      NULL));
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type5, "inner2",
                                      kefir_ast_type_pointer(&kft_mem, context->type_bundle, kefir_ast_type_void()),
                                      NULL));
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type5, "inner3",
                                      kefir_ast_type_unbounded_array(&kft_mem, context->type_bundle,
...
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type6, "outer1", kefir_ast_type_signed_int(),
                                      NULL));
