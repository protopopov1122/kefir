...
DEFINE_CASE(ast_type_analysis_scalars, "AST type analysis - scalars")
    ...
    for (kefir_size_t i = 0; i < TYPES_LEN; i++) {
        ...
        type1 = TYPES[i];
        ASSERT_OK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_FUNCTION_PARAMETER, type1));
        ASSERT(KEFIR_AST_TYPE_SAME(TYPES[i], type1));
    }
...
DEFINE_CASE(ast_type_analysis_pointers_qualifications, "AST type analysis - pointers & qualifications")
    ...
    const struct kefir_ast_type *type1 = kefir_ast_type_function(&kft_mem, context->type_bundle,
        kefir_ast_type_void(), "", &func_type1);
    ...
    for (kefir_size_t i = 0; i < TYPES_LEN; i++) {
        ...
        const struct kefir_ast_type *type2 = kefir_ast_type_qualified(&kft_mem, context->type_bundle, TYPES[i], qualification);
        ASSERT_OK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type2));
        ASSERT(KEFIR_AST_TYPE_SAME(kefir_ast_type_qualified(&kft_mem, context->type_bundle, TYPES[i], qualification), type2));

        type2 = kefir_ast_type_qualified(&kft_mem, context->type_bundle, TYPES[i], qualification);
        ASSERT_OK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_FUNCTION_PARAMETER, type2));
        ASSERT(KEFIR_AST_TYPE_SAME(kefir_ast_type_qualified(&kft_mem, context->type_bundle, TYPES[i], qualification), type2));
    }

    const struct kefir_ast_type *type4 = kefir_ast_type_pointer(&kft_mem, context->type_bundle,
        kefir_ast_type_array_static(&kft_mem, context->type_bundle, kefir_ast_type_unsigned_long(),
            kefir_ast_constant_expression_integer(&kft_mem, 16), &(struct kefir_ast_type_qualification){
                .constant = true
            }));
    ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type4));
    ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_FUNCTION_PARAMETER, type4));

    const struct kefir_ast_type *type5 = kefir_ast_type_qualified(&kft_mem, context->type_bundle,
        kefir_ast_type_array_static(&kft_mem, context->type_bundle, kefir_ast_type_unsigned_long(),
            kefir_ast_constant_expression_integer(&kft_mem, 16), &(struct kefir_ast_type_qualification){
                .constant = true
            }), (struct kefir_ast_type_qualification) {
    ...
    ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type5));
    ASSERT_OK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_FUNCTION_PARAMETER, type5));
...
DEFINE_CASE(ast_type_analysis_enumerations, "AST type analysis - enumerations")
    ...
    const struct kefir_ast_type *type1 = kefir_ast_type_enumeration(&kft_mem, context->type_bundle,
        "", type_traits->underlying_enumeration_type, &enum_type1);

    ASSERT_OK(kefir_ast_enumeration_type_constant_auto(&kft_mem, context->symbols,
        enum_type1, "A"));
    ASSERT_OK(kefir_ast_enumeration_type_constant_auto(&kft_mem, context->symbols,
        enum_type1, "B"));
    ASSERT_OK(kefir_ast_enumeration_type_constant_auto(&kft_mem, context->symbols,
    ...
    ASSERT_OK(kefir_ast_enumeration_type_constant(&kft_mem, context->symbols, enum_type1,
        "X", kefir_ast_new_constant_expression(&kft_mem,
    ...
    const struct kefir_ast_type *type2 = kefir_ast_type_enumeration(&kft_mem, context->type_bundle,
        "", kefir_ast_type_float(), &enum_type2);
    ASSERT_OK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type2));

    const struct kefir_ast_type *type3 = kefir_ast_type_incomplete_enumeration(&kft_mem, context->type_bundle,
        "enum1", type_traits->underlying_enumeration_type);
    ASSERT_OK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type3));

    const struct kefir_ast_type *type4 = kefir_ast_type_incomplete_enumeration(&kft_mem, context->type_bundle,
        "enum2", kefir_ast_type_void());
    ASSERT_OK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type4));
...
DEFINE_CASE(ast_type_analysis_arrays, "AST type analysis - arrays")
    ...
    const struct kefir_ast_type *type1 = kefir_ast_type_array(&kft_mem, context->type_bundle,
        kefir_ast_type_void(), kefir_ast_constant_expression_integer(&kft_mem, 1), NULL);
    ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type1));
    ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_FUNCTION_PARAMETER, type1));
    ...
    const struct kefir_ast_type *type2 = kefir_ast_type_array(&kft_mem, context->type_bundle,
        kefir_ast_type_function(&kft_mem, context->type_bundle,
            kefir_ast_type_char(), "", &func_type2), kefir_ast_constant_expression_integer(&kft_mem, 2), NULL);
    ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type2));
    ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_FUNCTION_PARAMETER, type2));

    const struct kefir_ast_type *type3 = kefir_ast_type_array(&kft_mem, context->type_bundle,
        kefir_ast_type_signed_short(), kefir_ast_constant_expression_integer(&kft_mem, 16), NULL);
    ASSERT_OK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type3));
    ASSERT_OK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_FUNCTION_PARAMETER, type3));

    const struct kefir_ast_type *type4 = kefir_ast_type_array_static(&kft_mem, context->type_bundle,
        kefir_ast_type_signed_short(), kefir_ast_constant_expression_integer(&kft_mem, 16), NULL);
    ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type4));
    ASSERT_OK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_FUNCTION_PARAMETER, type4));

    const struct kefir_ast_type *type5 = kefir_ast_type_array(&kft_mem, context->type_bundle,
        kefir_ast_type_signed_short(), kefir_ast_constant_expression_integer(&kft_mem, 16),
            &(struct kefir_ast_type_qualification){
                .constant = true
            });
    ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type5));
    ...
    const struct kefir_ast_type *type6 = kefir_ast_type_array(&kft_mem, context->type_bundle,
        kefir_ast_type_signed_short(), kefir_ast_new_constant_expression(&kft_mem,
            KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 5))), NULL);
    ...
    const struct kefir_ast_type *type7 = kefir_ast_type_array_static(&kft_mem, context->type_bundle,
        kefir_ast_type_signed_short(), kefir_ast_new_constant_expression(&kft_mem,
            KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 15))), NULL);
    ASSERT_OK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_FUNCTION_PARAMETER, type7));
    ...
    const struct kefir_ast_type *type8 = kefir_ast_type_array(&kft_mem, context->type_bundle,
        kefir_ast_type_signed_short(), kefir_ast_new_constant_expression(&kft_mem,
            KEFIR_AST_NODE_BASE(kefir_ast_new_constant_float(&kft_mem, 3.1))), NULL);
    ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type8));

    const struct kefir_ast_type *type9 = kefir_ast_type_vlen_array(&kft_mem, context->type_bundle,
        kefir_ast_type_signed_short(), KEFIR_AST_NODE_BASE(kefir_ast_new_constant_float(&kft_mem, 1.27)), NULL);
    ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type9));

    const struct kefir_ast_type *type10 = kefir_ast_type_vlen_array(&kft_mem, context->type_bundle,
        kefir_ast_type_signed_short(), KEFIR_AST_NODE_BASE(kefir_ast_new_constant_long(&kft_mem, 31)), NULL);
    ASSERT_OK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type10));
    ASSERT(KEFIR_AST_TYPE_IS_INTEGRAL_TYPE(type10->array_type.vla_length->properties.type));

    const struct kefir_ast_type *type11 = kefir_ast_type_unbounded_array(&kft_mem, context->type_bundle,
        kefir_ast_type_double(), NULL);
    ASSERT_OK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type11));
...
DEFINE_CASE(ast_type_analysis_structs, "AST type analysis - structures/unions")
    ...
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type3,
        "field1", kefir_ast_type_signed_char(), NULL));
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type3,
        "field2", kefir_ast_type_signed_short(),
        kefir_ast_alignment_as_type(&kft_mem, kefir_ast_type_char())));
    ASSERT_OK(kefir_ast_struct_type_bitfield(&kft_mem, context->symbols, struct_type3,
        "field3", kefir_ast_type_signed_long(), NULL,
        kefir_ast_new_constant_expression(&kft_mem,
            KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 2)))));
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type3,
        "field4", kefir_ast_type_array(&kft_mem, context->type_bundle, kefir_ast_type_double(),
            kefir_ast_constant_expression_integer(&kft_mem, 8), NULL), NULL));
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type3,
        "field5", kefir_ast_type_unbounded_array(&kft_mem, context->type_bundle, kefir_ast_type_char(), NULL), NULL));
    ASSERT_OK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type3));
    ...
    ASSERT(KEFIR_AST_TYPE_SAME(field4->type, kefir_ast_type_array(&kft_mem, context->type_bundle, kefir_ast_type_double(),
    ...
    ASSERT(KEFIR_AST_TYPE_SAME(field5->type, kefir_ast_type_unbounded_array(&kft_mem,
        context->type_bundle, kefir_ast_type_char(), NULL)));
    ...
    const struct kefir_ast_type *type4 = kefir_ast_type_structure(&kft_mem, context->type_bundle,
        NULL, &struct_type4);
    ASSERT_OK(kefir_ast_struct_type_bitfield(&kft_mem, context->symbols, struct_type4,
        "x", kefir_ast_type_signed_long(), NULL,
        kefir_ast_new_constant_expression(&kft_mem,
            KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, -1)))));
    ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type4));

    struct kefir_ast_struct_type *struct_type5 = NULL;
    const struct kefir_ast_type *type5 = kefir_ast_type_structure(&kft_mem, context->type_bundle,
        NULL, &struct_type5);
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type5,
        "a", kefir_ast_type_unbounded_array(&kft_mem, context->type_bundle, kefir_ast_type_char(), NULL), NULL));
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type5,
        "b", kefir_ast_type_bool(), NULL));
    ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type5));

    struct kefir_ast_struct_type *struct_type6 = NULL;
    const struct kefir_ast_type *type6 = kefir_ast_type_union(&kft_mem, context->type_bundle,
        NULL, &struct_type6);
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type6,
        "union_field_one", kefir_ast_type_bool(), NULL));
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type6,
        "union_field_two", kefir_ast_type_unbounded_array(&kft_mem, context->type_bundle, kefir_ast_type_char(), NULL), NULL));
    ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type6));

    struct kefir_ast_struct_type *struct_type7 = NULL;
    const struct kefir_ast_type *type7 = kefir_ast_type_structure(&kft_mem, context->type_bundle,
        NULL, &struct_type7);
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type7,
        "ABC", kefir_ast_type_vlen_array(&kft_mem, context->type_bundle, kefir_ast_type_float(),
            KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 10)), NULL), NULL));
    ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type7));

    struct kefir_ast_struct_type *struct_type8 = NULL;
    const struct kefir_ast_type *type8 = kefir_ast_type_structure(&kft_mem, context->type_bundle,
        NULL, &struct_type8);
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type8,
        "void", kefir_ast_type_void(), NULL));
    ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type8));

    struct kefir_ast_function_type *func_type9 = NULL;
    const struct kefir_ast_type *type9f = kefir_ast_type_function(&kft_mem, context->type_bundle,
        kefir_ast_type_void(), "fn", &func_type9);
    struct kefir_ast_struct_type *struct_type9 = NULL;
    const struct kefir_ast_type *type9 = kefir_ast_type_structure(&kft_mem, context->type_bundle,
        NULL, &struct_type9);
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type9,
        "function", type9f, NULL));
    ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type9));
...
DEFINE_CASE(ast_type_analysis_functions, "AST type analysis - functions")
    ...
    const struct kefir_ast_type *type1 = kefir_ast_type_function(&kft_mem, context->type_bundle,
        kefir_ast_type_array(&kft_mem, context->type_bundle, kefir_ast_type_char(),
            kefir_ast_constant_expression_integer(&kft_mem, 1), NULL), NULL, &func_type1);
    ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type1));

    struct kefir_ast_function_type *func_type2 = NULL;
    const struct kefir_ast_type *type2 = kefir_ast_type_function(&kft_mem, context->type_bundle,
        type1, NULL, &func_type2);
    ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type2));
    ...
    const struct kefir_ast_type *type3 = kefir_ast_type_function(&kft_mem, context->type_bundle,
        kefir_ast_type_void(), NULL, &func_type3);
    ...
    const struct kefir_ast_type *type4 = kefir_ast_type_function(&kft_mem, context->type_bundle,
        kefir_ast_type_void(), NULL, &func_type4);
    ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type4,
        "somethin", kefir_ast_type_void(), NULL));
    ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type4));
    ...
    const struct kefir_ast_type *type5 = kefir_ast_type_function(&kft_mem, context->type_bundle,
        kefir_ast_type_void(), NULL, &func_type5);
    ...
    const struct kefir_ast_type *struct_type6 = kefir_ast_type_incomplete_structure(&kft_mem, context->type_bundle, "S");
    struct kefir_ast_function_type *func_type6 = NULL;
    const struct kefir_ast_type *type6 = kefir_ast_type_function(&kft_mem, context->type_bundle,
        kefir_ast_type_void(), NULL, &func_type6);
    ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type6,
        NULL, struct_type6, NULL));
    ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type6));

    struct kefir_ast_struct_type *structure7 = NULL;
    const struct kefir_ast_type *struct_type7 = kefir_ast_type_structure(&kft_mem, context->type_bundle, "S2", &structure7);
    struct kefir_ast_function_type *func_type7 = NULL;
    const struct kefir_ast_type *type7 = kefir_ast_type_function(&kft_mem, context->type_bundle,
        kefir_ast_type_void(), NULL, &func_type7);
    ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type7,
        NULL, struct_type7, NULL));
    ASSERT_OK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type7));

    struct kefir_ast_function_type *func_type8 = NULL;
    const struct kefir_ast_type *type8 = kefir_ast_type_function(&kft_mem, context->type_bundle,
        kefir_ast_type_void(), NULL, &func_type8);
    ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type8,
        NULL, kefir_ast_type_unbounded_array(&kft_mem, context->type_bundle, kefir_ast_type_bool(), NULL), NULL));
    ASSERT_OK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type8));

    struct kefir_ast_function_type *func_type9 = NULL;
    const struct kefir_ast_type *type9 = kefir_ast_type_function(&kft_mem, context->type_bundle,
        kefir_ast_type_void(), NULL, &func_type9);
    ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type9,
        NULL, kefir_ast_type_signed_int(), &(kefir_ast_scoped_identifier_storage_t){KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_AUTO}));
    ASSERT_NOK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type9));

    struct kefir_ast_function_type *func_type10 = NULL;
    const struct kefir_ast_type *type10 = kefir_ast_type_function(&kft_mem, context->type_bundle,
        kefir_ast_type_void(), NULL, &func_type10);
    ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type10,
        NULL, kefir_ast_type_signed_int(), &(kefir_ast_scoped_identifier_storage_t){KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_REGISTER}));
    ASSERT_OK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type10));

    struct kefir_ast_function_type *func_type11 = NULL;
    const struct kefir_ast_type *type11 = kefir_ast_type_function(&kft_mem, context->type_bundle,
        kefir_ast_type_void(), NULL, &func_type11);
    ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type11,
        NULL, kefir_ast_type_signed_int(), NULL));
    ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type11,
        NULL, kefir_ast_type_float(), NULL));
    ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type11,
        NULL, kefir_ast_type_pointer(&kft_mem, context->type_bundle, kefir_ast_type_void()), NULL));
    ASSERT_OK(kefir_ast_analyze_type(&kft_mem, context, KEFIR_AST_TYPE_ANALYSIS_DEFAULT, type11));
