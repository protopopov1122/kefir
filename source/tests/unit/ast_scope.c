...
#define ASSERT_RESOLVED_IDENTIFIER(_context, _identifier, _storage, _type, _linkage) \
    do { \
        ...
        ASSERT(KEFIR_AST_TYPE_COMPATIBLE(__scoped_id->object.type, (_type))); \
...
DEFINE_CASE(ast_scope_rules7, "AST Declaration scoping - local extern")
    ...
    ASSERT_OK(kefir_ast_context_declare_external(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_context_declare_external(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_define_static(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_define_static_thread_local(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_declare_external_thread_local(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_declare_external(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_long()));
    ASSERT_OK(kefir_ast_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_context_declare_external(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_declare_external(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_long()));
    ASSERT_OK(kefir_ast_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_context_define_static(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_context_declare_external(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_declare_external_thread_local(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_declare_external(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_long()));
...
DEFINE_CASE(ast_scope_rules8, "AST Declaration scoping - local extern thread_local")
    ...
    ASSERT_OK(kefir_ast_context_declare_external_thread_local(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_context_declare_external_thread_local(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_define_static(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_define_static_thread_local(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_declare_external(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_declare_external_thread_local(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_long()));
    ASSERT_OK(kefir_ast_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_context_declare_external_thread_local(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_declare_external_thread_local(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_long()));
    ASSERT_OK(kefir_ast_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_context_define_static_thread_local(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_context_declare_external_thread_local(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_declare_external(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_declare_external_thread_local(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_long()));
...
DEFINE_CASE(ast_scope_rules9, "AST Declaration scoping - local static")
    ...
    ASSERT_OK(kefir_ast_context_define_static(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_define_static(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_declare_external(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_context_define_static(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_define_static(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_context_define_static_thread_local(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_define_static_thread_local(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_context_pop_block_scope(&context));
    ASSERT_NOK(kefir_ast_context_define_static(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_context_pop_block_scope(&context));
    ASSERT_NOK(kefir_ast_context_define_static(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int()));
...
DEFINE_CASE(ast_scope_rules10, "AST Declaration scoping - local static thread_local")
    ...
    ASSERT_OK(kefir_ast_context_define_static_thread_local(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_define_static_thread_local(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_declare_external_thread_local(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_context_define_static_thread_local(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_define_static_thread_local(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_context_push_block_scope(&kft_mem, &context));
...
DEFINE_CASE(ast_scope_rules11, "AST Declaration scoping - local auto")
    ...
    ASSERT_OK(kefir_ast_context_define_auto(&kft_mem, &context, "auto_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_define_auto(&kft_mem, &context, "auto_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_define_static(&kft_mem, &context, "auto_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_define_static_thread_local(&kft_mem, &context, "auto_int1", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_context_define_auto(&kft_mem, &context, "auto_int1", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_context_define_auto(&kft_mem, &context, "auto_int2", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_define_auto(&kft_mem, &context, "auto_int2", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_context_define_auto(&kft_mem, &context, "auto_int2", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_define_register(&kft_mem, &context, "auto_int2", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_context_define_register(&kft_mem, &context, "auto_int3", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_context_define_static(&kft_mem, &context, "auto_int1", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_context_pop_block_scope(&context));
    ASSERT_OK(kefir_ast_context_pop_block_scope(&context));
    ASSERT_OK(kefir_ast_context_pop_block_scope(&context));
...
DEFINE_CASE(ast_scope_rules12, "AST Declaration scoping - local register")
    ...
    ASSERT_OK(kefir_ast_context_define_register(&kft_mem, &context, "auto_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_define_register(&kft_mem, &context, "auto_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_define_static(&kft_mem, &context, "auto_int1", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_define_static_thread_local(&kft_mem, &context, "auto_int1", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_context_define_register(&kft_mem, &context, "auto_int1", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_context_define_register(&kft_mem, &context, "auto_int2", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_define_register(&kft_mem, &context, "auto_int2", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_context_define_register(&kft_mem, &context, "auto_int2", kefir_ast_type_signed_int()));
    ASSERT_NOK(kefir_ast_context_define_auto(&kft_mem, &context, "auto_int2", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_context_define_auto(&kft_mem, &context, "auto_int3", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_context_define_static(&kft_mem, &context, "auto_int1", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_context_pop_block_scope(&context));
    ASSERT_OK(kefir_ast_context_pop_block_scope(&context));
...
DEFINE_CASE(ast_scope_rules13, "AST Declaration scoping - block scoping")
    ...
    ASSERT_OK(kefir_ast_global_context_declare_external(
        &kft_mem, &global_context, "variable1", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_global_context_declare_external_thread_local(
        &kft_mem, &global_context, "variable2", kefir_ast_type_signed_long()));
    ASSERT_OK(kefir_ast_global_context_define_external(
        &kft_mem, &global_context, "variable1", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_global_context_define_external(
        &kft_mem, &global_context, "variable3", kefir_ast_type_bool()));
    ASSERT_OK(kefir_ast_global_context_define_static(
        &kft_mem, &global_context, "variable4", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_global_context_declare_external(
        &kft_mem, &global_context, "variable4", kefir_ast_type_signed_int()));
    ASSERT_OK(kefir_ast_global_context_define_static_thread_local(
        &kft_mem, &global_context, "variable5", kefir_ast_type_signed_int()));
    ...
    ASSERT_OK(kefir_ast_context_declare_external(
        &kft_mem, &context, "variable1", kefir_ast_type_signed_int()));
    ASSERT_RESOLVED_IDENTIFIER(&context, "variable1", KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_EXTERN,
        kefir_ast_type_signed_int(), KEFIR_AST_SCOPED_IDENTIFIER_EXTERNAL_LINKAGE);
    ASSERT_RESOLVED_IDENTIFIER(&context, "variable2", KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_EXTERN_THREAD_LOCAL,
        kefir_ast_type_signed_long(), KEFIR_AST_SCOPED_IDENTIFIER_EXTERNAL_LINKAGE);
    ASSERT_OK(kefir_ast_context_define_static(
        &kft_mem, &context, "variable2", kefir_ast_type_float()));
    ...
    ASSERT_OK(kefir_ast_context_define_auto(
        &kft_mem, &context, "variable10", kefir_ast_type_unsigned_int()));
    ...
    ASSERT_OK(kefir_ast_context_define_static_thread_local(
        &kft_mem, &context, "variable2", kefir_ast_type_unsigned_int()));
