...
#define ASSERT_TYPE_SPECIFIER(_id, _spec)                                                            \
    ...
    ASSERT_TYPE_SPECIFIER(boolean, KEFIR_AST_TYPE_SPECIFIER_BOOL);
    ASSERT_TYPE_SPECIFIER(complex, KEFIR_AST_TYPE_SPECIFIER_COMPLEX);
    ASSERT_TYPE_SPECIFIER(va_list, KEFIR_AST_TYPE_SPECIFIER_VA_LIST);
...
DEFINE_CASE(ast_declarator_specifier_construction6,
            "AST declarator specifiers - typedef type specifier construction #1") {
...
DEFINE_CASE(ast_declarator_specifier_construction7,
            "AST declarator specifiers - typedef type specifier construction #2") {
    const char *TYPE_NAMES[] = {
        "", "Hello_world", "Goodbye_cruel_world", "one_two_THREE", "A", "some_pretty_long_type_name_1"};
    const kefir_size_t TYPE_NAME_COUNT = sizeof(TYPE_NAMES) / sizeof(TYPE_NAMES[0]);
    struct kefir_symbol_table symbols;
...
DEFINE_CASE(ast_declarator_specifier_construction8,
