...
DEFINE_CASE(ast_initializer_construction1, "AST initializer - construction #1")
    ...
    struct kefir_ast_initializer *init3 = kefir_ast_new_expression_initializer(&kft_mem,
        KEFIR_AST_NODE_BASE(kefir_ast_new_identifier(&kft_mem, context->symbols, "test")));
    ASSERT(init3 != NULL);
...
DEFINE_CASE(ast_initializer_construction2, "AST initializer - construction #2")
    ...
    ASSERT_OK(kefir_ast_initializer_list_append(&kft_mem, &init1->list, NULL,
    ...
    ASSERT_OK(kefir_ast_initializer_list_append(&kft_mem, &init1->list, NULL,
        kefir_ast_new_expression_initializer(&kft_mem,
            KEFIR_AST_NODE_BASE(kefir_ast_new_identifier(&kft_mem, context->symbols, "abc")))));
