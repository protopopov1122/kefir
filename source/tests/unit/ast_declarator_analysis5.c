...
struct kefir_ast_structure_specifier *specifier1 =
    kefir_ast_structure_specifier_init(&kft_mem, context->symbols, "param1_struct", true);
...
struct kefir_ast_declaration *param1 =
    kefir_ast_new_declaration(&kft_mem, kefir_ast_declarator_identifier(&kft_mem, context->symbols, "param1"), NULL);
...
struct kefir_ast_declaration *param2 =
    kefir_ast_new_declaration(&kft_mem, kefir_ast_declarator_identifier(&kft_mem, context->symbols, "param2"), NULL);
...
struct kefir_ast_declaration *param4 =
    kefir_ast_new_declaration(&kft_mem, kefir_ast_declarator_identifier(&kft_mem, context->symbols, "paramN"), NULL);
...
struct kefir_ast_declarator *decl0 =
    kefir_ast_declarator_function(&kft_mem, kefir_ast_declarator_identifier(&kft_mem, context->symbols, "someFunc"));
...
const struct kefir_ast_type *type1 =
    kefir_ast_type_structure(&kft_mem, context->type_bundle, "param1_struct", &struct_type1);
...
const struct kefir_ast_type *type2 = kefir_ast_type_function(
    ...
    "someFunc", &func_type2);
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type2, "param1", type1,
                                            &(kefir_ast_scoped_identifier_storage_t){
                                                KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_UNKNOWN}));
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type2, "param2", type1,
                                            &(kefir_ast_scoped_identifier_storage_t){
                                                KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_UNKNOWN}));
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type2, NULL,
                                            kefir_ast_type_pointer(&kft_mem, context->type_bundle, type1),
                                            &(kefir_ast_scoped_identifier_storage_t){
                                                KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_UNKNOWN}));
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type2, "paramN",
...
struct kefir_ast_structure_specifier *specifier1 =
    kefir_ast_structure_specifier_init(&kft_mem, context->symbols, "oneStructure", true);
struct kefir_ast_structure_declaration_entry *entry1 = kefir_ast_structure_declaration_entry_alloc(&kft_mem);
ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &entry1->declaration.specifiers,
...
ASSERT_OK(kefir_ast_structure_declaration_entry_append(
    &kft_mem, entry1,
    kefir_ast_declarator_pointer(&kft_mem, kefir_ast_declarator_identifier(&kft_mem, context->symbols, "content")),
    NULL));
ASSERT_OK(kefir_ast_structure_specifier_append_entry(&kft_mem, specifier1, entry1));
...
ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &entry2->declaration.specifiers,
                                                     kefir_ast_type_specifier_int(&kft_mem)));
ASSERT_OK(kefir_ast_structure_declaration_entry_append(
    &kft_mem, entry2, kefir_ast_declarator_identifier(&kft_mem, context->symbols, "length"), NULL));
...
ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &decl1->specifiers,
                                                     kefir_ast_type_specifier_struct(&kft_mem, specifier1)));
ASSERT_OK(kefir_ast_analyze_declaration(&kft_mem, context, &decl1->specifiers, decl1->declarator, NULL, NULL, NULL,
                                        NULL, NULL));

struct kefir_ast_struct_type *struct_type1 = NULL;
const struct kefir_ast_type *type1 =
    kefir_ast_type_structure(&kft_mem, context->type_bundle, "oneStructure", &struct_type1);
ASSERT_OK(kefir_ast_struct_type_field(
    &kft_mem, context->symbols, struct_type1, "content",
    ...
    NULL));
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type1, "length", kefir_ast_type_unsigned_int(),
                                      NULL));

struct kefir_ast_declaration *param1 =
    kefir_ast_new_declaration(&kft_mem, kefir_ast_declarator_identifier(&kft_mem, context->symbols, "param1"), NULL);
ASSERT_OK(kefir_ast_declarator_specifier_list_append(
    &kft_mem, &param1->specifiers,
    kefir_ast_type_specifier_struct(&kft_mem, kefir_ast_structure_specifier_init(&kft_mem, context->symbols,
                                                                                 "oneStructure", false))));
...
struct kefir_ast_declaration *param3_1 =
    kefir_ast_new_declaration(&kft_mem, kefir_ast_declarator_identifier(&kft_mem, context->symbols, "num"), NULL);
...
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type2, "num", kefir_ast_type_float(),
                                            &(kefir_ast_scoped_identifier_storage_t){
                                                KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_UNKNOWN}));
...
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type3, "param1", type1,
                                            &(kefir_ast_scoped_identifier_storage_t){
                                                KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_UNKNOWN}));
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type3, NULL, kefir_ast_type_char(),
                                            &(kefir_ast_scoped_identifier_storage_t){
                                                KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_REGISTER}));
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type3, "callback", type2,
                                            &(kefir_ast_scoped_identifier_storage_t){
                                                KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_REGISTER}));
...
struct kefir_ast_declarator *decl1 = kefir_ast_declarator_function(
    &kft_mem,
    kefir_ast_declarator_pointer(&kft_mem, kefir_ast_declarator_identifier(&kft_mem, context->symbols, "callback")));
...
ASSERT_OK(kefir_ast_structure_declaration_entry_append(
    &kft_mem, entry2,
    kefir_ast_declarator_pointer(&kft_mem, kefir_ast_declarator_identifier(&kft_mem, context->symbols, "payload")),
    NULL));
ASSERT_OK(kefir_ast_structure_specifier_append_entry(&kft_mem, specifier1, entry2));
...
kefir_ast_function_specifier_t func_specifier2 = KEFIR_AST_FUNCTION_SPECIFIER_NONE;
kefir_size_t alignment2 = 0;
ASSERT_OK(kefir_ast_analyze_declaration(&kft_mem, context, &decl_specifiers, decl, &identifier2, &decl_type2, &storage2,
                                        &func_specifier2, &alignment2));
...
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type2, NULL, type1,
                                            &(kefir_ast_scoped_identifier_storage_t){
                                                KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_UNKNOWN}));

struct kefir_ast_struct_type *struct_type3 = NULL;
const struct kefir_ast_type *type3 =
    kefir_ast_type_structure(&kft_mem, context->type_bundle, "callback_S", &struct_type3);
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type3, "callback",
                                      kefir_ast_type_pointer(&kft_mem, context->type_bundle, type2), NULL));
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type3, "payload", type1, NULL));
...
ASSERT_OK(kefir_ast_structure_specifier_append_entry(
    &kft_mem, specifier1,
...
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type1, "x", kefir_ast_type_signed_int(),
                                      NULL));
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type1, "y", kefir_ast_type_float(), NULL));
...
ASSERT(kefir_ast_analyze_declaration(&kft_mem, context, &decl_specifiers, NULL, &identifier, &type, &storage,
                                     &func_specifier, &alignment) == KEFIR_STATIC_ASSERT);
