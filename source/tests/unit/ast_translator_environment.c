...
#define ASSERT_OBJECT_OFFSET(_mem, _env, _type, _min, _max, _size) \
    do { \
        kefir_ast_target_environment_opaque_type_t opaque_type; \
        ASSERT_OK(KEFIR_AST_TARGET_ENVIRONMENT_GET_TYPE((_mem), (_env), \
            (_type), &opaque_type)); \
...
DEFINE_CASE(ast_translator_environment2, "AST translator - environment object offset")
    ...
    ASSERT_OBJECT_OFFSET(&kft_mem, &env.target_env,
        kefir_ast_type_char(), -100, 100, 1);
    ASSERT_OBJECT_OFFSET(&kft_mem, &env.target_env,
        kefir_ast_type_unsigned_char(), -100, 100, 1);
    ASSERT_OBJECT_OFFSET(&kft_mem, &env.target_env,
        kefir_ast_type_signed_char(), -100, 100, 1);

    ASSERT_OBJECT_OFFSET(&kft_mem, &env.target_env,
        kefir_ast_type_signed_short(), -100, 100, 2);
    ASSERT_OBJECT_OFFSET(&kft_mem, &env.target_env,
        kefir_ast_type_unsigned_short(), -100, 100, 2);

    ASSERT_OBJECT_OFFSET(&kft_mem, &env.target_env,
        kefir_ast_type_signed_int(), -100, 100, 4);
    ASSERT_OBJECT_OFFSET(&kft_mem, &env.target_env,
        kefir_ast_type_unsigned_int(), -100, 100, 4);
    ASSERT_OBJECT_OFFSET(&kft_mem, &env.target_env,
        kefir_ast_type_float(), -100, 100, 4);

    ASSERT_OBJECT_OFFSET(&kft_mem, &env.target_env,
        kefir_ast_type_signed_long(), -100, 100, 8);
    ASSERT_OBJECT_OFFSET(&kft_mem, &env.target_env,
        kefir_ast_type_unsigned_long(), -100, 100, 8);
    ASSERT_OBJECT_OFFSET(&kft_mem, &env.target_env,
        kefir_ast_type_signed_long_long(), -100, 100, 8);
    ASSERT_OBJECT_OFFSET(&kft_mem, &env.target_env,
        kefir_ast_type_unsigned_long_long(), -100, 100, 8);
    ASSERT_OBJECT_OFFSET(&kft_mem, &env.target_env,
        kefir_ast_type_double(), -100, 100, 8);
    ...
    ASSERT_OBJECT_OFFSET(&kft_mem, &env.target_env,
        kefir_ast_type_pointer(&kft_mem, &type_bundle,
            kefir_ast_type_pointer(&kft_mem, &type_bundle, kefir_ast_type_unsigned_short())), -100, 100, 8);

    ASSERT_OBJECT_OFFSET(&kft_mem, &env.target_env,
        kefir_ast_type_array(&kft_mem, &type_bundle, kefir_ast_type_unsigned_char(),
            kefir_ast_constant_expression_integer(&kft_mem, 16), NULL), -100, 100, 16);

    struct kefir_ast_struct_type *struct_type1 = NULL;
    const struct kefir_ast_type *type1 = kefir_ast_type_structure(&kft_mem, &type_bundle,
        "", &struct_type1);
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, &symbols, struct_type1,
        "x", kefir_ast_type_unsigned_long(), NULL));
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, &symbols, struct_type1,
        "y", kefir_ast_type_pointer(&kft_mem, &type_bundle, kefir_ast_type_bool()), NULL));
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, &symbols, struct_type1,
        "z", kefir_ast_type_array(&kft_mem, &type_bundle, kefir_ast_type_float(),
