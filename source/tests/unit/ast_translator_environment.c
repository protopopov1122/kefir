...
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, &symbols, struct1_type, "z",
...
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, &symbols, struct2_type, "field1", kefir_ast_type_signed_long(), NULL));
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, &symbols, struct2_type, "field2", kefir_ast_type_signed_int(), NULL));
...
for (kefir_size_t i = 0; i < 16; i++) {
    ASSERT_DESIGNATOR_OFFSET(
        ...
        kefir_ast_new_index_desginator(&kft_mem, i, kefir_ast_new_member_desginator(&kft_mem, &symbols, "z", NULL)), 2,
        2, 20 + i * 2);
}
...
ASSERT_DESIGNATOR_OFFSET(&kft_mem, &env.target_env, opaque_type,
                         kefir_ast_new_member_desginator(&kft_mem, &symbols, "x",
                                                         kefir_ast_new_member_desginator(&kft_mem, &symbols, "field3",
                                                                                         NULL)),
                         1, 1, 52);
...
ASSERT_OBJECT_OFFSET(&kft_mem, &env.target_env, kefir_ast_type_pointer(&kft_mem, &type_bundle, kefir_ast_type_void()),
                     -100, 100, 8);
ASSERT_OBJECT_OFFSET(&kft_mem, &env.target_env, kefir_ast_type_pointer(&kft_mem, &type_bundle, kefir_ast_type_char()),
                     -100, 100, 8);
...
ASSERT_OBJECT_OFFSET(&kft_mem, &env.target_env, kefir_ast_type_pointer(&kft_mem, &type_bundle, kefir_ast_type_double()),
                     -100, 100, 8);
ASSERT_OBJECT_OFFSET(&kft_mem, &env.target_env,
