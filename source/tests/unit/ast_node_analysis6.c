...
#define ASSERT_COMPOUND_LITERAL(_mem, _context, _type, _init, _result_type, _constant) \
    do { \
        struct kefir_ast_compound_literal *compound = kefir_ast_new_compound_literal((_mem), \
            (struct kefir_ast_type_name *) KEFIR_AST_NODE_CLONE((_mem), KEFIR_AST_NODE_BASE(_type))->self);\
...
DEFINE_CASE(ast_node_analysis_compound_literal2, "AST node analysis - compound literals #2")
    ...
    ASSERT_OK(kefir_ast_local_context_declare_external(&kft_mem, &local_context,
        "x", kefir_ast_type_array(&kft_mem, context->type_bundle, kefir_ast_type_char(),
            kefir_ast_constant_expression_integer(&kft_mem, 10), NULL), NULL, NULL));
...
DEFINE_CASE(ast_node_analysis_compound_literal3, "AST node analysis - compound literals #3")
    ...
    ASSERT_OK(kefir_ast_structure_declaration_entry_append(&kft_mem, entry5,
...
DEFINE_CASE(ast_node_analysis_compound_literal4, "AST node analysis - compound literals temporaries")
    ...
    ASSERT_COMPOUND_LITERAL_TEMP(&kft_mem, context, type_name1, {
        ASSERT_OK(kefir_ast_initializer_list_append(&kft_mem, &compound->initializer->list,
            NULL, kefir_ast_new_expression_initializer(&kft_mem,
    ...
    ASSERT_COMPOUND_LITERAL_TEMP(&kft_mem, context, type_name2, {
        ASSERT_OK(kefir_ast_initializer_list_append(&kft_mem, &compound->initializer->list,
            NULL, kefir_ast_new_expression_initializer(&kft_mem,
            KEFIR_AST_NODE_BASE(kefir_ast_new_constant_char(&kft_mem, 'a')))));  
    }, 0, 1);
    ASSERT_COMPOUND_LITERAL_TEMP(&kft_mem, context, type_name3, {
        ASSERT_OK(kefir_ast_initializer_list_append(&kft_mem, &compound->initializer->list,
            NULL, kefir_ast_new_expression_initializer(&kft_mem,
    ...
    struct kefir_ast_type_name *type_name5 = kefir_ast_new_type_name(&kft_mem,
        kefir_ast_declarator_identifier(&kft_mem, NULL, NULL));
    ...
    ASSERT_COMPOUND_LITERAL_TEMP(&kft_mem, context, type_name4, {
        ASSERT_OK(kefir_ast_initializer_list_append(&kft_mem, &compound->initializer->list,
            NULL, kefir_ast_new_expression_initializer(&kft_mem,
            KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 100)))));  
    }, 1, 0);
    ASSERT_COMPOUND_LITERAL_TEMP(&kft_mem, context, type_name5, {
        ASSERT_OK(kefir_ast_initializer_list_append(&kft_mem, &compound->initializer->list,
            NULL, kefir_ast_new_expression_initializer(&kft_mem,
            KEFIR_AST_NODE_BASE(kefir_ast_new_constant_char(&kft_mem, 'a')))));  
    }, 1, 1);
    ASSERT_COMPOUND_LITERAL_TEMP(&kft_mem, context, type_name6, {
        ASSERT_OK(kefir_ast_initializer_list_append(&kft_mem, &compound->initializer->list,
            NULL, kefir_ast_new_expression_initializer(&kft_mem,
            KEFIR_AST_NODE_BASE(kefir_ast_new_constant_float(&kft_mem, 5.1f)))));  
    }, 1, 2);
    ...
    struct kefir_ast_type_name *type_name9 = kefir_ast_new_type_name(&kft_mem,
        kefir_ast_declarator_identifier(&kft_mem, NULL, NULL));
    ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &type_name9->type_decl.specifiers,
        kefir_ast_type_specifier_int(&kft_mem)));

    struct kefir_ast_type_name *type_name10 = kefir_ast_new_type_name(&kft_mem,
        kefir_ast_declarator_identifier(&kft_mem, NULL, NULL));
    ...
    struct kefir_ast_type_name *type_name11 = kefir_ast_new_type_name(&kft_mem,
        kefir_ast_declarator_identifier(&kft_mem, NULL, NULL));
    ...
    ASSERT_COMPOUND_LITERAL_TEMP(&kft_mem, context, (struct kefir_ast_type_name *) KEFIR_AST_NODE_CLONE(&kft_mem, KEFIR_AST_NODE_BASE(type_name8)), {
        ASSERT_OK(kefir_ast_initializer_list_append(&kft_mem, &compound->initializer->list,
            NULL, kefir_ast_new_expression_initializer(&kft_mem,
            KEFIR_AST_NODE_BASE(kefir_ast_new_constant_long(&kft_mem, 10000)))));  
    }, 2, 1);

    ASSERT_COMPOUND_LITERAL_TEMP(&kft_mem, &global_context.context, type_name8, {
        ASSERT_OK(kefir_ast_initializer_list_append(&kft_mem, &compound->initializer->list,
            NULL, kefir_ast_new_expression_initializer(&kft_mem,
            KEFIR_AST_NODE_BASE(kefir_ast_new_constant_long(&kft_mem, 10000)))));  
    }, 0, 0);
    ASSERT_COMPOUND_LITERAL_TEMP(&kft_mem, &global_context.context, type_name9, {
        ASSERT_OK(kefir_ast_initializer_list_append(&kft_mem, &compound->initializer->list,
            NULL, kefir_ast_new_expression_initializer(&kft_mem,
            KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 100)))));  
    }, 0, 1);
    ASSERT_COMPOUND_LITERAL_TEMP(&kft_mem, &global_context.context, type_name10, {
        ASSERT_OK(kefir_ast_initializer_list_append(&kft_mem, &compound->initializer->list,
            NULL, kefir_ast_new_expression_initializer(&kft_mem,
            KEFIR_AST_NODE_BASE(kefir_ast_new_constant_char(&kft_mem, 'a')))));  
    }, 0, 2);
    ASSERT_COMPOUND_LITERAL_TEMP(&kft_mem, &global_context.context,
    ...
    ASSERT_COMPOUND_LITERAL_TEMP(&kft_mem, &global_context.context, type_name11, {
        ASSERT_OK(kefir_ast_initializer_list_append(&kft_mem, &compound->initializer->list,
            NULL, kefir_ast_new_expression_initializer(&kft_mem,
            KEFIR_AST_NODE_BASE(kefir_ast_new_constant_float(&kft_mem, 5.1f)))));  
    }, 1, 0);
