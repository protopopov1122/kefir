...
const struct kefir_ast_type *type1 =
    kefir_ast_type_function(&kft_mem, context->type_bundle, kefir_ast_type_float(), "func1", &function1);
...
ASSERT_OK(kefir_ast_local_context_declare_function(&kft_mem, &local_context, KEFIR_AST_FUNCTION_SPECIFIER_NONE, type1,
                                                   NULL));
ASSERT_OK(kefir_ast_local_context_declare_function(&kft_mem, &local_context, KEFIR_AST_FUNCTION_SPECIFIER_INLINE, type2,
                                                   NULL));

ASSERT_FUNCTION_CALL(&kft_mem, context, "func1", kefir_ast_type_float(), {});
ASSERT_FUNCTION_CALL(&kft_mem, context, "func1", kefir_ast_type_float(), {
    ASSERT_OK(
        kefir_ast_function_call_append(&kft_mem, call1, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 0))));
});
ASSERT_FUNCTION_CALL(&kft_mem, context, "func1", kefir_ast_type_float(), {
    ASSERT_OK(
        kefir_ast_function_call_append(&kft_mem, call1, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 0))));
...
struct kefir_ast_function_type *function1 = NULL;
const struct kefir_ast_type *type1 =
    kefir_ast_type_function(&kft_mem, context->type_bundle, kefir_ast_type_signed_short(), "func1", &function1);
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, function1, NULL,
                                            kefir_ast_type_signed_long(), NULL));
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, function1, NULL, kefir_ast_type_double(),
                                            NULL));
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, function1, NULL, kefir_ast_type_char(),
                                            NULL));

struct kefir_ast_function_type *function2 = NULL;
const struct kefir_ast_type *type2 =
    kefir_ast_type_function(&kft_mem, context->type_bundle, kefir_ast_type_unsigned_short(), "func2", &function2);
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, function2, NULL,
                                            kefir_ast_type_signed_long(), NULL));
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, function2, NULL, kefir_ast_type_double(),
                                            NULL));
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, function2, NULL, kefir_ast_type_char(),
                                            NULL));
ASSERT_OK(kefir_ast_type_function_ellipsis(function2, true));

ASSERT_OK(kefir_ast_local_context_declare_function(&kft_mem, &local_context, KEFIR_AST_FUNCTION_SPECIFIER_NONE, type1,
                                                   NULL));
ASSERT_OK(kefir_ast_local_context_declare_function(&kft_mem, &local_context, KEFIR_AST_FUNCTION_SPECIFIER_NONE, type2,
                                                   NULL));
...
ASSERT_FUNCTION_CALL(&kft_mem, context, "func1", kefir_ast_type_signed_short(), {
    ASSERT_OK(
        kefir_ast_function_call_append(&kft_mem, call1, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_long(&kft_mem, 0))));
    ASSERT_OK(kefir_ast_function_call_append(&kft_mem, call1,
...
ASSERT_FUNCTION_CALL(&kft_mem, context, "func1", kefir_ast_type_signed_short(), {
    ...
    ASSERT_OK(
        kefir_ast_function_call_append(&kft_mem, call1, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 5))));
    ASSERT_OK(kefir_ast_function_call_append(&kft_mem, call1,
...
struct kefir_ast_function_type *function1 = NULL;
const struct kefir_ast_type *type1 =
    kefir_ast_type_function(&kft_mem, context->type_bundle, kefir_ast_type_signed_int(), "func1", &function1);
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, function1, "a", NULL, NULL));
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, function1, "b", NULL, NULL));
...
const struct kefir_ast_type *type1 =
    kefir_ast_type_function(&kft_mem, context->type_bundle, kefir_ast_type_float(), "func1", &function1);
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, function1, NULL, kefir_ast_type_void(),
                                            NULL));
...
ASSERT_FUNCTION_CALL_NOK(&kft_mem, context, "func1", {
    ASSERT_OK(
        kefir_ast_function_call_append(&kft_mem, call1, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 0))));
    ASSERT_OK(
        kefir_ast_function_call_append(&kft_mem, call1, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 1))));
});
...
_Static_assert(sizeof(TYPES2) / sizeof(TYPES2[0]) == sizeof(TYPES) / sizeof(TYPES[0]), "Type array length mismatch");
...
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, structure1, "field1", kefir_ast_type_signed_int(),
                                      NULL));
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, structure1, "field2", kefir_ast_type_char(), NULL));
...
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, structure2, "y",
...
ASSERT_COMPOUND_LITERAL(
    &kft_mem, context, type_name1,
    {
        ...
        ASSERT_OK(kefir_ast_initializer_list_append(
            &kft_mem, &compound->initializer->list, NULL,
            kefir_ast_new_expression_initializer(
                &kft_mem, KEFIR_AST_NODE_BASE(kefir_ast_new_unary_operation(
...
ASSERT_COMPOUND_LITERAL_TEMP(
    ...
    2, 0);
ASSERT_COMPOUND_LITERAL_TEMP(
    &kft_mem, context, (struct kefir_ast_type_name *) KEFIR_AST_NODE_CLONE(&kft_mem, KEFIR_AST_NODE_BASE(type_name8)),
    {
        ASSERT_OK(kefir_ast_initializer_list_append(
            &kft_mem, &compound->initializer->list, NULL,
            kefir_ast_new_expression_initializer(&kft_mem,
