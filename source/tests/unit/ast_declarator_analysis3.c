...
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type1, "b", kefir_ast_type_signed_long_long(),
                                      NULL));
...
const struct kefir_ast_type *type1 = kefir_ast_type_array(
    ...
    kefir_ast_type_qualified(&kft_mem, context->type_bundle, kefir_ast_type_signed_int(),
                             (struct kefir_ast_type_qualification){.constant = true}),
    kefir_ast_constant_expression_integer(&kft_mem, 10), &(struct kefir_ast_type_qualification){.restricted = true});
...
REQUIRE_OK(kefir_ast_local_context_declare_external(&kft_mem, &local_context, "variable", kefir_ast_type_signed_int(),
                                                    NULL, NULL));
