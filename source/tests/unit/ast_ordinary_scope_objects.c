...
DEFINE_CASE(ast_ordinary_scope_objects7, "AST Declaration scoping - local extern")
    ...
    ASSERT_NOK(kefir_ast_local_context_define_static_thread_local(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int(), NULL));
    ...
    do {
        ...
        do {
            ASSERT_OK(kefir_ast_local_context_push_block_scope(&kft_mem, &context));
            ASSERT_OK(kefir_ast_local_context_define_static(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int(), NULL));
            do {
                ASSERT_OK(kefir_ast_local_context_push_block_scope(&kft_mem, &context));
                ASSERT_OK(kefir_ast_local_context_declare_external(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int(), NULL));
                ASSERT_NOK(kefir_ast_local_context_declare_external_thread_local(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int(), NULL));
...
DEFINE_CASE(ast_ordinary_scope_objects8, "AST Declaration scoping - local extern thread_local")
    ...
    ASSERT_NOK(kefir_ast_local_context_declare_external_thread_local(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_long(), NULL));
    ASSERT_OK(kefir_ast_local_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_local_context_define_static_thread_local(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int(), NULL));
    ASSERT_OK(kefir_ast_local_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_local_context_declare_external_thread_local(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int(), NULL));
    ASSERT_NOK(kefir_ast_local_context_declare_external(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_int(), NULL));
    ASSERT_NOK(kefir_ast_local_context_declare_external_thread_local(&kft_mem, &context, "ext_int1", kefir_ast_type_signed_long(), NULL));
...
DEFINE_CASE(ast_ordinary_scope_objects9, "AST Declaration scoping - local static")
    ...
    ASSERT_OK(kefir_ast_local_context_define_static(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int(), NULL));
    ASSERT_NOK(kefir_ast_local_context_define_static(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int(), NULL));
    ASSERT_NOK(kefir_ast_local_context_declare_external(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int(), NULL));
    ASSERT_OK(kefir_ast_local_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_local_context_define_static(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int(), NULL));
    ASSERT_NOK(kefir_ast_local_context_define_static(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int(), NULL));
    ASSERT_OK(kefir_ast_local_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_local_context_define_static_thread_local(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int(), NULL));
    ASSERT_NOK(kefir_ast_local_context_define_static_thread_local(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int(), NULL));
    ASSERT_OK(kefir_ast_local_context_pop_block_scope(&context));
    ASSERT_NOK(kefir_ast_local_context_define_static(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int(), NULL));
    ASSERT_OK(kefir_ast_local_context_pop_block_scope(&context));
    ASSERT_NOK(kefir_ast_local_context_define_static(&kft_mem, &context, "static_int1", kefir_ast_type_signed_int(), NULL));
...
DEFINE_CASE(ast_ordinary_scope_objects11, "AST Declaration scoping - local auto")
    ...
    ASSERT_NOK(kefir_ast_local_context_define_static_thread_local(&kft_mem, &context, "auto_int1", kefir_ast_type_signed_int(), NULL));
    ASSERT_OK(kefir_ast_local_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_local_context_define_auto(&kft_mem, &context, "auto_int1", kefir_ast_type_signed_int(), NULL));
    ...
    ASSERT_OK(kefir_ast_local_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_local_context_define_auto(&kft_mem, &context, "auto_int2", kefir_ast_type_signed_int(), NULL));
    ASSERT_NOK(kefir_ast_local_context_define_register(&kft_mem, &context, "auto_int2", kefir_ast_type_signed_int(), NULL));
    ASSERT_OK(kefir_ast_local_context_define_register(&kft_mem, &context, "auto_int3", kefir_ast_type_signed_int(), NULL));
    ASSERT_OK(kefir_ast_local_context_push_block_scope(&kft_mem, &context));
...
DEFINE_CASE(ast_ordinary_scope_objects12, "AST Declaration scoping - local register")
    ...
    ASSERT_OK(kefir_ast_local_context_define_register(&kft_mem, &context, "auto_int1", kefir_ast_type_signed_int(), NULL));
    ASSERT_NOK(kefir_ast_local_context_define_register(&kft_mem, &context, "auto_int1", kefir_ast_type_signed_int(), NULL));
    ASSERT_NOK(kefir_ast_local_context_define_static(&kft_mem, &context, "auto_int1", kefir_ast_type_signed_int(), NULL));
    ASSERT_NOK(kefir_ast_local_context_define_static_thread_local(&kft_mem, &context, "auto_int1", kefir_ast_type_signed_int(), NULL));
    ASSERT_OK(kefir_ast_local_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_local_context_define_register(&kft_mem, &context, "auto_int1", kefir_ast_type_signed_int(), NULL));
    ASSERT_OK(kefir_ast_local_context_define_register(&kft_mem, &context, "auto_int2", kefir_ast_type_signed_int(), NULL));
    ASSERT_NOK(kefir_ast_local_context_define_register(&kft_mem, &context, "auto_int2", kefir_ast_type_signed_int(), NULL));
    ASSERT_OK(kefir_ast_local_context_push_block_scope(&kft_mem, &context));
    ASSERT_OK(kefir_ast_local_context_define_register(&kft_mem, &context, "auto_int2", kefir_ast_type_signed_int(), NULL));
    ASSERT_NOK(kefir_ast_local_context_define_auto(&kft_mem, &context, "auto_int2", kefir_ast_type_signed_int(), NULL));
...
DEFINE_CASE(ast_ordinary_scope_objects14, "AST Declaration scoping - block externals")
    ...
    ASSERT_OK(kefir_ast_global_context_define_external(&kft_mem, &global_context, "array1", array_type3, NULL));
    ASSERT_OK(kefir_ast_local_context_resolve_scoped_ordinary_identifier(&context, "array1", &scoped_id));
    ASSERT(scoped_id->object.storage == KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_EXTERN);
    ASSERT(scoped_id->object.linkage == KEFIR_AST_SCOPED_IDENTIFIER_EXTERNAL_LINKAGE);
    ASSERT(!scoped_id->object.external);
...
DEFINE_CASE(ast_ordinary_scope_objects15, "AST Declaration scoping - block externals #2")
    ...
    ASSERT_OK(kefir_ast_global_context_define_static_thread_local(&kft_mem, &global_context, "array1",
        array_type1, kefir_ast_alignment_const_expression(&kft_mem, kefir_ast_constant_expression_integer(&kft_mem, 8))));
