...
ASSIGN_DECL_CAST(struct kefir_ast_struct_member *, member2, ((struct kefir_ast_node_base *) member->structure)->self);
...
kefir_ast_unary_operation_type_t UNARY_OPERATORS[] = {KEFIR_AST_OPERATION_PLUS, KEFIR_AST_OPERATION_NEGATE,
                                                      KEFIR_AST_OPERATION_INVERT, KEFIR_AST_OPERATION_LOGICAL_NEGATE};
...
for (kefir_size_t i = 0; i < BINARY_OPERATOR_LENGTH; i++) {
    struct kefir_ast_binary_operation *binary_op = kefir_ast_new_binary_operation(
        &kft_mem, BINARY_OPERATORS[i], KEFIR_AST_NODE_BASE(kefir_ast_new_identifier(&kft_mem, &symbols, "helloWorld")),
