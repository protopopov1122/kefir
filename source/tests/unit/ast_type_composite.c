...
const struct kefir_ast_type *composite12 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, enum1, enum2);
...
const struct kefir_ast_type *composite21 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, enum2, enum1);
...
const struct kefir_ast_type *composite13 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, enum1, enum3);
ASSERT(composite13 == NULL);
const struct kefir_ast_type *composite31 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, enum3, enum1);
ASSERT(composite31 == NULL);
const struct kefir_ast_type *composite23 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, enum2, enum3);
ASSERT(composite23 == NULL);
const struct kefir_ast_type *composite32 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, enum3, enum2);
ASSERT(composite32 == NULL);

const struct kefir_ast_type *composite1u = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, enum1,
...
const struct kefir_ast_type *composite2u = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, enum2,
                                                                    kefir_ast_enumeration_underlying_type(enum2_type));
...
const struct kefir_ast_type *composite3u = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, enum3,
                                                                    kefir_ast_enumeration_underlying_type(enum3_type));
...
ASSERT(type1 != NULL);
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, &symbols, union_type1, "field1", kefir_ast_type_signed_int(), NULL));
...
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, &symbols, union_type2, "field2",
                                      kefir_ast_type_pointer(&kft_mem, &type_bundle, kefir_ast_type_float()), NULL));
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, &symbols, union_type2, "field1", kefir_ast_type_signed_int(), NULL));
...
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, &symbols, union_type3, "field2",
                                      kefir_ast_type_pointer(&kft_mem, &type_bundle, kefir_ast_type_float()), NULL));
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, &symbols, union_type3, "field1", kefir_ast_type_signed_int(), NULL));
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, &symbols, union_type3, "field0", kefir_ast_type_unsigned_char(), NULL));

const struct kefir_ast_type *composite12 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, type1, type2);
ASSERT(composite12 != NULL);
ASSERT(KEFIR_AST_TYPE_COMPATIBLE(type_traits, type1, composite12));
ASSERT(KEFIR_AST_TYPE_COMPATIBLE(type_traits, type2, composite12));
ASSERT(!KEFIR_AST_TYPE_COMPATIBLE(type_traits, type3, composite12));
...
const struct kefir_ast_type *composite21 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, type2, type1);
ASSERT(composite21 != NULL);
ASSERT(KEFIR_AST_TYPE_COMPATIBLE(type_traits, type1, composite21));
ASSERT(KEFIR_AST_TYPE_COMPATIBLE(type_traits, type2, composite21));
ASSERT(!KEFIR_AST_TYPE_COMPATIBLE(type_traits, type3, composite21));
...
const struct kefir_ast_type *composite13 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, type1, type3);
ASSERT(composite13 == NULL);
const struct kefir_ast_type *composite23 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, type2, type3);
ASSERT(composite23 == NULL);
const struct kefir_ast_type *composite31 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, type3, type1);
ASSERT(composite31 == NULL);
const struct kefir_ast_type *composite32 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, type3, type2);
ASSERT(composite32 == NULL);
...
const struct kefir_ast_type *composite33 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, type3, type3);
...
const struct kefir_ast_type *composite34 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, type3, type4);
...
const struct kefir_ast_type *composite35 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, type3, type5);
...
const struct kefir_ast_type *composite15 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, type1, type5);
...
const struct kefir_ast_type *composite12 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, type1, type2);
ASSERT(composite12 == NULL);
const struct kefir_ast_type *composite25 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, type2, type5);
...
static const struct kefir_ast_type *build_function_type(struct kefir_mem *mem,
    ...
    ASSERT_OK(
        kefir_ast_type_function_parameter(mem, type_bundle, *function_type, "param1", kefir_ast_type_double(), NULL));
    ASSERT_OK(kefir_ast_type_function_parameter(
        mem, type_bundle, *function_type, "param2",
        kefir_ast_type_array_static(mem, type_bundle, kefir_ast_type_char(),
                                    kefir_ast_constant_expression_integer(&kft_mem, 140), NULL),
        NULL));
    ASSERT_OK(kefir_ast_type_function_parameter(
        mem, type_bundle, *function_type, "param3",
...
static const struct kefir_ast_type *build_function_type2(struct kefir_mem *mem,
    ...
    ASSERT_OK(kefir_ast_type_function_parameter(mem, type_bundle, *function_type, "param2", NULL, NULL));
    ASSERT_OK(kefir_ast_type_function_parameter(mem, type_bundle, *function_type, "param3", NULL, NULL));
...
const struct kefir_ast_type *composite51 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, type5, type1);
...
const struct kefir_ast_type *composite13 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, type1, type3);
ASSERT(composite13 == NULL);
const struct kefir_ast_type *composite14 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, type1, type4);
ASSERT(composite14 == NULL);
const struct kefir_ast_type *composite23 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, type2, type3);
ASSERT(composite23 == NULL);
const struct kefir_ast_type *composite24 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, type2, type4);
ASSERT(composite24 == NULL);
const struct kefir_ast_type *composite53 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, type5, type3);
ASSERT(composite53 == NULL);
const struct kefir_ast_type *composite54 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits, type5, type4);
