...
DEFINE_CASE(ast_type_enum_composite, "AST types - enum composite type")
    ...
    const struct kefir_ast_type *composite12 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
        enum1, enum2);
    ...
    const struct kefir_ast_type *composite21 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
        enum2, enum1);
    ...
    const struct kefir_ast_type *composite13 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
        enum1, enum3);
    ASSERT(composite13 == NULL);
    const struct kefir_ast_type *composite31 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
        enum3, enum1);
    ASSERT(composite31 == NULL);
    const struct kefir_ast_type *composite23 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
        enum2, enum3);
    ASSERT(composite23 == NULL);
    const struct kefir_ast_type *composite32 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
        enum3, enum2);
    ASSERT(composite32 == NULL);

    const struct kefir_ast_type *composite1u = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
    ...
    const struct kefir_ast_type *composite2u = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
    ...
    const struct kefir_ast_type *composite3u = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
...
DEFINE_CASE(ast_type_qualified_composite, "AST types - qualified composite type")
    ...
    for (kefir_size_t i = 0; i < BASE_TYPES_LENGTH; i++) {
        for (kefir_size_t j = 0; j < QUALIFICATIONS_LENGTH; j++) {
            const struct kefir_ast_type *qualified1 = kefir_ast_type_qualified(&kft_mem, &type_bundle,
                BASE_TYPES[i], QUALIFICATIONS[j]);
            if (KEFIR_AST_TYPE_IS_ZERO_QUALIFICATION(&QUALIFICATIONS[j])) {
                const struct kefir_ast_type *composite = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
            ...
            for (kefir_size_t k = 0; k < QUALIFICATIONS_LENGTH; k++) {
                const struct kefir_ast_type *qualified2 = kefir_ast_type_qualified(&kft_mem, &type_bundle,
...
DEFINE_CASE(ast_type_struct_composite, "AST types - structure composite type")
    ...
    const struct kefir_ast_type *composite12 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, context->type_bundle, type_traits,
        type1, type2);
    ASSERT(composite12 != NULL);
    ...
    const struct kefir_ast_type *composite21 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, context->type_bundle, type_traits,
        type2, type1);
    ASSERT(composite21 != NULL);
    ...
    const struct kefir_ast_type *composite13 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, context->type_bundle, type_traits,
        type1, type3);
    ASSERT(composite13 == NULL);
    const struct kefir_ast_type *composite31 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, context->type_bundle, type_traits,
        type3, type1);
    ASSERT(composite31 == NULL);
    const struct kefir_ast_type *composite23 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, context->type_bundle, type_traits,
        type2, type3);
    ASSERT(composite23 == NULL);
    const struct kefir_ast_type *composite32 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, context->type_bundle, type_traits,
        type3, type2);
    ASSERT(composite32 == NULL);
    const struct kefir_ast_type *composite14 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, context->type_bundle, type_traits,
        type1, type4);
    ASSERT(composite14 == NULL);
    const struct kefir_ast_type *composite41 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, context->type_bundle, type_traits,
        type4, type1);
    ASSERT(composite41 == NULL);
    const struct kefir_ast_type *composite24 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, context->type_bundle, type_traits,
        type2, type4);
    ASSERT(composite24 == NULL);
    const struct kefir_ast_type *composite42 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, context->type_bundle, type_traits,
        type4, type2);
...
DEFINE_CASE(ast_type_array_composite, "AST types - array composite type")
    ...
    const struct kefir_ast_type *composite33 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
        type3, type3);
    ...
    const struct kefir_ast_type *composite34 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
        type3, type4);
    ...
    const struct kefir_ast_type *composite35 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
        type3, type5);
    ...
    const struct kefir_ast_type *composite15 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
        type1, type5);
    ...
    const struct kefir_ast_type *composite12 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
        type1, type2);
    ASSERT(composite12 == NULL);
    const struct kefir_ast_type *composite25 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
        type2, type5);
...
static const struct kefir_ast_type *build_function_type(struct kefir_mem *mem,
    ...
    return kefir_ast_type_qualified(mem, type_bundle, kefir_ast_type_pointer(mem, type_bundle, type),
        (const struct kefir_ast_type_qualification){
            .constant = true,
            .restricted = false,
            .volatile_type = false
...
DEFINE_CASE(ast_type_function_composite, "AST types - function composite type")
    ...
    const struct kefir_ast_type *composite51 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
        type5, type1);
    ...
    const struct kefir_ast_type *composite13 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
        type1, type3);
    ASSERT(composite13 == NULL);
    const struct kefir_ast_type *composite14 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
        type1, type4);
    ASSERT(composite14 == NULL);
    const struct kefir_ast_type *composite23 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
        type2, type3);
    ASSERT(composite23 == NULL);
    const struct kefir_ast_type *composite24 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
        type2, type4);
    ASSERT(composite24 == NULL);
    const struct kefir_ast_type *composite53 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
        type5, type3);
    ASSERT(composite53 == NULL);
    const struct kefir_ast_type *composite54 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_bundle, type_traits,
        type5, type4);
