        ...
        for (kefir_size_t j = 0; j < BASIC_TYPES_LENGTH; j++) {
            if (i == j) {
                ...
                const struct kefir_ast_type *composite =
                    KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits, BASIC_TYPES[i], BASIC_TYPES[j]);
    ...
    for (kefir_size_t i = 0; i < BASE_TYPES_LENGTH; i++) {
        for (kefir_size_t j = 0; j < BASE_TYPES_LENGTH; j++) {
            if (i == j) {
                ...
                const struct kefir_ast_type *composite =
                    KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits, BASE_TYPES[i], BASE_TYPES[j]);
                ASSERT(composite != NULL);
...
DEFINE_CASE(ast_type_enum_composite, "AST types - enum composite type")
    ...
    const struct kefir_ast_type *composite12 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        enum1, enum2);
    ...
    const struct kefir_ast_type *composite21 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        enum2, enum1);
    ...
    const struct kefir_ast_type *composite13 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        enum1, enum3);
    ASSERT(composite13 == NULL);
    const struct kefir_ast_type *composite31 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        enum3, enum1);
    ASSERT(composite31 == NULL);
    const struct kefir_ast_type *composite23 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        enum2, enum3);
    ASSERT(composite23 == NULL);
    const struct kefir_ast_type *composite32 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        enum3, enum2);
    ASSERT(composite32 == NULL);

    const struct kefir_ast_type *composite1u = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        enum1, kefir_ast_enumeration_underlying_type(enum1_type));
    ...
    const struct kefir_ast_type *composite2u = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        enum2, kefir_ast_enumeration_underlying_type(enum2_type));
    ...
    const struct kefir_ast_type *composite3u = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        enum3, kefir_ast_enumeration_underlying_type(enum3_type));
...
DEFINE_CASE(ast_type_qualified_composite, "AST types - qualified composite type")
    ...
    for (kefir_size_t i = 0; i < BASE_TYPES_LENGTH; i++) {
        for (kefir_size_t j = 0; j < QUALIFICATIONS_LENGTH; j++) {
            const struct kefir_ast_type *qualified1 = kefir_ast_type_qualified(&kft_mem, &type_storage,
                BASE_TYPES[i], QUALIFICATIONS[j]);
            if (KEFIR_AST_TYPE_IS_ZERO_QUALIFICATION(&QUALIFICATIONS[j])) {
                const struct kefir_ast_type *composite = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
                    qualified1, BASE_TYPES[i]);
            ...
            for (kefir_size_t k = 0; k < QUALIFICATIONS_LENGTH; k++) {
                const struct kefir_ast_type *qualified2 = kefir_ast_type_qualified(&kft_mem, &type_storage,
                    BASE_TYPES[i], QUALIFICATIONS[k]);
                const struct kefir_ast_type *composite = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
                    qualified1, qualified2);
                if (j == k) {
...
static const struct kefir_ast_type *build_struct_type(struct kefir_mem *mem,
    ...
    const struct kefir_ast_type *type = kefir_ast_type_structure(mem, type_storage, id, struct_type);
    ASSERT(type != NULL);
    ASSERT_OK(kefir_ast_struct_type_field(mem, type_storage->symbols, *struct_type, "field1",
        kefir_ast_type_pointer(mem, type_storage, kefir_ast_type_void()), NULL));
    ASSERT_OK(kefir_ast_struct_type_field(mem, type_storage->symbols, *struct_type, "field2",
        kefir_ast_type_double(), NULL));
    ASSERT_OK(kefir_ast_struct_type_bitfield(mem, type_storage->symbols, *struct_type, "field3",
        kefir_ast_type_signed_long_long(), NULL, kefir_ast_constant_expression_integer(mem, 10)));
    ASSERT_OK(kefir_ast_struct_type_bitfield(mem, type_storage->symbols, *struct_type, "field4",
        kefir_ast_type_signed_long_long(), NULL, kefir_ast_constant_expression_integer(mem, 2)));
    ASSERT_OK(kefir_ast_struct_type_field(mem, type_storage->symbols, *struct_type, "field5",
...
DEFINE_CASE(ast_type_struct_composite, "AST types - structure composite type")
    ...
    const struct kefir_ast_type *type1 = build_struct_type(&kft_mem, &type_storage, "struct1", &struct_type1);
    struct kefir_ast_struct_type *struct_type2 = NULL;
    const struct kefir_ast_type *type2 = build_struct_type(&kft_mem, &type_storage, "struct1", &struct_type2);
    struct kefir_ast_struct_type *struct_type3 = NULL;
    const struct kefir_ast_type *type3 = build_struct_type(&kft_mem, &type_storage, "struct2", &struct_type3);
    struct kefir_ast_struct_type *struct_type4 = NULL;
    const struct kefir_ast_type *type4 = build_struct_type(&kft_mem, &type_storage, "struct1", &struct_type4);
    ...
    const struct kefir_ast_type *composite32 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        type3, type2);
    ASSERT(composite32 == NULL);
    const struct kefir_ast_type *composite14 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        type1, type4);
    ASSERT(composite14 == NULL);
    const struct kefir_ast_type *composite41 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        type4, type1);
    ASSERT(composite41 == NULL);
    const struct kefir_ast_type *composite24 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        type2, type4);
    ASSERT(composite24 == NULL);
    const struct kefir_ast_type *composite42 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        type4, type2);
...
DEFINE_CASE(ast_type_union_composite, "AST types - union composite type")
    ...
    const struct kefir_ast_type *type1 = kefir_ast_type_union(&kft_mem, &type_storage, "union1", &union_type1);
    ASSERT(type1 != NULL);
    ...
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, &symbols, union_type1, "field2",
        kefir_ast_type_pointer(&kft_mem, &type_storage, kefir_ast_type_float()), NULL));
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, &symbols, union_type1, "field3",
        kefir_ast_type_array(&kft_mem, &type_storage, kefir_ast_type_unsigned_char(), 128, NULL), NULL));
    
    struct kefir_ast_struct_type *union_type2 = NULL;
    const struct kefir_ast_type *type2 = kefir_ast_type_union(&kft_mem, &type_storage, "union1", &union_type2);
    ASSERT(type2 != NULL);
    ...
    const struct kefir_ast_type *type3 = kefir_ast_type_union(&kft_mem, &type_storage, "union1", &union_type3);
    ASSERT(type3 != NULL);
...
DEFINE_CASE(ast_type_array_composite, "AST types - array composite type")
    ...
    const struct kefir_ast_type *type1 = kefir_ast_type_array(&kft_mem, &type_storage, kefir_ast_type_unsigned_char(), 100, NULL);
    const struct kefir_ast_type *type2 = kefir_ast_type_array(&kft_mem, &type_storage, kefir_ast_type_unsigned_char(), 101, NULL);
    const struct kefir_ast_type *type3 = kefir_ast_type_unbounded_array(&kft_mem, &type_storage,
        kefir_ast_type_unsigned_char(), NULL);
    const struct kefir_ast_type *type4 = kefir_ast_type_vlen_array(&kft_mem, &type_storage,
        kefir_ast_type_unsigned_char(), NULL, NULL);
    const struct kefir_ast_type *type5 = kefir_ast_type_array_static(&kft_mem, &type_storage,
    ...
    const struct kefir_ast_type *composite33 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        type3, type3);
    ...
    const struct kefir_ast_type *composite34 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        type3, type4);
    ...
    const struct kefir_ast_type *composite35 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        type3, type5);
    ...
    const struct kefir_ast_type *composite15 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        type1, type5);
    ...
    const struct kefir_ast_type *composite12 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        type1, type2);
    ASSERT(composite12 == NULL);
    const struct kefir_ast_type *composite25 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        type2, type5);
...
static const struct kefir_ast_type *build_function_type(struct kefir_mem *mem,
    ...
    const struct kefir_ast_type *type = kefir_ast_type_function(mem, type_storage, return_type,
        id, function_type);
    ASSERT(type != NULL);
    ASSERT(*function_type != NULL);
    ASSERT_OK(kefir_ast_type_function_parameter(mem, type_storage, *function_type, "param1",
        kefir_ast_type_double(), NULL));
    ASSERT_OK(kefir_ast_type_function_parameter(mem, type_storage, *function_type, "param2",
        kefir_ast_type_array_static(mem, type_storage, kefir_ast_type_char(), 140, NULL), NULL));
    ASSERT_OK(kefir_ast_type_function_parameter(mem, type_storage, *function_type, "param3",
        kefir_ast_type_pointer(mem, type_storage,
            kefir_ast_type_qualified(mem, type_storage, kefir_ast_type_unsigned_char(), (struct kefir_ast_type_qualification){
                .constant = true,
                .restricted = false,
                .volatile_type = false
            })), NULL));

    return kefir_ast_type_qualified(mem, type_storage, kefir_ast_type_pointer(mem, type_storage, type),
...
DEFINE_CASE(ast_type_function_composite, "AST types - function composite type")
    ...
    const struct kefir_ast_type *type1 = build_function_type(&kft_mem, &type_storage, kefir_ast_type_void(),
        "", &function_type1);
    struct kefir_ast_function_type *function_type2 = NULL;
    const struct kefir_ast_type *type2 = build_function_type(&kft_mem, &type_storage, kefir_ast_type_void(),
        "", &function_type2);
    struct kefir_ast_function_type *function_type3 = NULL;
    const struct kefir_ast_type *type3 = build_function_type(&kft_mem, &type_storage,
        kefir_ast_type_pointer(&kft_mem, &type_storage, kefir_ast_type_signed_int()),
        "", &function_type3);
    struct kefir_ast_function_type *function_type4 = NULL;
    const struct kefir_ast_type *type4 = build_function_type(&kft_mem, &type_storage, kefir_ast_type_void(),
        "", &function_type4);
    ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, &type_storage, function_type4, "param10",
        kefir_ast_type_signed_int(), NULL));
    struct kefir_ast_function_type *function_type5 = NULL;
    const struct kefir_ast_type *type5 = build_function_type2(&kft_mem, &type_storage, kefir_ast_type_void(),
        "", &function_type5);
    ...
    const struct kefir_ast_type *composite51 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        type5, type1);
    ...
    const struct kefir_ast_type *composite24 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        type2, type4);
    ASSERT(composite24 == NULL);
    const struct kefir_ast_type *composite53 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        type5, type3);
    ASSERT(composite53 == NULL);
    const struct kefir_ast_type *composite54 = KEFIR_AST_TYPE_COMPOSITE(&kft_mem, &type_storage, type_traits,
        type5, type4);
