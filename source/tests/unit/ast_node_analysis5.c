        ...
        for (kefir_size_t j = 0; j < TYPES_LEN; j++) {
            ASSERT_COMPOUND_ASSIGNMENT(
                ...
                KEFIR_AST_NODE_BASE(kefir_ast_new_unary_operation(
                    ...
                    KEFIR_AST_NODE_BASE(kefir_ast_new_cast_operator(
                        &kft_mem,
                        (struct kefir_ast_type_name *) KEFIR_AST_NODE_CLONE(&kft_mem, KEFIR_AST_NODE_BASE(TYPES2[i]))
                            ->self,
                        KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 0)))))),
                KEFIR_AST_NODE_BASE(make_constant(&kft_mem, TYPES[j])), TYPES[i]);
        }
...
ASSERT_OK(kefir_ast_local_context_declare_external(&kft_mem, &local_context, "x", kefir_ast_type_float(), NULL, NULL));
...
ASSERT(comma != NULL);
ASSERT_OK(kefir_ast_comma_append(&kft_mem, comma, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_bool(&kft_mem, true))));
