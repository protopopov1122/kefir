...
ASSERT_OK(kefir_ast_type_function_parameter(
    &kft_mem, &type_bundle, function_type, "param5",
    kefir_ast_type_array(
        &kft_mem, &type_bundle, kefir_ast_type_signed_int(), kefir_ast_constant_expression_integer(&kft_mem, 12),
        &(const struct kefir_ast_type_qualification){.constant = false, .restricted = false, .volatile_type = false}),
    NULL));
ASSERT_OK(kefir_ast_type_function_parameter(
    &kft_mem, &type_bundle, function_type, "param6",
    kefir_ast_type_array(
        &kft_mem, &type_bundle, kefir_ast_type_signed_int(), kefir_ast_constant_expression_integer(&kft_mem, 13),
...
ASSERT_FUNCTION_PARAM_ADJ_TYPE(function_type, 1,
                               kefir_ast_type_pointer(&kft_mem, &type_bundle, kefir_ast_type_signed_int()));
ASSERT_FUNCTION_PARAM_ADJ_TYPE(function_type, 2,
                               kefir_ast_type_pointer(&kft_mem, &type_bundle, kefir_ast_type_signed_int()));
ASSERT_FUNCTION_PARAM_ADJ_TYPE(function_type, 3,
                               kefir_ast_type_pointer(&kft_mem, &type_bundle, kefir_ast_type_signed_int()));
ASSERT_FUNCTION_PARAM_ADJ_TYPE(function_type, 4,
                               kefir_ast_type_pointer(&kft_mem, &type_bundle, kefir_ast_type_signed_int()));
ASSERT_FUNCTION_PARAM_ADJ_TYPE(
    function_type, 5,
...
const struct kefir_ast_type *type1 =
    kefir_ast_type_enumeration(&kft_mem, &type_bundle, "enum1", type_traits->underlying_enumeration_type, &enum1_type);
ASSERT(type1 != NULL);
const struct kefir_ast_type *BASIC_TYPES[] = {kefir_ast_type_void(),
                                              kefir_ast_type_bool(),
                                              kefir_ast_type_char(),
...
for (kefir_size_t i = 0; i < BASE_TYPES_LENGTH; i++) {
    ...
    const struct kefir_ast_type *zero_qualified = kefir_ast_type_qualified(
    ...
    const struct kefir_ast_type *qualified100 = kefir_ast_type_qualified(
        &kft_mem, &type_bundle, BASE_TYPES[i],
    ...
    const struct kefir_ast_type *qualified101 = kefir_ast_type_qualified(
        &kft_mem, &type_bundle, BASE_TYPES[i],
