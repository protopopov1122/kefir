...
DEFINE_CASE(ast_declarator_analysis9, "AST declarator analysis - enum declarators")
    ...
    const struct kefir_ast_type *type1 = kefir_ast_type_incomplete_enumeration(&kft_mem, context->type_bundle,
        "enum1", context->type_traits->underlying_enumeration_type);

    ASSERT_IDENTIFIER_TYPE(&kft_mem, context,
        type1, KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_EXTERN, 
        KEFIR_AST_FUNCTION_SPECIFIER_NONE, 0, 2,
    ...
    ASSERT_OK(kefir_ast_enum_specifier_append(&kft_mem, specifier2, context->symbols,
        "A", NULL));
    ASSERT_OK(kefir_ast_enum_specifier_append(&kft_mem, specifier2, context->symbols,
        "B", KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 1))));
    ASSERT_OK(kefir_ast_enum_specifier_append(&kft_mem, specifier2, context->symbols,
        "C", KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 2))));
    ASSERT_OK(kefir_ast_enum_specifier_append(&kft_mem, specifier2, context->symbols,
        "D", NULL));
    ASSERT_OK(kefir_ast_enum_specifier_append(&kft_mem, specifier2, context->symbols,
        "CONST1", KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 200))));

    struct kefir_ast_enum_type *enum_type2 = NULL;
    const struct kefir_ast_type *type2 = kefir_ast_type_enumeration(&kft_mem, context->type_bundle, "enum2",
        context->type_traits->underlying_enumeration_type, &enum_type2);
    ...
    struct kefir_ast_enum_specifier *specifier3 = kefir_ast_enum_specifier_init(&kft_mem, context->symbols,
        NULL, true);
    ASSERT_OK(kefir_ast_enum_specifier_append(&kft_mem, specifier3, context->symbols,
        "ONE", KEFIR_AST_NODE_BASE(kefir_ast_new_unary_operation(&kft_mem, KEFIR_AST_OPERATION_NEGATE,
            KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, -1))))));
    ASSERT_OK(kefir_ast_enum_specifier_append(&kft_mem, specifier3, context->symbols,
        "TWO", NULL));
    ASSERT_OK(kefir_ast_enum_specifier_append(&kft_mem, specifier3, context->symbols,
        "THREE", NULL));
    ASSERT_OK(kefir_ast_enum_specifier_append(&kft_mem, specifier3, context->symbols,
        "TEN", KEFIR_AST_NODE_BASE(kefir_ast_new_constant_long_long(&kft_mem, 10))));

    struct kefir_ast_enum_type *enum_type3 = NULL;
    const struct kefir_ast_type *type3 = kefir_ast_type_enumeration(&kft_mem, context->type_bundle, NULL,
        context->type_traits->underlying_enumeration_type, &enum_type3);
    ...
    ASSERT_IDENTIFIER_TYPE(&kft_mem, context,
        ...
        KEFIR_AST_FUNCTION_SPECIFIER_NONE, 4, 3,
        kefir_ast_type_qualifier_const(&kft_mem),
        kefir_ast_type_specifier_enum(&kft_mem, specifier3),
