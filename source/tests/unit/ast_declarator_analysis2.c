...
ASSERT_OK(kefir_ast_structure_declaration_entry_append(&kft_mem, entry2,
                                                       kefir_ast_declarator_identifier(&kft_mem, context->symbols, "a"),
                                                       NULL));
...
ASSERT_OK(kefir_ast_structure_declaration_entry_append(&kft_mem, entry3,
                                                       kefir_ast_declarator_identifier(&kft_mem, context->symbols, "b"),
                                                       NULL));
ASSERT_OK(kefir_ast_structure_declaration_entry_append(&kft_mem, entry3,
...
ASSERT_OK(kefir_ast_structure_declaration_entry_append(&kft_mem, entry4,
                                                       kefir_ast_declarator_identifier(&kft_mem, context->symbols, "w"),
                                                       NULL));
...
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type3, "b", kefir_ast_type_signed_long_long(),
                                      NULL));
...
ASSERT_IDENTIFIER_TYPE(&kft_mem, context,
                       ...
                       KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_EXTERN_THREAD_LOCAL, KEFIR_AST_FUNCTION_SPECIFIER_NONE, 0, 4,
                       kefir_ast_type_qualifier_volatile(&kft_mem), kefir_ast_storage_class_specifier_extern(&kft_mem),
...
ASSERT_IDENTIFIER_TYPE(&kft_mem, context, type1, KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_STATIC,
                       KEFIR_AST_FUNCTION_SPECIFIER_NONE, 32, 3, kefir_ast_type_specifier_union(&kft_mem, specifier1),
                       kefir_ast_storage_class_specifier_static(&kft_mem),
                       kefir_ast_alignment_specifier(&kft_mem,
                                                     KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 32))));
...
struct kefir_ast_enum_specifier *specifier1 = kefir_ast_enum_specifier_init(&kft_mem, context->symbols, "enum1", false);
ASSERT(specifier1 != NULL);
...
ASSERT_IDENTIFIER_TYPE(&kft_mem, context,
                       ...
                       kefir_ast_storage_class_specifier_thread_local(&kft_mem),
                       kefir_ast_type_specifier_enum(&kft_mem, specifier2), kefir_ast_type_qualifier_volatile(&kft_mem),
                       kefir_ast_storage_class_specifier_static(&kft_mem));
...
ASSERT_OK(kefir_ast_enum_specifier_append(
    ...
    KEFIR_AST_NODE_BASE(kefir_ast_new_unary_operation(&kft_mem, KEFIR_AST_OPERATION_NEGATE,
                                                      KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, -1))))));
