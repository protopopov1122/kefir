...
DEFINE_CASE(ast_declarator_analysis19, "AST declarator analysis - typedefs #1")
    ...
    ASSERT_OK(kefir_ast_global_context_define_type(&kft_mem, &global_context, "someint_t",
        kefir_ast_type_signed_long(), NULL));
    ASSERT_OK(kefir_ast_global_context_define_type(&kft_mem, &global_context, "someint2_t",
        kefir_ast_type_signed_int(), NULL));
    ASSERT_OK(kefir_ast_global_context_define_type(&kft_mem, &global_context, "somenonint_t",
...
DEFINE_CASE(ast_declarator_analysis20, "AST declarator analysis - typedefs #2")
    ...
    struct kefir_ast_struct_type *struct_type1 = NULL;
    const struct kefir_ast_type *type1 = kefir_ast_type_structure(&kft_mem, context->type_bundle, "structure_one", &struct_type1);
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type1,
        "integer", kefir_ast_type_unsigned_int(),
        kefir_ast_alignment_const_expression(&kft_mem, kefir_ast_constant_expression_integer(&kft_mem, 8))));
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type1,
        "floatingPoint", kefir_ast_type_float(), NULL));
    ...
    ASSERT_OK(kefir_ast_struct_type_bitfield(&kft_mem, context->symbols, struct_type1,
        "field3", kefir_ast_type_signed_char(), NULL,
    ...
    ASSERT_OK(kefir_ast_structure_declaration_entry_append(&kft_mem, entry1,
        kefir_ast_declarator_identifier(&kft_mem, context->symbols, "integer"), NULL));
    ASSERT_OK(kefir_ast_structure_specifier_append_entry(&kft_mem, specifier1, entry1));
    ...
    ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &entry2->declaration.specifiers,
        kefir_ast_type_specifier_float(&kft_mem)));
    ASSERT_OK(kefir_ast_structure_declaration_entry_append(&kft_mem, entry2,
        kefir_ast_declarator_identifier(&kft_mem, context->symbols, "floatingPoint"), NULL));
    ASSERT_OK(kefir_ast_structure_specifier_append_entry(&kft_mem, specifier1, entry2));
    ...
    ASSERT_OK(kefir_ast_structure_declaration_entry_append(&kft_mem, entry3,
        kefir_ast_declarator_identifier(&kft_mem, context->symbols, "field1"),
    ...
    ASSERT_NODECL_TYPE(&kft_mem, context, type1, 1,
        kefir_ast_type_specifier_struct(&kft_mem, specifier1));
    ASSERT_OK(kefir_ast_local_context_define_type(&kft_mem, &local_context, "structure_one_t",
        type1, NULL));
    ...
    ASSERT_IDENTIFIER_TYPE(&kft_mem, context,
        ...
        0, 1,
        kefir_ast_type_specifier_struct(&kft_mem,
            kefir_ast_structure_specifier_init(&kft_mem, context->symbols, "structure_one", false)));
