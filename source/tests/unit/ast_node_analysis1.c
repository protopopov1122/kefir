...
DEFINE_CASE(ast_node_analysis_identifiers, "AST node analysis - identifiers")
    ...
    ASSERT_OK(kefir_ast_local_context_define_constant(&kft_mem, &local_context, "X",
        kefir_ast_constant_expression_integer(&kft_mem, 100), context->type_traits->underlying_enumeration_type, NULL));
    ASSERT_OK(kefir_ast_local_context_define_constant(&kft_mem, &local_context, "Y",
        kefir_ast_constant_expression_integer(&kft_mem, -150), context->type_traits->underlying_enumeration_type, NULL));
...
DEFINE_CASE(ast_node_analysis_array_subscripts, "AST node analysis - array subscripts")
    ...
    const struct kefir_ast_type *array3 = kefir_ast_type_array(&kft_mem, context->type_bundle,
        kefir_ast_type_signed_int(), kefir_ast_constant_expression_integer(&kft_mem, 256), NULL);
...
DEFINE_CASE(ast_node_analysis_struct_members2, "AST node analysis - struct members #2")
    ...
    ASSERT_OK(kefir_ast_global_context_define_external(&kft_mem, &global_context,
        "var2", struct3_type, NULL, NULL, NULL));
...
DEFINE_CASE(ast_node_analysis_indirect_struct_members1, "AST node analysis - indirect struct members #1")
    ...
    ASSERT_OK(kefir_ast_global_context_define_external(&kft_mem, &global_context, "var1",
        kefir_ast_type_pointer(&kft_mem, context->type_bundle, struct1_type), NULL, NULL, NULL));
...
DEFINE_CASE(ast_node_analysis_indirect_struct_members2, "AST node analysis - indirect struct members #2")
    ...
    ASSERT_OK(kefir_ast_global_context_define_external(&kft_mem, &global_context,
        "var2", kefir_ast_type_pointer(&kft_mem, context->type_bundle, struct3_type), NULL, NULL, NULL));
...
DEFINE_CASE(ast_node_analysis_function_calls, "AST node analysis - function calls")
    ...
    ASSERT_OK(kefir_ast_local_context_define_auto(&kft_mem, &local_context, "func2",
        function1_ptr_type, NULL, NULL, NULL));
...
DEFINE_CASE(ast_node_analysis_unary_operation_address, "AST node analysis - unary address operations")
    ...
    const struct kefir_ast_type *type2 = kefir_ast_type_array(&kft_mem, context->type_bundle,
        kefir_ast_type_float(), kefir_ast_constant_expression_integer(&kft_mem, 100), NULL);
...
DEFINE_CASE(ast_node_analysis_unary_operation_indirect, "AST node analysis - unary indirect operations")
    ...
    ASSERT_OK(kefir_ast_local_context_declare_external(&kft_mem, &local_context, "var2",
        kefir_ast_type_pointer(&kft_mem, context->type_bundle, type1), NULL, NULL));
    ASSERT_OK(kefir_ast_local_context_declare_external(&kft_mem, &local_context, "var3",
        kefir_ast_type_pointer(&kft_mem, context->type_bundle, type2), NULL, NULL));
    ...
    ASSERT_OK(kefir_ast_local_context_declare_external(&kft_mem, &local_context, "var5",
        kefir_ast_type_pointer(&kft_mem, context->type_bundle, function_type3), NULL, NULL));
...
DEFINE_CASE(ast_node_analysis_unary_operation_sizeof, "AST node analysis - unary sizeof operations")
    ...
    ASSERT_OK(kefir_ast_local_context_declare_external(&kft_mem, &local_context, "y",
        kefir_ast_type_array(&kft_mem, context->type_bundle, kefir_ast_type_char(),
            kefir_ast_constant_expression_integer(&kft_mem, 1), NULL), NULL, NULL));
    ASSERT_OK(kefir_ast_local_context_declare_external(&kft_mem, &local_context, "z",
        kefir_ast_type_vlen_array(&kft_mem, context->type_bundle, kefir_ast_type_char(),
            KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 128)), NULL), NULL, NULL));
...
#undef MAKE_TYPENAME
    ...
    struct kefir_ast_type_name *type_name6 = kefir_ast_new_type_name(&kft_mem,
        kefir_ast_declarator_array(&kft_mem, KEFIR_AST_DECLARATOR_ARRAY_BOUNDED,
            ...
            kefir_ast_declarator_identifier(&kft_mem, NULL, NULL)));
    ASSERT_OK(append_specifiers(&kft_mem, &type_name6->type_decl.specifiers, 1,
        kefir_ast_type_specifier_char(&kft_mem)));
    type_name6->type_decl.declarator->array.static_array = true;

    struct kefir_ast_type_name *type_name7 = kefir_ast_new_type_name(&kft_mem,
        kefir_ast_declarator_pointer(&kft_mem, kefir_ast_declarator_identifier(&kft_mem, NULL, NULL)));
    ASSERT_OK(append_specifiers(&kft_mem, &type_name7->type_decl.specifiers, 2,
        kefir_ast_type_specifier_char(&kft_mem),
...
#undef MAKE_TYPENAME
    ...
    struct kefir_ast_type_name *type_name7 = kefir_ast_new_type_name(&kft_mem,
        kefir_ast_declarator_pointer(&kft_mem, kefir_ast_declarator_identifier(&kft_mem, NULL, NULL)));
    ASSERT_OK(append_specifiers(&kft_mem, &type_name7->type_decl.specifiers, 1,
        kefir_ast_type_specifier_void(&kft_mem)));

    struct kefir_ast_type_name *type_name8 = kefir_ast_new_type_name(&kft_mem,
        kefir_ast_declarator_pointer(&kft_mem,
            kefir_ast_declarator_array(&kft_mem, KEFIR_AST_DECLARATOR_ARRAY_BOUNDED,
                KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 256)),
                kefir_ast_declarator_identifier(&kft_mem, NULL, NULL))));
    ASSERT_OK(append_specifiers(&kft_mem, &type_name8->type_decl.specifiers, 1,
        kefir_ast_type_specifier_void(&kft_mem)));
    ...
    ASSERT_TYPE_NAME(&kft_mem, context, type_name7, kefir_ast_type_pointer(&kft_mem, context->type_bundle, kefir_ast_type_void()));
    ASSERT_TYPE_NAME(&kft_mem, context, type_name8, kefir_ast_type_array(&kft_mem, context->type_bundle,
