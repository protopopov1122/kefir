...
const struct kefir_ast_type *function1_type =
    kefir_ast_type_function(&kft_mem, context->type_bundle, kefir_ast_type_void(), "func1", &function1);
...
const struct kefir_ast_type *function2_type =
    kefir_ast_type_function(&kft_mem, context->type_bundle, kefir_ast_type_unsigned_short(), "func2", &function2);
...
const struct kefir_ast_type *array3 = kefir_ast_type_array(&kft_mem, context->type_bundle, kefir_ast_type_signed_int(),
...
ASSERT_OK(kefir_ast_global_context_declare_external(&kft_mem, &global_context, "var1", array1, NULL, NULL));
ASSERT_OK(kefir_ast_global_context_declare_external(&kft_mem, &global_context, "var2", array2, NULL, NULL));
ASSERT_OK(kefir_ast_global_context_declare_external(&kft_mem, &global_context, "var3", array3, NULL, NULL));
ASSERT_OK(kefir_ast_global_context_declare_external(&kft_mem, &global_context, "var4", array4, NULL, NULL));
ASSERT_OK(kefir_ast_global_context_declare_external(&kft_mem, &global_context, "var5", array5, NULL, NULL));
ASSERT_OK(kefir_ast_global_context_declare_external(&kft_mem, &global_context, "var6", array6, NULL, NULL));
...
const struct kefir_ast_type *struct1_type = kefir_ast_type_structure(&kft_mem, context->type_bundle, "type1", &struct1);
...
ASSERT_OK(kefir_ast_global_context_define_external(&kft_mem, &global_context, "var1", struct1_type, NULL, NULL, NULL));
...
const struct kefir_ast_type *struct2_type = kefir_ast_type_structure(&kft_mem, context->type_bundle, "type2", &struct2);
...
const struct kefir_ast_type *struct3_type = kefir_ast_type_structure(&kft_mem, context->type_bundle, "type3", &struct3);
...
ASSERT_OK(kefir_ast_global_context_define_external(&kft_mem, &global_context, "var2", struct3_type, NULL, NULL, NULL));
...
ASSERT_OK(kefir_ast_global_context_define_external(&kft_mem, &global_context, "var1",
                                                   kefir_ast_type_pointer(&kft_mem, context->type_bundle, struct1_type),
                                                   NULL, NULL, NULL));
...
ASSERT_INDIRECT_STRUCT_MEMBER(&kft_mem, context, "var1", "field4",
                              kefir_ast_type_unbounded_array(&kft_mem, context->type_bundle, kefir_ast_type_float(),
                                                             NULL));
...
ASSERT_OK(kefir_ast_global_context_define_external(&kft_mem, &global_context, "var2",
                                                   kefir_ast_type_pointer(&kft_mem, context->type_bundle, struct3_type),
                                                   NULL, NULL, NULL));
...
struct kefir_ast_function_type *function1 = NULL;
const struct kefir_ast_type *function1_type =
    kefir_ast_type_function(&kft_mem, context->type_bundle, kefir_ast_type_signed_long_long(), "func1", &function1);
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, function1, "",
                                            kefir_ast_type_unsigned_int(), NULL));

struct kefir_ast_function_type *function2 = NULL;
const struct kefir_ast_type *function2_type =
    kefir_ast_type_function(&kft_mem, context->type_bundle, kefir_ast_type_signed_long_long(), "func3", &function2);
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, function2, "", kefir_ast_type_float(),
                                            NULL));
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, function2, "", kefir_ast_type_float(),
                                            NULL));
...
ASSERT_OK(kefir_ast_local_context_declare_function(&kft_mem, &local_context, KEFIR_AST_FUNCTION_SPECIFIER_NONE,
                                                   function2_type, NULL));
ASSERT_OK(kefir_ast_local_context_define_auto(&kft_mem, &local_context, "func2", function1_ptr_type, NULL, NULL, NULL));
...
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_PLUS, kefir_ast_new_constant_double(&kft_mem, 2.71828),
                       kefir_ast_type_double(), true, false, false);
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_NEGATE, kefir_ast_new_constant_float(&kft_mem, 3.14f),
                       kefir_ast_type_float(), true, false, false);
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_NEGATE, kefir_ast_new_constant_double(&kft_mem, 2.71828),
                       kefir_ast_type_double(), true, false, false);

ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_LOGICAL_NEGATE,
                       kefir_ast_new_constant_bool(&kft_mem, false), kefir_ast_type_signed_int(), true, false, false);
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_LOGICAL_NEGATE,
...
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_LOGICAL_NEGATE,
                       kefir_ast_new_constant_long(&kft_mem, -1000), kefir_ast_type_signed_int(), true, false, false);
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_LOGICAL_NEGATE,
                       kefir_ast_new_constant_ulong(&kft_mem, 1000), kefir_ast_type_signed_int(), true, false, false);
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_LOGICAL_NEGATE,
...
const struct kefir_ast_type *type2 = kefir_ast_type_array(&kft_mem, context->type_bundle, kefir_ast_type_float(),
...
const struct kefir_ast_type *function_type3 =
    kefir_ast_type_function(&kft_mem, context->type_bundle, kefir_ast_type_void(), "func1", &function3);
...
ASSERT_OK(kefir_ast_global_context_declare_external(
    ...
    NULL, NULL));
ASSERT_OK(kefir_ast_local_context_declare_external(&kft_mem, &local_context, "var1", kefir_ast_type_signed_int(), NULL,
                                                   NULL));
ASSERT_OK(kefir_ast_local_context_define_static(&kft_mem, &local_context, "var2", type1, NULL, NULL, NULL));
...
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_ADDRESS,
                       ...
                       kefir_ast_type_pointer(&kft_mem, context->type_bundle, kefir_ast_type_signed_int()), true, false,
                       false);
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_ADDRESS,
...
struct kefir_ast_function_type *function3 = NULL;
const struct kefir_ast_type *function_type3 =
    kefir_ast_type_function(&kft_mem, context->type_bundle, kefir_ast_type_double(), "func1", &function3);
ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, function3, "", kefir_ast_type_bool(),
                                            NULL));
...
ASSERT_OK(kefir_ast_local_context_declare_external(&kft_mem, &local_context, "var3",
                                                   ...
                                                   NULL));
ASSERT_OK(kefir_ast_local_context_declare_external(&kft_mem, &local_context, "var4", type2, NULL, NULL));
...
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_INDIRECTION,
                       kefir_ast_new_identifier(&kft_mem, context->symbols, "var1"), kefir_ast_type_signed_int(), false,
                       true, true);
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_INDIRECTION,
...
ASSERT_OK(kefir_ast_local_context_declare_external(&kft_mem, &local_context, "var1", kefir_ast_type_bool(), NULL,
                                                   NULL));
ASSERT_OK(kefir_ast_local_context_declare_external(&kft_mem, &local_context, "var2", kefir_ast_type_signed_int(), NULL,
                                                   NULL));
ASSERT_OK(kefir_ast_local_context_declare_external(&kft_mem, &local_context, "var3",
...
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_POSTFIX_DECREMENT,
                       ...
                       false, false);
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_POSTFIX_DECREMENT,
                       kefir_ast_new_identifier(&kft_mem, context->symbols, "var2"), kefir_ast_type_signed_int(), false,
                       false, false);
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_POSTFIX_DECREMENT,
                       kefir_ast_new_identifier(&kft_mem, context->symbols, "var3"),
...
ASSERT_OK(kefir_ast_local_context_declare_external(&kft_mem, &local_context, "x", kefir_ast_type_unsigned_int(), NULL,
                                                   NULL));
...
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_SIZEOF,
                       ...
                       false, false);
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_SIZEOF,
                       kefir_ast_new_identifier(&kft_mem, context->symbols, "z"), kefir_ast_type_signed_int(), false,
                       false, false);
...
ASSERT_OK(append_specifiers(&kft_mem, &type_name6->type_decl.specifiers, 1, kefir_ast_type_specifier_char(&kft_mem)));
...
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_ALIGNOF, type_name1, kefir_ast_type_signed_int(), true,
                       false, false);
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_ALIGNOF, type_name2, kefir_ast_type_signed_int(), true,
                       false, false);
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_ALIGNOF, type_name3, kefir_ast_type_signed_int(), true,
                       false, false);
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_ALIGNOF, type_name4, kefir_ast_type_signed_int(), true,
                       false, false);
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_ALIGNOF, type_name5, kefir_ast_type_signed_int(), true,
                       false, false);
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_ALIGNOF, type_name6, kefir_ast_type_signed_int(), true,
                       false, false);
ASSERT_UNARY_OPERATION(&kft_mem, context, KEFIR_AST_OPERATION_ALIGNOF, type_name7, kefir_ast_type_signed_int(), true,
                       false, false);
...
struct kefir_ast_type_name *type_name7 = kefir_ast_new_type_name(
    &kft_mem, kefir_ast_declarator_pointer(&kft_mem, kefir_ast_declarator_identifier(&kft_mem, NULL, NULL)));
ASSERT_OK(append_specifiers(&kft_mem, &type_name7->type_decl.specifiers, 1, kefir_ast_type_specifier_void(&kft_mem)));
...
struct kefir_ast_type_name *type_name8 = kefir_ast_new_type_name(
    &kft_mem, kefir_ast_declarator_pointer(
                  &kft_mem, kefir_ast_declarator_array(&kft_mem, KEFIR_AST_DECLARATOR_ARRAY_BOUNDED,
                                                       ...
                                                       kefir_ast_declarator_identifier(&kft_mem, NULL, NULL))));
ASSERT_OK(append_specifiers(&kft_mem, &type_name8->type_decl.specifiers, 1, kefir_ast_type_specifier_void(&kft_mem)));
