...
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, structure1, "x", kefir_ast_type_signed_long_long(),
                                      NULL));
...
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, structure3, "c",
                                      ...
                                      NULL));
ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, structure3, "d",
                                      kefir_ast_type_array(&kft_mem, context->type_bundle,
                                                           kefir_ast_type_unsigned_char(),
...
ASSERT_OK(kefir_ast_initializer_list_append(
    &kft_mem, &init6->list, NULL,
    kefir_ast_new_expression_initializer(&kft_mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_char(&kft_mem, 'e')))));
ASSERT_OK(kefir_ast_initializer_list_append(
    &kft_mem, &init6->list, NULL,
    kefir_ast_new_expression_initializer(&kft_mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_char(&kft_mem, 'l')))));
ASSERT_OK(kefir_ast_initializer_list_append(
