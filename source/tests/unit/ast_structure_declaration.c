...
DEFINE_CASE(ast_structure_declaration1, "AST Declarations - structure declarations #1")
    ...
    ASSERT_OK(kefir_ast_structure_specifier_init(&kft_mem, &specifier1, &symbols, "struct1", false));
    ...
    ASSERT_OK(kefir_ast_structure_declaration_entry_append(&kft_mem, entry1,
        kefir_ast_declarator_identifier(&kft_mem, &symbols, "field1"), NULL));
...
DEFINE_CASE(ast_structure_declaration2, "AST Declarations - structure declarations #2")
    ...
    struct kefir_ast_structure_specifier specifier1;
    ASSERT_OK(kefir_ast_structure_specifier_init(&kft_mem, &specifier1, &symbols, "struct_one", true));
    ...
    do {
        ...
        ASSERT_OK(kefir_ast_structure_declaration_entry_append(&kft_mem, entry1,
            kefir_ast_declarator_identifier(&kft_mem, &symbols, "a"), NULL));
    ...
    do {
        ...
        ASSERT_OK(kefir_ast_structure_declaration_entry_append(&kft_mem, entry2,
            kefir_ast_declarator_identifier(&kft_mem, &symbols, "b"), NULL));
        ASSERT_OK(kefir_ast_structure_declaration_entry_append(&kft_mem, entry2,
            kefir_ast_declarator_identifier(&kft_mem, &symbols, "c"),
    ...
    do {
        ...
        const struct kefir_list_entry *specifier_iter = kefir_ast_declarator_specifier_list_iter(&entry->declaration.specifiers, &entry_specifier1);
    ...
    do {
        ...
        struct kefir_ast_declarator_specifier *entry_specifier1 = NULL;
        struct kefir_list_entry *specifier_iter = kefir_ast_declarator_specifier_list_iter(&entry->declaration.specifiers, &entry_specifier1);
        ASSERT(specifier_iter != NULL);
        ASSERT(entry_specifier1->klass == KEFIR_AST_TYPE_SPECIFIER);
        ASSERT(entry_specifier1->type_specifier.specifier == KEFIR_AST_TYPE_SPECIFIER_LONG);
    ...
    do {
        ...
        ASSERT(((struct kefir_ast_constant *) entry_declarator->declarator->array.length->self)->type == KEFIR_AST_INT_CONSTANT);
