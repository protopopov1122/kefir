...
ASSERT_IDENTIFIER_TYPE(&kft_mem, context, kefir_ast_type_unsigned_char(), KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_UNKNOWN,
...
ASSERT_IDENTIFIER_TYPE(&kft_mem, context, kefir_ast_type_unsigned_short(), KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_UNKNOWN,
                       KEFIR_AST_FUNCTION_SPECIFIER_NONE, 0, 2, kefir_ast_type_specifier_unsigned(&kft_mem),
                       kefir_ast_type_specifier_short(&kft_mem));
ASSERT_IDENTIFIER_TYPE(&kft_mem, context, kefir_ast_type_unsigned_short(), KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_UNKNOWN,
                       KEFIR_AST_FUNCTION_SPECIFIER_NONE, 0, 3, kefir_ast_type_specifier_unsigned(&kft_mem),
                       kefir_ast_type_specifier_short(&kft_mem), kefir_ast_type_specifier_int(&kft_mem));
...
ASSERT_IDENTIFIER_TYPE(&kft_mem, context, kefir_ast_type_unsigned_long(), KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_UNKNOWN,
                       KEFIR_AST_FUNCTION_SPECIFIER_NONE, 0, 2, kefir_ast_type_specifier_unsigned(&kft_mem),
                       kefir_ast_type_specifier_long(&kft_mem));
ASSERT_IDENTIFIER_TYPE(&kft_mem, context, kefir_ast_type_unsigned_long(), KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_UNKNOWN,
                       KEFIR_AST_FUNCTION_SPECIFIER_NONE, 0, 3, kefir_ast_type_specifier_unsigned(&kft_mem),
                       kefir_ast_type_specifier_long(&kft_mem), kefir_ast_type_specifier_int(&kft_mem));
ASSERT_IDENTIFIER_TYPE(&kft_mem, context, kefir_ast_type_signed_long_long(), KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_UNKNOWN,
                       KEFIR_AST_FUNCTION_SPECIFIER_NONE, 0, 2, kefir_ast_type_specifier_long(&kft_mem),
                       kefir_ast_type_specifier_long(&kft_mem));
ASSERT_IDENTIFIER_TYPE(&kft_mem, context, kefir_ast_type_signed_long_long(), KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_UNKNOWN,
                       KEFIR_AST_FUNCTION_SPECIFIER_NONE, 0, 3, kefir_ast_type_specifier_signed(&kft_mem),
...
ASSERT_IDENTIFIER_TYPE(&kft_mem, context, kefir_ast_type_signed_long_long(), KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_UNKNOWN,
                       KEFIR_AST_FUNCTION_SPECIFIER_NONE, 0, 3, kefir_ast_type_specifier_long(&kft_mem),
                       kefir_ast_type_specifier_long(&kft_mem), kefir_ast_type_specifier_int(&kft_mem));
ASSERT_IDENTIFIER_TYPE(&kft_mem, context, kefir_ast_type_signed_long_long(), KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_UNKNOWN,
                       KEFIR_AST_FUNCTION_SPECIFIER_NONE, 0, 4, kefir_ast_type_specifier_signed(&kft_mem),
                       kefir_ast_type_specifier_long(&kft_mem), kefir_ast_type_specifier_long(&kft_mem),
                       kefir_ast_type_specifier_int(&kft_mem));
...
ASSERT_IDENTIFIER_TYPE(&kft_mem, context, kefir_ast_type_bool(), KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_UNKNOWN,
...
ASSERT_IDENTIFIER_TYPE(&kft_mem, context, kefir_ast_type_unsigned_short(), KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_EXTERN,
                       KEFIR_AST_FUNCTION_SPECIFIER_NONE, 0, 4, kefir_ast_type_specifier_int(&kft_mem),
                       kefir_ast_storage_class_specifier_extern(&kft_mem), kefir_ast_type_specifier_unsigned(&kft_mem),
                       kefir_ast_type_specifier_short(&kft_mem));
...
ASSERT_IDENTIFIER_TYPE(&kft_mem, context, kefir_ast_type_signed_int(),
                       KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_EXTERN_THREAD_LOCAL, KEFIR_AST_FUNCTION_SPECIFIER_NONE, 0, 2,
                       kefir_ast_storage_class_specifier_extern(&kft_mem),
                       kefir_ast_storage_class_specifier_thread_local(&kft_mem));

ASSERT_IDENTIFIER_TYPE(&kft_mem, context, kefir_ast_type_unsigned_int(),
                       KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_EXTERN_THREAD_LOCAL, KEFIR_AST_FUNCTION_SPECIFIER_NONE, 0, 3,
                       kefir_ast_storage_class_specifier_thread_local(&kft_mem),
                       kefir_ast_type_specifier_unsigned(&kft_mem), kefir_ast_storage_class_specifier_extern(&kft_mem));

ASSERT_IDENTIFIER_TYPE(&kft_mem, context, kefir_ast_type_signed_char(),
                       KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_STATIC_THREAD_LOCAL, KEFIR_AST_FUNCTION_SPECIFIER_NONE, 0, 4,
                       kefir_ast_storage_class_specifier_static(&kft_mem), kefir_ast_type_specifier_char(&kft_mem),
                       kefir_ast_storage_class_specifier_thread_local(&kft_mem),
...
ASSERT_IDENTIFIER_TYPE(&kft_mem, context,
                       ...
                       kefir_ast_type_qualifier_const(&kft_mem), kefir_ast_storage_class_specifier_register(&kft_mem),
...
ASSERT_FUNCTION_TYPE(&kft_mem, context, type5, KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_EXTERN,
                     ...
                     kefir_ast_type_qualifier_const(&kft_mem), kefir_ast_function_specifier_noreturn(&kft_mem),
                     kefir_ast_type_qualifier_restrict(&kft_mem), kefir_ast_type_specifier_long(&kft_mem),
                     kefir_ast_function_specifier_noreturn(&kft_mem), kefir_ast_function_specifier_inline(&kft_mem));
...
ASSERT_IDENTIFIER_TYPE(&kft_mem, context, kefir_ast_type_char(), KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_UNKNOWN,
                       ...
                       kefir_ast_alignment_specifier(&kft_mem,
...
ASSERT_IDENTIFIER_TYPE(
    ...
    kefir_ast_alignment_specifier(&kft_mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 1))),
    kefir_ast_type_specifier_long(&kft_mem), kefir_ast_storage_class_specifier_register(&kft_mem),
    kefir_ast_alignment_specifier(&kft_mem, KEFIR_AST_NODE_BASE(type_name9)), kefir_ast_type_specifier_long(&kft_mem),
    kefir_ast_alignment_specifier(&kft_mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 4))),
