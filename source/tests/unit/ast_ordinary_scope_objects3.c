...
const struct kefir_ast_type *type2 = kefir_ast_type_array(&kft_mem, context.context.type_bundle, kefir_ast_type_float(),
...
ASSERT_NOK(kefir_ast_local_context_define_static(&kft_mem, &context, "var1", kefir_ast_type_signed_int(), NULL, init1,
                                                 NULL));
ASSERT_OK(kefir_ast_initializer_free(&kft_mem, init1));

struct kefir_ast_initializer *init2 =
    kefir_ast_new_expression_initializer(&kft_mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_char(&kft_mem, 'a')));
ASSERT_OK(kefir_ast_local_context_define_static(&kft_mem, &context, "var2", kefir_ast_type_signed_char(), NULL, init2,
                                                NULL));
ASSERT_RESOLVED_IDENTIFIER(&context, "var2", KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_STATIC, kefir_ast_type_signed_char(),
                           KEFIR_AST_SCOPED_IDENTIFIER_NONE_LINKAGE);

ASSERT_NOK(kefir_ast_local_context_define_static(&kft_mem, &context, "var2", kefir_ast_type_signed_char(), NULL, NULL,
                                                 NULL));
ASSERT_RESOLVED_IDENTIFIER(&context, "var2", KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_STATIC, kefir_ast_type_signed_char(),
                           KEFIR_AST_SCOPED_IDENTIFIER_NONE_LINKAGE);
...
ASSERT_OK(kefir_ast_initializer_list_append(
    &kft_mem, &init4->list, kefir_ast_new_index_desginator(&kft_mem, 2, NULL),
...
ASSERT_OK(kefir_ast_local_context_define_static_thread_local(&kft_mem, &context, "var1", kefir_ast_type_signed_int(),
                                                             NULL, NULL, NULL));
...
struct kefir_ast_initializer *init1 =
    kefir_ast_new_expression_initializer(&kft_mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_char(&kft_mem, 'a')));
ASSERT_NOK(kefir_ast_local_context_define_static_thread_local(&kft_mem, &context, "var1", kefir_ast_type_signed_int(),
                                                              NULL, init1, NULL));
ASSERT_OK(kefir_ast_initializer_free(&kft_mem, init1));

struct kefir_ast_initializer *init2 =
    kefir_ast_new_expression_initializer(&kft_mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_char(&kft_mem, 'a')));
ASSERT_OK(kefir_ast_local_context_define_static_thread_local(&kft_mem, &context, "var2", kefir_ast_type_signed_char(),
                                                             NULL, init2, NULL));
...
ASSERT_NOK(kefir_ast_local_context_define_static_thread_local(&kft_mem, &context, "var2", kefir_ast_type_signed_char(),
                                                              NULL, NULL, NULL));
ASSERT_RESOLVED_IDENTIFIER(&context, "var2", KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_STATIC_THREAD_LOCAL,
                           kefir_ast_type_signed_char(), KEFIR_AST_SCOPED_IDENTIFIER_NONE_LINKAGE);
...
ASSERT_NOK(kefir_ast_local_context_define_auto(&kft_mem, &context, "var1", kefir_ast_type_signed_short(), NULL, NULL,
                                               NULL));
ASSERT_RESOLVED_IDENTIFIER(&context, "var1", KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_AUTO, kefir_ast_type_signed_short(),
                           KEFIR_AST_SCOPED_IDENTIFIER_NONE_LINKAGE);
...
ASSERT_NOK(kefir_ast_local_context_define_auto(&kft_mem, &context, "var1", kefir_ast_type_signed_short(), NULL, init1,
                                               NULL));
ASSERT_OK(kefir_ast_initializer_free(&kft_mem, init1));
ASSERT_RESOLVED_IDENTIFIER(&context, "var1", KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_AUTO, kefir_ast_type_signed_short(),
                           KEFIR_AST_SCOPED_IDENTIFIER_NONE_LINKAGE);
...
ASSERT_OK(kefir_ast_initializer_list_append(
    ...
    kefir_ast_new_expression_initializer(&kft_mem, KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 60)))));
ASSERT_OK(kefir_ast_local_context_define_auto(&kft_mem, &context, "var2", kefir_ast_type_double(), NULL, init2, NULL));
...
struct kefir_ast_initializer *init3 = kefir_ast_new_expression_initializer(
    &kft_mem, KEFIR_AST_NODE_BASE(KEFIR_AST_MAKE_STRING_LITERAL(&kft_mem, "Hey!")));
...
ASSERT_OK(kefir_ast_local_context_define_register(&kft_mem, &context, "var1", kefir_ast_type_signed_short(), NULL, NULL,
                                                  NULL));
ASSERT_RESOLVED_IDENTIFIER(&context, "var1", KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_REGISTER, kefir_ast_type_signed_short(),
                           KEFIR_AST_SCOPED_IDENTIFIER_NONE_LINKAGE);
