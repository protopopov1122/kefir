...
DEFINE_CASE(ast_nodes_labeled_statements1, "AST nodes - labeled statements #1")
    ...
    struct kefir_ast_labeled_statement *stmt1 = kefir_ast_new_labeled_statement(&kft_mem, &symbols,
        "label1", KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(&kft_mem, 0)));
    ASSERT(stmt1 != NULL);
    ...
    struct kefir_ast_labeled_statement *stmt2 = kefir_ast_new_labeled_statement(&kft_mem, &symbols,
        "label2", KEFIR_AST_NODE_BASE(kefir_ast_new_identifier(&kft_mem, &symbols, "label3")));
    ASSERT(stmt2 != NULL);
...
DEFINE_CASE(ast_nodes_case_statements1, "AST nodes - case statements #1")
    ...
    struct kefir_ast_case_statement *stmt1 = kefir_ast_new_case_statement(&kft_mem,
        KEFIR_AST_NODE_BASE(kefir_ast_new_constant_long(&kft_mem, 1)),
    ...
    struct kefir_ast_case_statement *stmt3 = kefir_ast_new_case_statement(&kft_mem,
        NULL,
        KEFIR_AST_NODE_BASE(kefir_ast_new_identifier(&kft_mem, &symbols, "case3")));
    ASSERT(stmt3 != NULL);
...
DEFINE_CASE(ast_nodes_expression_statements1, "AST nodes - expression statements #1")
    ...
    struct kefir_ast_expression_statement *stmt1 = kefir_ast_new_expression_statement(&kft_mem,
        KEFIR_AST_NODE_BASE(KEFIR_AST_MAKE_STRING_LITERAL(&kft_mem, "Goodbye, cruel world!")));
    ASSERT(stmt1 != NULL);
    ...
    struct kefir_ast_expression_statement *stmt2 = kefir_ast_new_expression_statement(&kft_mem,
        KEFIR_AST_NODE_BASE(kefir_ast_new_constant_float(&kft_mem, 6.1987f)));
    ASSERT(stmt2 != NULL);
