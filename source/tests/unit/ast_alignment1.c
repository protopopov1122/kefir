...
DEFINE_CASE(ast_alignment_as_type, "AST alignment - as type")
    ...
    ASSERT_ALIGN_AS_TYPE(&kft_mem, context,
        kefir_ast_type_bool(), 1);
    ASSERT_ALIGN_AS_TYPE(&kft_mem, context,
        kefir_ast_type_char(), 1);
    ASSERT_ALIGN_AS_TYPE(&kft_mem, context,
        kefir_ast_type_unsigned_char(), 1);
    ASSERT_ALIGN_AS_TYPE(&kft_mem, context,
        kefir_ast_type_signed_char(), 1);
...
DEFINE_CASE(ast_alignment_as_const_expr, "AST alignment - as constant expression")
    ...
    for (kefir_size_t i = 0; i < 5; i++) {
        ASSERT_ALIGN_AS_CONST_EXPR(&kft_mem, context,
            kefir_ast_constant_expression_integer(&kft_mem, 1 << i),
            1u << i);

        ASSERT_ALIGN_AS_CONST_EXPR(&kft_mem, context,
            kefir_ast_new_constant_expression(&kft_mem,
                KEFIR_AST_NODE_BASE(kefir_ast_new_constant_uint(&kft_mem, 1 << i))),
            1u << i);

        ASSERT_ALIGN_AS_CONST_EXPR(&kft_mem, context,
            kefir_ast_new_constant_expression(&kft_mem,
                KEFIR_AST_NODE_BASE(kefir_ast_new_binary_operation(&kft_mem, KEFIR_AST_OPERATION_SHIFT_LEFT,
