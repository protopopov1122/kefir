...
DEFINE_CASE(ast_node_analysis_declarations5, "AST node analysis - declarations #5")
    ...
    const struct kefir_ast_scoped_identifier *scoped_id2 = NULL;
    ASSERT_OK(kefir_ast_global_context_resolve_scoped_ordinary_identifier(&global_context, "XVAL",
        &scoped_id2));
    ...
    ASSERT_OK(kefir_ast_global_context_resolve_scoped_ordinary_identifier(&global_context, "YVAL",
        &scoped_id3));
    ...
    ASSERT_OK(kefir_ast_global_context_resolve_scoped_ordinary_identifier(&global_context, "AVAL",
        &scoped_id4));
...
DEFINE_CASE(ast_node_analysis_declarations6, "AST node analysis - declarations #6")
    ...
    ASSERT_OK(kefir_ast_initializer_list_append(&kft_mem, &init1->list,
        NULL, kefir_ast_new_expression_initializer(&kft_mem, KEFIR_AST_NODE_BASE(KEFIR_AST_MAKE_STRING_LITERAL(&kft_mem, "Hello, world!")))));
    ...
    const struct kefir_ast_scoped_identifier *scoped_id1 = NULL;
    ASSERT_OK(kefir_ast_global_context_resolve_scoped_ordinary_identifier(&global_context, "arr",
        &scoped_id1));
    ASSERT(scoped_id1->klass == KEFIR_AST_SCOPE_IDENTIFIER_OBJECT);
    ASSERT(scoped_id1->object.storage == KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_EXTERN);
    ASSERT(scoped_id1->object.linkage == KEFIR_AST_SCOPED_IDENTIFIER_EXTERNAL_LINKAGE);
...
DEFINE_CASE(ast_node_analysis_declarations7, "AST node analysis - declarations #7")
    ...
    struct kefir_ast_structure_specifier *specifier1 = kefir_ast_structure_specifier_init(&kft_mem,
        context->symbols, "structureOne", true);
    struct kefir_ast_structure_declaration_entry *entry1 = kefir_ast_structure_declaration_entry_alloc(&kft_mem);
    ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &entry1->declaration.specifiers,
        kefir_ast_type_specifier_int(&kft_mem)));
    ASSERT_OK(kefir_ast_structure_declaration_entry_append(&kft_mem, entry1,
    ...
    struct kefir_ast_declaration *decl2 = kefir_ast_new_declaration(&kft_mem,
        kefir_ast_declarator_identifier(&kft_mem, context->symbols, "some_struct"), init2);
    ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &decl2->specifiers,
        kefir_ast_type_specifier_struct(&kft_mem,
            kefir_ast_structure_specifier_init(&kft_mem, context->symbols, "structureOne", false))));
    ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &decl2->specifiers,
        kefir_ast_type_qualifier_volatile(&kft_mem)));
    ...
    struct kefir_ast_struct_type *struct_type1 = NULL;
    const struct kefir_ast_type *type1 = kefir_ast_type_structure(&kft_mem, context->type_bundle,
        "structureOne", &struct_type1);
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type1,
        "x", kefir_ast_type_signed_int(), NULL));
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type1,
        "y", kefir_ast_type_pointer(&kft_mem, context->type_bundle, kefir_ast_type_float()), NULL));
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, struct_type1,
        "z", kefir_ast_type_unsigned_long(), NULL));

    const struct kefir_ast_type *type2 = kefir_ast_type_qualified(&kft_mem, context->type_bundle,
        type1, (struct kefir_ast_type_qualification){
            .volatile_type = true
        });
    ...
    const struct kefir_ast_scoped_identifier *scoped_id1 = NULL;
    ASSERT_OK(kefir_ast_local_context_resolve_scoped_ordinary_identifier(&local_context, "some_struct",
        &scoped_id1));
    ASSERT(scoped_id1->klass == KEFIR_AST_SCOPE_IDENTIFIER_OBJECT);
    ASSERT(scoped_id1->object.storage == KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_AUTO);
    ASSERT(scoped_id1->object.linkage == KEFIR_AST_SCOPED_IDENTIFIER_NONE_LINKAGE);
...
DEFINE_CASE(ast_node_analysis_declarations8, "AST node analysis - declarations #8")
    ...
    struct kefir_ast_structure_specifier *specifier1 = kefir_ast_structure_specifier_init(&kft_mem,
        context->symbols, "unionS", true);
    struct kefir_ast_structure_declaration_entry *entry1 = kefir_ast_structure_declaration_entry_alloc(&kft_mem);
    ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &entry1->declaration.specifiers,
        kefir_ast_type_specifier_unsigned(&kft_mem)));
    ASSERT_OK(kefir_ast_structure_declaration_entry_append(&kft_mem, entry1,
        kefir_ast_declarator_identifier(&kft_mem, context->symbols, "option1"), NULL));
    ASSERT_OK(kefir_ast_structure_specifier_append_entry(&kft_mem, specifier1, entry1));
    ...
    ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &entry2->declaration.specifiers,
        kefir_ast_type_specifier_double(&kft_mem)));
    ASSERT_OK(kefir_ast_structure_declaration_entry_append(&kft_mem, entry2,
        kefir_ast_declarator_pointer(&kft_mem, kefir_ast_declarator_identifier(&kft_mem, context->symbols, "option2")), NULL));
    ASSERT_OK(kefir_ast_structure_specifier_append_entry(&kft_mem, specifier1, entry2));
    ...
    ASSERT_OK(kefir_ast_structure_declaration_entry_append(&kft_mem, entry3,
        kefir_ast_declarator_identifier(&kft_mem, context->symbols, "option3"), NULL));
    ASSERT_OK(kefir_ast_structure_specifier_append_entry(&kft_mem, specifier1, entry3));
    ...
    struct kefir_ast_declaration *decl1 = kefir_ast_new_declaration(&kft_mem,
        kefir_ast_declarator_array(&kft_mem, KEFIR_AST_DECLARATOR_ARRAY_BOUNDED,
            ...
            kefir_ast_declarator_identifier(&kft_mem, context->symbols, "unionS_t")), NULL);
    ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &decl1->specifiers,
    ...
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, union_type1,
        "option1", kefir_ast_type_unsigned_int(), NULL));
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, union_type1,
        "option2", kefir_ast_type_pointer(&kft_mem, context->type_bundle, kefir_ast_type_double()), NULL));
    ASSERT_OK(kefir_ast_struct_type_field(&kft_mem, context->symbols, union_type1,
        "option3", kefir_ast_type_signed_short(), NULL));

    const struct kefir_ast_type *type2 = kefir_ast_type_array(&kft_mem, context->type_bundle,
        type1, kefir_ast_constant_expression_integer(&kft_mem, 16), NULL);
    ...
    const struct kefir_ast_scoped_identifier *scoped_id1 = NULL;
    ASSERT_OK(kefir_ast_local_context_resolve_scoped_ordinary_identifier(&local_context, "unionS_t",
        &scoped_id1));
    ...
    struct kefir_ast_declaration *decl2 = kefir_ast_new_declaration(&kft_mem,
        kefir_ast_declarator_pointer(&kft_mem,
            kefir_ast_declarator_identifier(&kft_mem, context->symbols, "Something")), NULL);
    ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &decl2->specifiers,
        kefir_ast_type_specifier_typedef(&kft_mem, context->symbols, "unionS_t")));
    ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &decl2->specifiers,
        kefir_ast_storage_class_specifier_static(&kft_mem)));
    ASSERT_OK(kefir_ast_analyze_node(&kft_mem, context, KEFIR_AST_NODE_BASE(decl2)));

    const struct kefir_ast_type *type3 = kefir_ast_type_pointer(&kft_mem, context->type_bundle,
        type2);
    ...
    const struct kefir_ast_scoped_identifier *scoped_id2 = NULL;
    ASSERT_OK(kefir_ast_local_context_resolve_scoped_ordinary_identifier(&local_context, "Something",
        &scoped_id2));
...
DEFINE_CASE(ast_node_analysis_declarations11, "AST node analysis - declarations #11")
    ...
    struct kefir_ast_declaration *param1 = kefir_ast_new_declaration(&kft_mem,
        kefir_ast_declarator_identifier(&kft_mem, context->symbols, "p1"), NULL);
    ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &param1->specifiers,
        kefir_ast_type_qualifier_const(&kft_mem)));
    ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &param1->specifiers,
        kefir_ast_type_specifier_enum(&kft_mem, specifier1)));
    ...
    struct kefir_ast_declaration *param2 = kefir_ast_new_declaration(&kft_mem,
        kefir_ast_declarator_array(&kft_mem, KEFIR_AST_DECLARATOR_ARRAY_BOUNDED,
            ...
            kefir_ast_declarator_identifier(&kft_mem, context->symbols, "p2")), NULL);
    ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &param2->specifiers,
        kefir_ast_type_specifier_enum(&kft_mem, kefir_ast_enum_specifier_init(&kft_mem, context->symbols, "enum1", false))));
    ...
    struct kefir_ast_declarator *decl1 = kefir_ast_declarator_function(&kft_mem,
        kefir_ast_declarator_identifier(&kft_mem, context->symbols, "fun"));
    ASSERT_OK(kefir_list_insert_after(&kft_mem, &decl1->function.parameters, kefir_list_tail(&decl1->function.parameters),
        param1));
    ASSERT_OK(kefir_list_insert_after(&kft_mem, &decl1->function.parameters, kefir_list_tail(&decl1->function.parameters),
        param2));
    ...
    struct kefir_ast_declaration *func1 = kefir_ast_new_declaration(&kft_mem,
        kefir_ast_declarator_pointer(&kft_mem, decl1), NULL);
    ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &func1->specifiers,
        kefir_ast_function_specifier_inline(&kft_mem)));
    ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &func1->specifiers,
        kefir_ast_function_specifier_noreturn(&kft_mem)));
    ...
    ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type2,
        "p1", kefir_ast_type_qualified(&kft_mem, context->type_bundle, type1,
            (struct kefir_ast_type_qualification){
                .constant = true
            }), &(kefir_ast_scoped_identifier_storage_t){KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_UNKNOWN}));
    ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type2,
        "p2", kefir_ast_type_array(&kft_mem, context->type_bundle,
            type1, kefir_ast_constant_expression_integer(&kft_mem, 2), NULL),
            &(kefir_ast_scoped_identifier_storage_t){KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_UNKNOWN}));
    ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle, func_type2,
        NULL, kefir_ast_type_pointer(&kft_mem, context->type_bundle,
...
DEFINE_CASE(ast_node_analysis_declarations12, "AST node analysis - declarations #12")
    ...
    struct kefir_ast_declaration *param1 = kefir_ast_new_declaration(&kft_mem,
        kefir_ast_declarator_array(&kft_mem, KEFIR_AST_DECLARATOR_ARRAY_BOUNDED,
            ...
            kefir_ast_declarator_identifier(&kft_mem, context->symbols, "array")), NULL);
    ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &param1->specifiers,
        kefir_ast_type_qualifier_const(&kft_mem)));
    ASSERT_OK(kefir_ast_declarator_specifier_list_append(&kft_mem, &param1->specifiers,
        kefir_ast_type_specifier_int(&kft_mem)));
    ...
    struct kefir_ast_declarator *decl1 = kefir_ast_declarator_function(&kft_mem,
        kefir_ast_declarator_identifier(&kft_mem, context->symbols, "array_processor"));
    ASSERT_OK(kefir_list_insert_after(&kft_mem, &decl1->function.parameters, kefir_list_tail(&decl1->function.parameters),
        param1));
    ...
    struct kefir_ast_function_type *func_type1 = NULL;
    const struct kefir_ast_type *type1 = kefir_ast_type_function(&kft_mem, context->type_bundle,
        kefir_ast_type_pointer(&kft_mem, context->type_bundle, kefir_ast_type_void()), "array_processor", &func_type1);
    ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle,
        func_type1, "array", kefir_ast_type_array_static(&kft_mem, context->type_bundle,
            kefir_ast_type_qualified(&kft_mem, context->type_bundle,
                kefir_ast_type_signed_int(), (struct kefir_ast_type_qualification){
                    .constant = true
                }),
...
DEFINE_CASE(ast_node_analysis_declarations13, "AST node analysis - declarations #13")
    ...
    struct kefir_ast_declarator *decl1 = kefir_ast_declarator_function(&kft_mem,
        kefir_ast_declarator_identifier(&kft_mem, context->symbols, "array_processor2"));
    ASSERT_OK(kefir_list_insert_after(&kft_mem, &decl1->function.parameters, kefir_list_tail(&decl1->function.parameters),
        param1));
    ASSERT_OK(kefir_list_insert_after(&kft_mem, &decl1->function.parameters, kefir_list_tail(&decl1->function.parameters),
        param2));
    ...
    struct kefir_ast_function_type *func_type1 = NULL;
    const struct kefir_ast_type *type1 = kefir_ast_type_function(&kft_mem, context->type_bundle,
        kefir_ast_type_pointer(&kft_mem, context->type_bundle, kefir_ast_type_float()), "array_processor2", &func_type1);
    ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle,
        func_type1, "length", kefir_ast_type_signed_long(),
        &(kefir_ast_scoped_identifier_storage_t){KEFIR_AST_SCOPE_IDENTIFIER_STORAGE_UNKNOWN}));
    ASSERT_OK(kefir_ast_type_function_parameter(&kft_mem, context->type_bundle,
        func_type1, "array", kefir_ast_type_vlen_array_static(&kft_mem, context->type_bundle,
            kefir_ast_type_qualified(&kft_mem, context->type_bundle,
                kefir_ast_type_float(), (struct kefir_ast_type_qualification){
                    .volatile_type = true
                }),
            KEFIR_AST_NODE_BASE(kefir_ast_new_identifier(&kft_mem, context->symbols, "length")),
            &(struct kefir_ast_type_qualification){
