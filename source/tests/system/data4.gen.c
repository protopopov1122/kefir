...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    struct kefir_ir_function *trunc = kefir_ir_module_new_function(mem, &module, trunc_decl->identifier, 1024);
    REQUIRE(trunc != NULL, KEFIR_INTERNAL_ERROR);
    REQUIRE_OK(kefir_ir_module_declare_global(mem, &module, trunc_decl->identifier));
    kefir_ir_type_append_v(trunc->declaration->params, KEFIR_IR_TYPE_WORD, 0, 0);
    kefir_ir_type_append_v(trunc->declaration->params, KEFIR_IR_TYPE_LONG, 0, 0);

    kefir_irblock_append(&trunc->body, KEFIR_IROPCODE_PICK, 0);                 // 0: [S*, V, V]
    kefir_irblock_append(&trunc->body, KEFIR_IROPCODE_PICK, 2);                 // 0: [S*, V, V, S*]
    kefir_irblock_append2(&trunc->body, KEFIR_IROPCODE_OFFSETPTR, type1_id, 1); // 0: [S*, V, V, I8*]
    kefir_irblock_append(&trunc->body, KEFIR_IROPCODE_STORE8, 0);               // 0: [S*, V]
    kefir_irblock_append(&trunc->body, KEFIR_IROPCODE_PICK, 0);                 // 0: [S*, V, V]
    kefir_irblock_append(&trunc->body, KEFIR_IROPCODE_PICK, 2);                 // 0: [S*, V, V, S*]
    kefir_irblock_append2(&trunc->body, KEFIR_IROPCODE_OFFSETPTR, type1_id, 2); // 0: [S*, V, V, I16*]
    kefir_irblock_append(&trunc->body, KEFIR_IROPCODE_STORE16, 0);              // 0: [S*, V]
    kefir_irblock_append(&trunc->body, KEFIR_IROPCODE_PICK, 0);                 // 0: [S*, V, V]
    kefir_irblock_append(&trunc->body, KEFIR_IROPCODE_PICK, 2);                 // 0: [S*, V, V, S*]
    kefir_irblock_append2(&trunc->body, KEFIR_IROPCODE_OFFSETPTR, type1_id, 3); // 0: [S*, V, V, I32*]
    kefir_irblock_append(&trunc->body, KEFIR_IROPCODE_STORE32, 0);              // 0: [S*, V]
    kefir_irblock_append(&trunc->body, KEFIR_IROPCODE_PICK, 0);                 // 0: [S*, V, V]
    kefir_irblock_append(&trunc->body, KEFIR_IROPCODE_PICK, 2);                 // 0: [S*, V, V, S*]
    kefir_irblock_append2(&trunc->body, KEFIR_IROPCODE_OFFSETPTR, type1_id, 4); // 0: [S*, V, V, I64*]
    kefir_irblock_append(&trunc->body, KEFIR_IROPCODE_STORE64, 0);              // 0: [S*, V]
