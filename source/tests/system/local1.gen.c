...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    kefir_irbuilder_block_appendi64(mem, &fill->body, KEFIR_IROPCODE_STORE8, 0);                    // 4:  [S*, VAL]
    kefir_irbuilder_block_appendi64(mem, &fill->body, KEFIR_IROPCODE_PICK, 0);                      // 5:  [S*, VAL, VAL]
    kefir_irbuilder_block_appendi64(mem, &fill->body, KEFIR_IROPCODE_PICK, 2);                      // 6:  [S*, VAL, VAL, S*]
    kefir_irbuilder_block_appendu32(mem, &fill->body, KEFIR_IROPCODE_OFFSETPTR, type1_id, 2);      // 7:  [S*, VAL, VAL, U16*]
    kefir_irbuilder_block_appendi64(mem, &fill->body, KEFIR_IROPCODE_STORE16, 0);                   // 8:  [S*, VAL]
    kefir_irbuilder_block_appendi64(mem, &fill->body, KEFIR_IROPCODE_PICK, 0);                      // 9:  [S*, VAL, VAL]
    kefir_irbuilder_block_appendi64(mem, &fill->body, KEFIR_IROPCODE_PICK, 2);                      // 10: [S*, VAL, VAL, S*]
    kefir_irbuilder_block_appendu32(mem, &fill->body, KEFIR_IROPCODE_OFFSETPTR, type1_id, 3);      // 11: [S*, VAL, VAL, U32*]
    kefir_irbuilder_block_appendi64(mem, &fill->body, KEFIR_IROPCODE_STORE32, 0);                   // 12: [S*, VAL]
    kefir_irbuilder_block_appendi64(mem, &fill->body, KEFIR_IROPCODE_PICK, 1);                      // 13: [S*, VAL, S*]
    kefir_irbuilder_block_appendu32(mem, &fill->body, KEFIR_IROPCODE_OFFSETPTR, type1_id, 4);      // 14: [S*, VAL, U64*]
    kefir_irbuilder_block_appendi64(mem, &fill->body, KEFIR_IROPCODE_STORE64, 0);                   // 15: [S*]
