...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    kefir_irbuilder_block_appendu32(mem, &sum->body, KEFIR_IROPCODE_OFFSETPTR, type1_id, 2);  // 2: [S*, 0, B50*]
    kefir_irbuilder_block_appendi64(mem, &sum->body, KEFIR_IROPCODE_LOAD56U, 0);              // 3: [S*, 0, B50]
    kefir_irbuilder_block_appendi64(mem, &sum->body, KEFIR_IROPCODE_IADD, 0);                 // 4: [S*, B50]
    kefir_irbuilder_block_appendu32(mem, &sum->body, KEFIR_IROPCODE_EXTSBITS, 0, 50);         // 5: [S*, V1]
    kefir_irbuilder_block_appendi64(mem, &sum->body, KEFIR_IROPCODE_PICK, 1);                 // 6: [S*, V1, S*]
    kefir_irbuilder_block_appendu32(mem, &sum->body, KEFIR_IROPCODE_OFFSETPTR, type1_id, 4);  // 7: [S*, V1, B44*]
    kefir_irbuilder_block_appendi64(mem, &sum->body, KEFIR_IROPCODE_LOAD48U, 0);              // 8: [S*, V1, B44]
    kefir_irbuilder_block_appendu32(mem, &sum->body, KEFIR_IROPCODE_EXTSBITS, 0, 44);         // 9: [S*, V1, V2]
