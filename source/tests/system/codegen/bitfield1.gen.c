...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    kefir_irbuilder_block_appendu32(mem, &sum->body, KEFIR_IROPCODE_OFFSETPTR, type1_id, 2);  // 2: [S*, 0, B24*]
    kefir_irbuilder_block_appendi64(mem, &sum->body, KEFIR_IROPCODE_LOAD24U, 0);                // 3: [S*, 0, B24]
    kefir_irbuilder_block_appendi64(mem, &sum->body, KEFIR_IROPCODE_IADD, 0);                 // 4: [S*, B24]
    kefir_irbuilder_block_appendu32(mem, &sum->body, KEFIR_IROPCODE_EXTSBITS, 0, 24);         // 5: [S*, V1]
    kefir_irbuilder_block_appendi64(mem, &sum->body, KEFIR_IROPCODE_PICK, 1);                 // 6: [S*, V1, S*]
    kefir_irbuilder_block_appendu32(mem, &sum->body, KEFIR_IROPCODE_OFFSETPTR, type1_id, 4);  // 7: [S*, V1, B20*]
    kefir_irbuilder_block_appendi64(mem, &sum->body, KEFIR_IROPCODE_LOAD32U, 0);              // 8: [S*, V1, B20]
    kefir_irbuilder_block_appendu32(mem, &sum->body, KEFIR_IROPCODE_EXTSBITS, 0, 20);         // 9: [S*, V1, V2]
