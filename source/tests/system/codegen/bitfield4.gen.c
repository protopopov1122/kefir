...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    REQUIRE_OK(kefir_irbuilder_type_append_v(mem, type1, KEFIR_IR_TYPE_BITS, 0, KEFIR_IR_BITS_PARAM(KEFIR_IR_TYPE_INT32, 24, 0)));
    REQUIRE_OK(kefir_irbuilder_type_append_v(mem, type1, KEFIR_IR_TYPE_INT8, 0, 0));
    REQUIRE_OK(kefir_irbuilder_type_append_v(mem, type1, KEFIR_IR_TYPE_BITS, 0, KEFIR_IR_BITS_PARAM(KEFIR_IR_TYPE_INT32, 20, 0)));
    REQUIRE_OK(kefir_irbuilder_type_append_v(mem, type1, KEFIR_IR_TYPE_INT8, 0, 0));
    ...
    kefir_irbuilder_block_appendu32(mem, &assign->body, KEFIR_IROPCODE_OFFSETPTR, type1_id, 2);   // 2: [V, S*, B24*]
    kefir_irbuilder_block_appendi64(mem, &assign->body, KEFIR_IROPCODE_PICK, 2);                  // 3: [V, S*, B24*, V]
    kefir_irbuilder_block_appendi64(mem, &assign->body, KEFIR_IROPCODE_STORE24, 0);               // 4: [V, S*]
    kefir_irbuilder_block_appendi64(mem, &assign->body, KEFIR_IROPCODE_PICK, 0);                  // 5: [V, S*, S*]
    kefir_irbuilder_block_appendu32(mem, &assign->body, KEFIR_IROPCODE_OFFSETPTR, type1_id, 4);   // 6: [V, S*, B20*]
    kefir_irbuilder_block_appendi64(mem, &assign->body, KEFIR_IROPCODE_PICK, 2);                  // 7: [V, S*, B24*, V]
    kefir_irbuilder_block_appendi64(mem, &assign->body, KEFIR_IROPCODE_IADD1, 1);                 // 8: [V, S*, B24*, V+1]
    kefir_irbuilder_block_appendi64(mem, &assign->body, KEFIR_IROPCODE_STORE24, 0);               // 9: [V, S*]
