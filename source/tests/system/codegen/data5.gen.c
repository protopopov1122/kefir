...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    struct kefir_ir_data *int8data1 =
        kefir_ir_module_new_named_data(mem, &module, "int8_1", KEFIR_IR_DATA_THREAD_LOCAL_STORAGE, int8type_id);
    REQUIRE_OK(kefir_ir_data_set_integer(int8data1, 0, 104));
    REQUIRE_OK(kefir_ir_data_finalize(int8data1));
    REQUIRE_OK(kefir_ir_module_declare_global(mem, &module, "int8_1", KEFIR_IR_IDENTIFIER_THREAD_LOCAL));
    REQUIRE_OK(kefir_ir_data_finalize(
        kefir_ir_module_new_named_data(mem, &module, "int8_2", KEFIR_IR_DATA_THREAD_LOCAL_STORAGE, int8type_id)));
    REQUIRE_OK(kefir_ir_module_declare_global(mem, &module, "int8_2", KEFIR_IR_IDENTIFIER_THREAD_LOCAL));
    ...
    struct kefir_ir_data *int16data1 =
        kefir_ir_module_new_named_data(mem, &module, "int16_1", KEFIR_IR_DATA_THREAD_LOCAL_STORAGE, int16type_id);
    REQUIRE_OK(kefir_ir_data_set_integer(int16data1, 0, 2589));
    REQUIRE_OK(kefir_ir_data_finalize(int16data1));
    REQUIRE_OK(kefir_ir_module_declare_global(mem, &module, "int16_1", KEFIR_IR_IDENTIFIER_THREAD_LOCAL));
    REQUIRE_OK(kefir_ir_data_finalize(
        kefir_ir_module_new_named_data(mem, &module, "int16_2", KEFIR_IR_DATA_THREAD_LOCAL_STORAGE, int16type_id)));
    REQUIRE_OK(kefir_ir_module_declare_global(mem, &module, "int16_2", KEFIR_IR_IDENTIFIER_THREAD_LOCAL));
    ...
    struct kefir_ir_data *int32data1 =
        kefir_ir_module_new_named_data(mem, &module, "int32_1", KEFIR_IR_DATA_THREAD_LOCAL_STORAGE, int32type_id);
    REQUIRE_OK(kefir_ir_data_set_integer(int32data1, 0, 0x1ffff));
    REQUIRE_OK(kefir_ir_data_finalize(int32data1));
    REQUIRE_OK(kefir_ir_module_declare_global(mem, &module, "int32_1", KEFIR_IR_IDENTIFIER_THREAD_LOCAL));
    REQUIRE_OK(kefir_ir_data_finalize(
        kefir_ir_module_new_named_data(mem, &module, "int32_2", KEFIR_IR_DATA_THREAD_LOCAL_STORAGE, int32type_id)));
    REQUIRE_OK(kefir_ir_module_declare_global(mem, &module, "int32_2", KEFIR_IR_IDENTIFIER_THREAD_LOCAL));
    ...
    struct kefir_ir_data *int64data1 =
        kefir_ir_module_new_named_data(mem, &module, "int64_1", KEFIR_IR_DATA_THREAD_LOCAL_STORAGE, int64type_id);
    REQUIRE_OK(kefir_ir_data_set_integer(int64data1, 0, 0x1ffffffff));
    REQUIRE_OK(kefir_ir_data_finalize(int64data1));
    REQUIRE_OK(kefir_ir_module_declare_global(mem, &module, "int64_1", KEFIR_IR_IDENTIFIER_THREAD_LOCAL));
    REQUIRE_OK(kefir_ir_data_finalize(
        kefir_ir_module_new_named_data(mem, &module, "int64_2", KEFIR_IR_DATA_THREAD_LOCAL_STORAGE, int64type_id)));
    REQUIRE_OK(kefir_ir_module_declare_global(mem, &module, "int64_2", KEFIR_IR_IDENTIFIER_THREAD_LOCAL));
    ...
    struct kefir_ir_data *float32data1 =
        kefir_ir_module_new_named_data(mem, &module, "float32_1", KEFIR_IR_DATA_THREAD_LOCAL_STORAGE, float32type_id);
    REQUIRE_OK(kefir_ir_data_set_float32(float32data1, 0, 3.14159));
    REQUIRE_OK(kefir_ir_data_finalize(float32data1));
    REQUIRE_OK(kefir_ir_module_declare_global(mem, &module, "float32_1", KEFIR_IR_IDENTIFIER_THREAD_LOCAL));
    REQUIRE_OK(kefir_ir_data_finalize(
        kefir_ir_module_new_named_data(mem, &module, "float32_2", KEFIR_IR_DATA_THREAD_LOCAL_STORAGE, float32type_id)));
    REQUIRE_OK(kefir_ir_module_declare_global(mem, &module, "float32_2", KEFIR_IR_IDENTIFIER_THREAD_LOCAL));
    ...
    struct kefir_ir_data *float64data1 =
        kefir_ir_module_new_named_data(mem, &module, "float64_1", KEFIR_IR_DATA_THREAD_LOCAL_STORAGE, float64type_id);
    REQUIRE_OK(kefir_ir_data_set_float64(float64data1, 0, 2.718281828));
    REQUIRE_OK(kefir_ir_data_finalize(float64data1));
    REQUIRE_OK(kefir_ir_module_declare_global(mem, &module, "float64_1", KEFIR_IR_IDENTIFIER_THREAD_LOCAL));
    REQUIRE_OK(kefir_ir_data_finalize(
        kefir_ir_module_new_named_data(mem, &module, "float64_2", KEFIR_IR_DATA_THREAD_LOCAL_STORAGE, float64type_id)));
    REQUIRE_OK(kefir_ir_module_declare_global(mem, &module, "float64_2", KEFIR_IR_IDENTIFIER_THREAD_LOCAL));
    ...
    REQUIRE_OK(kefir_ir_module_declare_global(mem, &module, "long_double_1", KEFIR_IR_IDENTIFIER_THREAD_LOCAL));
    REQUIRE_OK(kefir_ir_data_finalize(kefir_ir_module_new_named_data(
        mem, &module, "long_double_2", KEFIR_IR_DATA_THREAD_LOCAL_STORAGE, long_double_type_id)));
    REQUIRE_OK(kefir_ir_module_declare_global(mem, &module, "long_double_2", KEFIR_IR_IDENTIFIER_THREAD_LOCAL));
    ...
    struct kefir_ir_data *struct1_data1 =
        kefir_ir_module_new_named_data(mem, &module, "struct1_1", KEFIR_IR_DATA_THREAD_LOCAL_STORAGE, struct1_type_id);
    ...
    REQUIRE_OK(kefir_ir_module_declare_global(mem, &module, "struct1_1", KEFIR_IR_IDENTIFIER_THREAD_LOCAL));
    ...
    struct kefir_ir_data *array1_data =
        kefir_ir_module_new_named_data(mem, &module, "array1_1", KEFIR_IR_DATA_THREAD_LOCAL_STORAGE, array1_type_id);
    ...
    REQUIRE_OK(kefir_ir_module_declare_global(mem, &module, "array1_1", KEFIR_IR_IDENTIFIER_THREAD_LOCAL));
    ...
    struct kefir_ir_data *union1_data =
        kefir_ir_module_new_named_data(mem, &module, "union1_1", KEFIR_IR_DATA_THREAD_LOCAL_STORAGE, uniont1_type_id);
    ...
    struct kefir_ir_data *pad1_data =
        kefir_ir_module_new_named_data(mem, &module, "pad1_1", KEFIR_IR_DATA_THREAD_LOCAL_STORAGE, pad1_type_id);
    REQUIRE_OK(kefir_ir_data_set_raw(pad1_data, 0, MSG, strlen(MSG) + 1));
    REQUIRE_OK(kefir_ir_data_finalize(pad1_data));
    REQUIRE_OK(kefir_ir_module_declare_global(mem, &module, "pad1_1", KEFIR_IR_IDENTIFIER_THREAD_LOCAL));
    ...
    struct kefir_ir_data *strpointer1_data = kefir_ir_module_new_named_data(
        mem, &module, "strpointer1_1", KEFIR_IR_DATA_THREAD_LOCAL_STORAGE, strpointer1_type_id);
