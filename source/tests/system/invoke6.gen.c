...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    struct kefir_ir_function_decl *proxyadd_decl =
        kefir_ir_module_new_function_declaration(mem, &module, "proxyadd", proxyadd_decl_params, proxyadd_decl_result);
    ...
    struct kefir_ir_function_decl *addstruct_decl =
        kefir_ir_module_new_function_declaration(mem, &module,
            kefir_ir_module_symbol(mem, &module, "addstruct", &addstruct_id), addstruct_decl_params, addstruct_decl_result);
    REQUIRE(addstruct_decl != NULL, KEFIR_INTERNAL_ERROR);

    kefir_codegen_amd64_sysv_init(&codegen, stdout, mem);
    codegen.asmgen.settings.enable_comments = false;

    REQUIRE_OK(kefir_ir_type_append_v(proxyadd_decl_params, KEFIR_IR_TYPE_WORD, 0, 0));
    REQUIRE_OK(kefir_ir_type_append_v(proxyadd_decl_params, KEFIR_IR_TYPE_STRUCT, 0, 2));
    REQUIRE_OK(kefir_ir_type_append_v(proxyadd_decl_params, KEFIR_IR_TYPE_INT64, 0, 0));
    REQUIRE_OK(kefir_ir_type_append_v(proxyadd_decl_params, KEFIR_IR_TYPE_FLOAT64, 0, 0));
    REQUIRE_OK(kefir_ir_type_append_v(proxyadd_decl_result, KEFIR_IR_TYPE_STRUCT, 0, 2));
    REQUIRE_OK(kefir_ir_type_append_v(proxyadd_decl_result, KEFIR_IR_TYPE_INT64, 0, 0));
    REQUIRE_OK(kefir_ir_type_append_v(proxyadd_decl_result, KEFIR_IR_TYPE_FLOAT64, 0, 0));
    kefir_irblock_append(&proxyadd->body, KEFIR_IROPCODE_PUSH, 7);
    kefir_irblock_append(&proxyadd->body, KEFIR_IROPCODE_XCHG, 1);
    kefir_irblock_append(&proxyadd->body, KEFIR_IROPCODE_INVOKEV, addstruct_id);

    REQUIRE_OK(kefir_ir_type_append_v(addstruct_decl_params, KEFIR_IR_TYPE_INT64, 0, 0));
    REQUIRE_OK(kefir_ir_type_append_v(addstruct_decl_params, KEFIR_IR_TYPE_STRUCT, 0, 2));
    REQUIRE_OK(kefir_ir_type_append_v(addstruct_decl_params, KEFIR_IR_TYPE_INT64, 0, 0));
    REQUIRE_OK(kefir_ir_type_append_v(addstruct_decl_params, KEFIR_IR_TYPE_FLOAT64, 0, 0));
    REQUIRE_OK(kefir_ir_type_append_v(addstruct_decl_result, KEFIR_IR_TYPE_STRUCT, 0, 2));
    REQUIRE_OK(kefir_ir_type_append_v(addstruct_decl_result, KEFIR_IR_TYPE_INT64, 0, 0));
    REQUIRE_OK(kefir_ir_type_append_v(addstruct_decl_result, KEFIR_IR_TYPE_FLOAT64, 0, 0));
