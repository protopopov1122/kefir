...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    REQUIRE_OK(kefir_ir_type_append_v(aggtype, KEFIR_IR_TYPE_STRUCT, 0, 1));
    REQUIRE_OK(kefir_ir_type_append_v(aggtype, KEFIR_IR_TYPE_ARRAY, 0, 4));
    REQUIRE_OK(kefir_ir_type_append_v(aggtype, KEFIR_IR_TYPE_LONG, 0, 0));
    REQUIRE_OK(kefir_ir_type_append_v(getarg_decl_result, KEFIR_IR_TYPE_STRUCT, 0, 1));
    REQUIRE_OK(kefir_ir_type_append_v(getarg_decl_result, KEFIR_IR_TYPE_ARRAY, 0, 4));
    ...
    kefir_irblock_append2(&getarg->body, KEFIR_IROPCODE_VARARG_GET, aggtype_id, 0);      // 9: [C, A]
    ...
    kefir_irblock_append2(&getarg->body, KEFIR_IROPCODE_VARARG_GET, aggtype_id, 0);      // 16: [0, V*, A]
