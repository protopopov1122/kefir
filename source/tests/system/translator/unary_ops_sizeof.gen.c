...
static kefir_result_t define_get_sizeof_function(struct kefir_mem *mem, struct function *func,
    ...
    func->type =
        kefir_ast_type_function(mem, context_manager->current->type_bundle, struct_type, "get_sizeof", &func_type);
...
static kefir_result_t generate_ir(struct kefir_mem *mem, struct kefir_ir_module *module,
    ...
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry1, kefir_ast_declarator_identifier(mem, context_manager.current->symbols, "tstring"), NULL));
