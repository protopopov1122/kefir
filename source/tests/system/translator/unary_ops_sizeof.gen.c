...
static kefir_result_t define_get_sizeof_function(struct kefir_mem *mem,
                                           struct function *func,
                                           struct kefir_ast_context_manager *context_manager,
                                           struct kefir_ast_declarator_specifier *specifier) {
    REQUIRE_OK(kefir_list_init(&func->args));

    struct kefir_ast_type_name *type_name16 = kefir_ast_new_type_name(mem, kefir_ast_declarator_identifier(mem, context_manager->current->symbols, NULL));
    ...
    func->type = kefir_ast_type_function(mem, context_manager->current->type_bundle,
        struct_type, "get_sizeof", &func_type);
    ...
    REQUIRE_OK(kefir_ast_initializer_list_append(mem, &compound_literal->initializer->list, \
        NULL, kefir_ast_new_expression_initializer(mem, KEFIR_AST_NODE_BASE( \
            kefir_ast_new_unary_operation(mem, KEFIR_AST_OPERATION_SIZEOF, \
...
#undef MAKE_TYPENAME
    ...
    struct kefir_ast_type_name *type_name15 = kefir_ast_new_type_name(mem,
        kefir_ast_declarator_pointer(mem, kefir_ast_declarator_identifier(mem, NULL, NULL)));
    REQUIRE_OK(append_specifiers(mem, &type_name15->type_decl.specifiers, 1,
        kefir_ast_type_specifier_void(mem)));

    struct kefir_ast_type_name *type_name17 = kefir_ast_new_type_name(mem,
        kefir_ast_declarator_array(mem, KEFIR_AST_DECLARATOR_ARRAY_BOUNDED,
            KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(mem, 128)),
            kefir_ast_declarator_identifier(mem, NULL, NULL)));
    REQUIRE_OK(append_specifiers(mem, &type_name17->type_decl.specifiers, 1,
...
static kefir_result_t generate_ir(struct kefir_mem *mem, struct kefir_ir_module *module, struct kefir_ir_target_platform *ir_platform) {
    ...
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(mem, entry1,
        kefir_ast_declarator_identifier(mem, context_manager.current->symbols, "tboolean"), NULL));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(mem, entry1,
        kefir_ast_declarator_identifier(mem, context_manager.current->symbols, "tchar"), NULL));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(mem, entry1,
        kefir_ast_declarator_identifier(mem, context_manager.current->symbols, "tuchar"), NULL));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(mem, entry1,
        kefir_ast_declarator_identifier(mem, context_manager.current->symbols, "tschar"), NULL));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(mem, entry1,
        kefir_ast_declarator_identifier(mem, context_manager.current->symbols, "tushort"), NULL));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(mem, entry1,
        kefir_ast_declarator_identifier(mem, context_manager.current->symbols, "tsshort"), NULL));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(mem, entry1,
        kefir_ast_declarator_identifier(mem, context_manager.current->symbols, "tuint"), NULL));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(mem, entry1,
        kefir_ast_declarator_identifier(mem, context_manager.current->symbols, "tsint"), NULL));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(mem, entry1,
        kefir_ast_declarator_identifier(mem, context_manager.current->symbols, "tulong"), NULL));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(mem, entry1,
        kefir_ast_declarator_identifier(mem, context_manager.current->symbols, "tslong"), NULL));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(mem, entry1,
        kefir_ast_declarator_identifier(mem, context_manager.current->symbols, "tullong"), NULL));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(mem, entry1,
        kefir_ast_declarator_identifier(mem, context_manager.current->symbols, "tsllong"), NULL));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(mem, entry1,
        kefir_ast_declarator_identifier(mem, context_manager.current->symbols, "tfloat"), NULL));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(mem, entry1,
        kefir_ast_declarator_identifier(mem, context_manager.current->symbols, "tdouble"), NULL));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(mem, entry1,
        kefir_ast_declarator_identifier(mem, context_manager.current->symbols, "tpvoid"), NULL));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(mem, entry1,
        kefir_ast_declarator_identifier(mem, context_manager.current->symbols, "tstruct"), NULL));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(mem, entry1,
        kefir_ast_declarator_identifier(mem, context_manager.current->symbols, "tarray"), NULL));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(mem, entry1,
        kefir_ast_declarator_identifier(mem, context_manager.current->symbols, "tstring"), NULL));
    REQUIRE_OK(kefir_ast_structure_specifier_append_entry(mem, specifier1, entry1));

    struct function get_sizeof;
    REQUIRE_OK(define_get_sizeof_function(mem, &get_sizeof, &context_manager,
        kefir_ast_type_specifier_struct(mem, specifier1)));

    REQUIRE_OK(analyze_function(mem, &get_sizeof, &context_manager));
    ...
    REQUIRE_OK(translate_function(mem, &get_sizeof, &context_manager, &global_scope, &translator_context));

    REQUIRE_OK(kefir_ast_translate_global_scope(mem, module, &global_scope));
    
    REQUIRE_OK(free_function(mem, &get_sizeof));
