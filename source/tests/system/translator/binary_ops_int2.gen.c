...
static kefir_result_t define_##_id##_function(struct kefir_mem *mem, \
 ...
 \
    ...
    func->type = kefir_ast_type_function(mem, context_manager->current->type_bundle, \
        kefir_ast_type_unsigned_int(), (_name), &func_type); \
 ...
 \
    REQUIRE_OK(kefir_ast_local_context_define_auto(mem, context_manager->local, "x", kefir_ast_type_unsigned_int(), \
        NULL, NULL, NULL)); \
    REQUIRE_OK(kefir_ast_local_context_define_auto(mem, context_manager->local, "y", kefir_ast_type_unsigned_int(), \
        NULL, NULL, NULL)); \
 \
...
DEFFUN(sum, "sum", KEFIR_AST_OPERATION_ADD)
DEFFUN(sub, "sub", KEFIR_AST_OPERATION_SUBTRACT)
DEFFUN(mul, "mul", KEFIR_AST_OPERATION_MULTIPLY)
DEFFUN(div, "divide", KEFIR_AST_OPERATION_DIVIDE)
DEFFUN(modulo, "modulo", KEFIR_AST_OPERATION_MODULO)
DEFFUN(shl, "shl", KEFIR_AST_OPERATION_SHIFT_LEFT)
DEFFUN(shr, "shr", KEFIR_AST_OPERATION_SHIFT_RIGHT)
DEFFUN(band, "band", KEFIR_AST_OPERATION_BITWISE_AND)
DEFFUN(bor, "bor", KEFIR_AST_OPERATION_BITWISE_OR)
DEFFUN(bxor, "bxor", KEFIR_AST_OPERATION_BITWISE_XOR)
DEFFUN(less, "less", KEFIR_AST_OPERATION_LESS)
DEFFUN(lesseq, "lesseq", KEFIR_AST_OPERATION_LESS_EQUAL)
DEFFUN(greater, "greater", KEFIR_AST_OPERATION_GREATER)
DEFFUN(greatereq, "greatereq", KEFIR_AST_OPERATION_GREATER_EQUAL)
DEFFUN(equals, "equals", KEFIR_AST_OPERATION_EQUAL)
DEFFUN(noteq, "noteq", KEFIR_AST_OPERATION_NOT_EQUAL)
...
static kefir_result_t generate_ir(struct kefir_mem *mem, struct kefir_ir_module *module, struct kefir_ir_target_platform *ir_platform) {
    ...
    struct function sum_func, sub_func, mul_func, div_func, mod_func,
                    ...
                    less_func, lesseq_func, greater_func, greatereq_func,
