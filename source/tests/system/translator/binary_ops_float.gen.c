...
static kefir_result_t define_##_id##_function(struct kefir_mem *mem, \
 ...
 \
    REQUIRE_OK(kefir_ast_local_context_define_auto(mem, context_manager->local, "x", kefir_ast_type_float(), \
        NULL, NULL)); \
    REQUIRE_OK(kefir_ast_local_context_define_auto(mem, context_manager->local, "y", kefir_ast_type_float(), \
        NULL, NULL)); \
 \
...
DEFFUN(sum, "sum", KEFIR_AST_OPERATION_ADD, kefir_ast_type_float())
DEFFUN(sub, "sub", KEFIR_AST_OPERATION_SUBTRACT, kefir_ast_type_float())
DEFFUN(mul, "mul", KEFIR_AST_OPERATION_MULTIPLY, kefir_ast_type_float())
DEFFUN(div, "divide", KEFIR_AST_OPERATION_DIVIDE, kefir_ast_type_float())
...
static kefir_result_t generate_ir(struct kefir_mem *mem, struct kefir_ir_module *module, struct kefir_ir_target_platform *ir_platform) {
    ...
    struct function sum_func, sub_func, mul_func, div_func,
