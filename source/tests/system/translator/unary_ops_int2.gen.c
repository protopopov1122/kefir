...
DEFFUN(plus, "plus", KEFIR_AST_OPERATION_PLUS)
DEFFUN(negate, "negate", KEFIR_AST_OPERATION_NEGATE)
DEFFUN(invert, "invert", KEFIR_AST_OPERATION_INVERT)
DEFFUN(logical_negate, "logical_negate", KEFIR_AST_OPERATION_LOGICAL_NEGATE)
...
static kefir_result_t generate_ir(struct kefir_mem *mem, struct kefir_ir_module *module, struct kefir_ir_target_platform *ir_platform) {
    ...
    struct function plus_func, negate_func, invert_func, logical_negate_func;
    ...
    REQUIRE_OK(define_invert_function(mem, &invert_func, &context_manager));
    ...
    REQUIRE_OK(analyze_function(mem, &invert_func, &context_manager));
    ...
    REQUIRE_OK(translate_function(mem, &invert_func, &context_manager, &global_scope, &translator_context));
    ...
    REQUIRE_OK(free_function(mem, &invert_func));
