...
static kefir_result_t generate_ir(struct kefir_mem *mem, struct kefir_ir_module *module,
    ...
    REQUIRE_OK(kefir_ast_struct_type_field(mem, context_manager.current->symbols, struct_type1, "arg1",
                                           kefir_ast_type_char(), NULL));
    REQUIRE_OK(kefir_ast_struct_type_field(mem, context_manager.current->symbols, struct_type1, "arg2",
                                           kefir_ast_type_char(), NULL));
    REQUIRE_OK(kefir_ast_struct_type_field(mem, context_manager.current->symbols, struct_type1, "result",
                                           kefir_ast_type_signed_int(), NULL));

    struct function char_sum;
    REQUIRE_OK(define_sum_function(mem, &char_sum, &context_manager, "char_sum",
                                   kefir_ast_type_pointer(mem, context_manager.current->type_bundle, type1)));

    REQUIRE_OK(analyze_function(mem, &char_sum, &context_manager));
    ...
    REQUIRE_OK(translate_function(mem, &char_sum, &context_manager, &global_scope, &translator_context));

    REQUIRE_OK(kefir_ast_translate_global_scope(mem, &global_context.context, module, &global_scope));

    REQUIRE_OK(free_function(mem, &char_sum));
