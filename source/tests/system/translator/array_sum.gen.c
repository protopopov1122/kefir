...
static kefir_result_t define_array_sum_function(struct kefir_mem *mem,
    ...
    func->type = kefir_ast_type_function(mem, context_manager->current->type_bundle,
        kefir_ast_type_float(), "array_sum", &func_type);
    REQUIRE_OK(kefir_ast_type_function_parameter(mem, context_manager->current->type_bundle, func_type,
        NULL, kefir_ast_type_pointer(mem, context_manager->current->type_bundle,
    ...
    struct kefir_ast_function_call *array_sum_cal = kefir_ast_new_function_call(mem,
        KEFIR_AST_NODE_BASE(kefir_ast_new_identifier(mem, context_manager->current->symbols, "array_sum")));
    REQUIRE_OK(kefir_ast_function_call_append(mem, array_sum_cal,
