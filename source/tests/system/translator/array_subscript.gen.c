...
static kefir_result_t define_set_array_function(struct kefir_mem *mem,
    ...
    const struct kefir_ast_type *array_type = kefir_ast_type_unbounded_array(mem, context_manager->current->type_bundle,
        element_type, NULL);
    const struct kefir_ast_type *pointer_type = kefir_ast_type_pointer(mem, context_manager->current->type_bundle,
        element_type);

    struct kefir_ast_function_type *func_type = NULL;
    func->type = kefir_ast_type_function(mem, context_manager->current->type_bundle,
        kefir_ast_type_void(), name, &func_type);
    REQUIRE_OK(kefir_ast_type_function_parameter(mem, context_manager->current->type_bundle, func_type,
        NULL, array_type, NULL));
    REQUIRE_OK(kefir_ast_type_function_parameter(mem, context_manager->current->type_bundle, func_type,
        NULL, kefir_ast_type_unsigned_int(), NULL));
    REQUIRE_OK(kefir_ast_type_function_parameter(mem, context_manager->current->type_bundle, func_type,
        NULL, element_type, NULL));
    ...
    REQUIRE_OK(kefir_ast_local_context_define_auto(mem, context_manager->local, "array",
        pointer_type, NULL, NULL, NULL));
    REQUIRE_OK(kefir_ast_local_context_define_auto(mem, context_manager->local, "index",
    ...
    REQUIRE_OK(kefir_list_insert_after(mem, &func->args, kefir_list_tail(&func->args), KEFIR_AST_NODE_BASE(
        kefir_ast_new_identifier(mem, context_manager->current->symbols, "array"))));
    REQUIRE_OK(kefir_list_insert_after(mem, &func->args, kefir_list_tail(&func->args), KEFIR_AST_NODE_BASE(
        kefir_ast_new_identifier(mem, context_manager->current->symbols, "index"))));
    REQUIRE_OK(kefir_list_insert_after(mem, &func->args, kefir_list_tail(&func->args), KEFIR_AST_NODE_BASE(
        kefir_ast_new_identifier(mem, context_manager->current->symbols, "value"))));

    func->body = KEFIR_AST_NODE_BASE(kefir_ast_new_simple_assignment(mem,
        KEFIR_AST_NODE_BASE(kefir_ast_new_array_subscript(mem,
...
static kefir_result_t define_get_array_function(struct kefir_mem *mem,
                                              struct function *func,
                                              struct kefir_ast_context_manager *context_manager,
                                              const char *name,
                                              const struct kefir_ast_type *element_type) {
    ...
    func->body = KEFIR_AST_NODE_BASE(kefir_ast_new_array_subscript(mem,
        KEFIR_AST_NODE_BASE(kefir_ast_new_identifier(mem, context_manager->current->symbols, "array")),
        KEFIR_AST_NODE_BASE(kefir_ast_new_identifier(mem, context_manager->current->symbols, "index"))));
...
static kefir_result_t generate_ir(struct kefir_mem *mem, struct kefir_ir_module *module, struct kefir_ir_target_platform *ir_platform) {
    ...
    REQUIRE_OK(kefir_ast_struct_type_field(mem, context_manager.current->symbols, struct_type1,
        "array", kefir_ast_type_array(mem, context_manager.current->type_bundle,
            kefir_ast_type_char(), kefir_ast_constant_expression_integer(mem, 32), NULL), NULL));

    struct function set_int, get_int,
