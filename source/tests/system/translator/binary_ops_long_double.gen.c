...
#define DEFFUN(_id, _name, _oper, _ret)                                                                                \
    static kefir_result_t define_##_id##_function(struct kefir_mem *mem, struct function *func,                        \
        ...
        REQUIRE_OK(kefir_ast_local_context_define_auto(mem, context_manager->local, "x", kefir_ast_type_long_double(), \
                                                       NULL, NULL, NULL, NULL));                                       \
        REQUIRE_OK(kefir_ast_local_context_define_auto(mem, context_manager->local, "y", kefir_ast_type_long_double(), \
                                                       NULL, NULL, NULL, NULL));                                       \
                                                                                                                       \
...
DEFFUN(sum, "sum", KEFIR_AST_OPERATION_ADD, kefir_ast_type_long_double())
DEFFUN(sub, "sub", KEFIR_AST_OPERATION_SUBTRACT, kefir_ast_type_long_double())
DEFFUN(mul, "mul", KEFIR_AST_OPERATION_MULTIPLY, kefir_ast_type_long_double())
DEFFUN(div, "divide", KEFIR_AST_OPERATION_DIVIDE, kefir_ast_type_long_double())
