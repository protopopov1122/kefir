...
static kefir_result_t define_assign_function_impl(struct kefir_mem *mem,
    ...
    const struct kefir_ast_type *ptr_type = kefir_ast_type_pointer(mem, context_manager->current->type_bundle,
            param_type);

    struct kefir_ast_function_type *func_type = NULL;
    func->type = kefir_ast_type_function(mem, context_manager->current->type_bundle,
        param_type, name, &func_type);
    ...
    func->body = KEFIR_AST_NODE_BASE(kefir_ast_new_compound_assignment(mem,
        oper,
...
static kefir_result_t generate_ir(struct kefir_mem *mem, struct kefir_ir_module *module, struct kefir_ir_target_platform *ir_platform) {
    ...
    const struct kefir_ast_type *struct_param_type = kefir_ast_type_structure(mem, context_manager.current->type_bundle,
        NULL, &struct_type);
    REQUIRE_OK(kefir_ast_struct_type_field(mem, context_manager.current->symbols, struct_type,
        "x", kefir_ast_type_signed_int(), NULL));
    REQUIRE_OK(kefir_ast_struct_type_field(mem, context_manager.current->symbols, struct_type,
        "y", kefir_ast_type_array(mem, context_manager.current->type_bundle,
            kefir_ast_type_char(), kefir_ast_constant_expression_integer(mem, 16), NULL), NULL));
    REQUIRE_OK(kefir_ast_struct_type_field(mem, context_manager.current->symbols, struct_type,
        "z", kefir_ast_type_pointer(mem, context_manager.current->type_bundle, kefir_ast_type_void()), NULL));

    struct function simple_assign_long, simple_assign_struct,
    ...
    REQUIRE_OK(define_assign_function(mem, &simple_assign_struct, &context_manager, "assign_struct",
        struct_param_type, KEFIR_AST_ASSIGNMENT_SIMPLE));
