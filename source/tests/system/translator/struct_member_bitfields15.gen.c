...
static kefir_result_t define_sum_function(struct kefir_mem *mem, struct function *func,
    ...
    REQUIRE_OK(
        kefir_ast_local_context_define_auto(mem, context_manager->local, "param", param_type, NULL, NULL, NULL, NULL));
    ...
    struct kefir_ast_node_base *add5 = KEFIR_AST_NODE_BASE(kefir_ast_new_binary_operation(
...
static kefir_result_t generate_ir(struct kefir_mem *mem, struct kefir_ir_module *module,
    ...
    REQUIRE_OK(kefir_ast_declarator_specifier_list_append(mem, &entry6->declaration.specifiers,
                                                          kefir_ast_type_specifier_char(mem)));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry6, kefir_ast_declarator_identifier(mem, context_manager.current->symbols, NULL),
    ...
    REQUIRE_OK(kefir_ast_declarator_specifier_list_append(mem, &entry7->declaration.specifiers,
                                                          kefir_ast_type_specifier_long(mem)));
    REQUIRE_OK(kefir_ast_structure_declaration_entry_append(
        mem, entry7, kefir_ast_declarator_identifier(mem, context_manager.current->symbols, "f"),
