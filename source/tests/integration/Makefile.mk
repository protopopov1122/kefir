...
$(BIN_DIR)/tests/integration/%: $(BIN_DIR)/tests/integration/%.o $(GENERATED_SOURCES) $(KEFIR_LIB_OBJECT_FILES) \
                                     $(BIN_DIR)/tests/int_test.o \
									 $(BIN_DIR)/tests/util/util.o
	@mkdir -p $(@D)
	@echo "Linking $@"
	@$(CC) -o $@ $(KEFIR_LIB_OBJECT_FILES) $(BIN_DIR)/tests/int_test.o \
	                             $(BIN_DIR)/tests/util/util.o \
								 $< \
								 $(KEFIR_INTEGRATION_TEST_LINKED_LIBS)
...
rebuild_integration_tests: $(KEFIR_INTEGRATION_TEST_RESULTS)
	@echo $(KEFIR_INTEGRATION_TEST_RESULTS)
