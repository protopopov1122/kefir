...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    REQUIRE_OK(kefir_token_new_identifier(mem, &symbols, "funBOOL", &TOKENS[counter++]));
    ...
    REQUIRE_OK(kefir_token_new_punctuator(KEFIR_PUNCTUATOR_QUESTION_MARK, &TOKENS[counter++]));
    REQUIRE_OK(kefir_token_new_identifier(mem, &symbols, "res1", &TOKENS[counter++]));
    REQUIRE_OK(kefir_token_new_punctuator(KEFIR_PUNCTUATOR_COLON, &TOKENS[counter++]));
    REQUIRE_OK(kefir_token_new_identifier(mem, &symbols, "res2", &TOKENS[counter++]));
    ...
    REQUIRE_OK(kefir_token_new_identifier(mem, &symbols, "condition3", &TOKENS[counter++]));
    REQUIRE_OK(kefir_token_new_punctuator(KEFIR_PUNCTUATOR_QUESTION_MARK, &TOKENS[counter++]));
    ...
    REQUIRE_OK(kefir_token_new_identifier(mem, &symbols, "boolean1", &TOKENS[counter++]));
    REQUIRE_OK(kefir_token_new_punctuator(KEFIR_PUNCTUATOR_QUESTION_MARK, &TOKENS[counter++]));
    REQUIRE_OK(kefir_token_new_punctuator(KEFIR_PUNCTUATOR_COLON, &TOKENS[counter++]));
    REQUIRE_OK(kefir_token_new_identifier(mem, &symbols, "boolean2", &TOKENS[counter++]));
    REQUIRE_OK(kefir_token_new_punctuator(KEFIR_PUNCTUATOR_SEMICOLON, &TOKENS[counter++]));
    ...
    struct kefir_parser_configuration configuration;
    REQUIRE_OK(kefir_parser_configuration_default(&configuration));
    configuration.omitted_conditional_operand = true;
