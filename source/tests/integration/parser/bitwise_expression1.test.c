...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    REQUIRE_OK(kefir_token_new_constant_char('U', &TOKENS[counter++]));
    ...
    REQUIRE_OK(kefir_token_new_identifier(mem, &symbols, "param", &TOKENS[counter++]));
    ...
    REQUIRE_OK(kefir_token_new_punctuator(KEFIR_PUNCTUATOR_AMPERSAND, &TOKENS[counter++]));
    REQUIRE_OK(kefir_token_new_identifier(mem, &symbols, "param2", &TOKENS[counter++]));
    ...
    REQUIRE_OK(kefir_token_new_identifier(mem, &symbols, "value", &TOKENS[counter++]));
    REQUIRE_OK(kefir_token_new_punctuator(KEFIR_PUNCTUATOR_AMPERSAND, &TOKENS[counter++]));
    REQUIRE_OK(kefir_token_new_identifier(mem, &symbols, "bitmask", &TOKENS[counter++]));
    REQUIRE_OK(kefir_token_new_punctuator(KEFIR_PUNCTUATOR_CARET, &TOKENS[counter++]));
    REQUIRE_OK(kefir_token_new_identifier(mem, &symbols, "caret", &TOKENS[counter++]));
    REQUIRE_OK(kefir_token_new_punctuator(KEFIR_PUNCTUATOR_VBAR, &TOKENS[counter++]));
    REQUIRE_OK(kefir_token_new_identifier(mem, &symbols, "finally", &TOKENS[counter++]));
