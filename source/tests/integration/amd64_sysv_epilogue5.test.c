...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    REQUIRE_OK(kefir_ir_function_decl_alloc(mem, "func1", 0, 5, &decl));
    ...
    REQUIRE_OK(kefir_ir_type_append_v(&func.declaration->result, KEFIR_IR_TYPE_ARRAY, 0, 4));
    REQUIRE_OK(kefir_ir_type_append_v(&func.declaration->result, KEFIR_IR_TYPE_FLOAT32, 0, 0));
    REQUIRE_OK(kefir_ir_type_append_v(&func.declaration->result, KEFIR_IR_TYPE_ARRAY, 0, 10));
