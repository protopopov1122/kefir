...
static kefir_result_t format_global_scope(struct kefir_json_output *json,
                                          struct kefir_ast_translator_global_scope_layout *scope) {
    REQUIRE_OK(kefir_json_output_object_key(json, "externals"));
    REQUIRE_OK(kefir_json_output_array_begin(json));
    for (const struct kefir_list_entry *iter = kefir_list_head(&scope->external_objects); iter != NULL;
    ...
    REQUIRE_OK(kefir_json_output_array_begin(json));
    for (const struct kefir_list_entry *iter = kefir_list_head(&scope->external_thread_local_objects); iter != NULL;
         kefir_list_next(&iter)) {
        ASSIGN_DECL_CAST(struct kefir_ast_translator_scoped_identifier_entry *, scoped_identifier, iter->value);
        ASSIGN_DECL_CAST(struct kefir_ast_translator_scoped_identifier_object *, scoped_identifier_payload,
                         scoped_identifier->value->payload.ptr);
    ...
    REQUIRE_OK(kefir_json_output_array_begin(json));
    for (const struct kefir_list_entry *iter = kefir_list_head(&scope->static_objects); iter != NULL;
         kefir_list_next(&iter)) {
        ASSIGN_DECL_CAST(struct kefir_ast_translator_scoped_identifier_entry *, scoped_identifier, iter->value);
        ASSIGN_DECL_CAST(struct kefir_ast_translator_scoped_identifier_object *, scoped_identifier_payload,
                         scoped_identifier->value->payload.ptr);
    ...
    REQUIRE_OK(kefir_json_output_object_key(json, "variables"));
    REQUIRE_OK(kefir_json_output_array_begin(json));
    for (const struct kefir_list_entry *iter = kefir_list_head(&scope->static_thread_local_objects); iter != NULL;
         kefir_list_next(&iter)) {
        ASSIGN_DECL_CAST(struct kefir_ast_translator_scoped_identifier_entry *, scoped_identifier, iter->value);
        ASSIGN_DECL_CAST(struct kefir_ast_translator_scoped_identifier_object *, scoped_identifier_payload,
                         scoped_identifier->value->payload.ptr);
...
static kefir_result_t format_local_scope(struct kefir_json_output *json,
                                         struct kefir_ast_translator_local_scope_layout *scope) {
...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    REQUIRE_OK(kefir_ast_global_context_init(mem, type_traits, &env.target_env, &global_context));
    REQUIRE_OK(kefir_ast_local_context_init(mem, &global_context, &local_context));
    struct kefir_ast_context *context = &local_context.context;

    REQUIRE_OK(kefir_ast_global_context_declare_external(mem, &global_context, "global_var1",
                                                         kefir_ast_type_signed_int(), NULL, NULL, NULL));
    REQUIRE_OK(kefir_ast_global_context_define_external(
        mem, &global_context, "global_var2", kefir_ast_type_signed_long(),
        kefir_ast_alignment_as_type(mem, kefir_ast_type_char()), NULL, NULL, NULL));
    REQUIRE_OK(kefir_ast_global_context_declare_external_thread_local(mem, &global_context, "global_var3",
                                                                      kefir_ast_type_float(), NULL, NULL, NULL));
    ...
    REQUIRE_OK(kefir_ast_global_context_define_static(
        ...
        kefir_ast_type_pointer(mem, context->type_bundle, kefir_ast_type_unsigned_long()), NULL, NULL, NULL, NULL));
    ...
    REQUIRE_OK(kefir_ast_struct_type_field(mem, context->symbols, struct_type1, "y",
                                           ...
                                           NULL));
    REQUIRE_OK(
        kefir_ast_global_context_define_static(mem, &global_context, "static_var2", type1, NULL, NULL, NULL, NULL));
    ...
    REQUIRE_OK(kefir_ast_global_context_define_static_thread_local(
        ...
        NULL, NULL, NULL, NULL));
    REQUIRE_OK(kefir_ast_global_context_define_static_thread_local(
        mem, &global_context, "static_var4", kefir_ast_type_signed_short(), NULL, NULL, NULL, NULL));

    REQUIRE_OK(kefir_ast_local_context_declare_external(mem, &local_context, "global_var100",
    ...
    REQUIRE_OK(kefir_ast_local_context_declare_external_thread_local(
        ...
        kefir_ast_alignment_const_expression(mem, kefir_ast_constant_expression_integer(mem, 32)), NULL));
    ...
    REQUIRE_OK(kefir_ast_local_context_define_static_thread_local(
        ...
        kefir_ast_alignment_const_expression(mem, kefir_ast_constant_expression_integer(mem, 8)), NULL, NULL));
    REQUIRE_OK(kefir_ast_local_context_define_static_thread_local(mem, &local_context, "local_static_var4",
                                                                  kefir_ast_type_float(), NULL, NULL, NULL));

    REQUIRE_OK(kefir_ast_local_context_define_auto(mem, &local_context, "local_var1", type1, NULL, NULL, NULL));
