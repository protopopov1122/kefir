...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    struct kefir_ast_declaration *function1_param1 =
        kefir_ast_new_declaration(mem, kefir_ast_declarator_identifier(mem, global_context.context.symbols, "a"), NULL);
    ...
    struct kefir_ast_declaration *function1_param2 =
        kefir_ast_new_declaration(mem, kefir_ast_declarator_identifier(mem, global_context.context.symbols, "b"), NULL);
    REQUIRE_OK(kefir_ast_declarator_specifier_list_append(mem, &function1_param2->specifiers,
                                                          kefir_ast_type_specifier_int(mem)));
    REQUIRE_OK(kefir_list_insert_after(mem, &function1_decl->function.parameters,
                                       kefir_list_tail(&function1_decl->function.parameters), function1_param2));
