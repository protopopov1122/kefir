...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    REQUIRE_OK(kefir_ast_local_context_declare_external(mem, &local_context,
        "bool", kefir_ast_type_bool(), NULL, NULL));
    REQUIRE_OK(kefir_ast_local_context_declare_external(mem, &local_context,
        "char", kefir_ast_type_char(), NULL, NULL));
    REQUIRE_OK(kefir_ast_local_context_declare_external(mem, &local_context,
        "uchar", kefir_ast_type_unsigned_char(), NULL, NULL));
    REQUIRE_OK(kefir_ast_local_context_declare_external(mem, &local_context,
        "schar", kefir_ast_type_signed_char(), NULL, NULL));
    REQUIRE_OK(kefir_ast_local_context_declare_external(mem, &local_context,
        "ushort", kefir_ast_type_unsigned_short(), NULL, NULL));
    REQUIRE_OK(kefir_ast_local_context_declare_external(mem, &local_context,
        "sshort", kefir_ast_type_signed_short(), NULL, NULL));
    REQUIRE_OK(kefir_ast_local_context_declare_external(mem, &local_context,
        "uint", kefir_ast_type_unsigned_int(), NULL, NULL));
    REQUIRE_OK(kefir_ast_local_context_declare_external(mem, &local_context,
        "sint", kefir_ast_type_signed_int(), NULL, NULL));
    REQUIRE_OK(kefir_ast_local_context_declare_external(mem, &local_context,
        "float", kefir_ast_type_float(), NULL, NULL));
    REQUIRE_OK(kefir_ast_local_context_declare_external(mem, &local_context,
        "ulong", kefir_ast_type_unsigned_long(), NULL, NULL));
    REQUIRE_OK(kefir_ast_local_context_declare_external(mem, &local_context,
        "slong", kefir_ast_type_signed_long(), NULL, NULL));
    REQUIRE_OK(kefir_ast_local_context_declare_external(mem, &local_context,
        "ullong", kefir_ast_type_unsigned_long_long(), NULL, NULL));
    REQUIRE_OK(kefir_ast_local_context_declare_external(mem, &local_context,
        "sllong", kefir_ast_type_signed_long_long(), NULL, NULL));
    REQUIRE_OK(kefir_ast_local_context_declare_external(mem, &local_context,
        "double", kefir_ast_type_double(), NULL, NULL));
    REQUIRE_OK(kefir_ast_local_context_declare_external(mem, &local_context,
        "enum", type1, NULL, NULL));
    REQUIRE_OK(kefir_ast_local_context_declare_external(mem, &local_context,
        "ptr", type2, NULL, NULL));
...
#define SIMPLE_ASSIGN_NODE(_node1, _node2) \
    do { \
        struct kefir_ast_node_base *node = KEFIR_AST_NODE_BASE(kefir_ast_new_simple_assignment(mem, (_node1), (_node2))); \
    ...
    FUNC("assign_char", {
        SIMPLE_ASSIGN_NODE(
            KEFIR_AST_NODE_BASE(kefir_ast_new_identifier(mem, context->symbols, "char")),
            KEFIR_AST_NODE_BASE(kefir_ast_new_constant_char(mem, 'A'))
        );
        SIMPLE_ASSIGN_NODE(
            KEFIR_AST_NODE_BASE(kefir_ast_new_identifier(mem, context->symbols, "uchar")),
    ...
    FUNC("assign_short", {
        SIMPLE_ASSIGN_NODE(
            KEFIR_AST_NODE_BASE(kefir_ast_new_identifier(mem, context->symbols, "ushort")),
            KEFIR_AST_NODE_BASE(kefir_ast_new_constant_int(mem, 0xfe))
        );
        SIMPLE_ASSIGN_NODE(
            KEFIR_AST_NODE_BASE(kefir_ast_new_identifier(mem, context->symbols, "sshort")),
    ...
    FUNC("assign_long", {
        SIMPLE_ASSIGN_NODE(
            KEFIR_AST_NODE_BASE(kefir_ast_new_identifier(mem, context->symbols, "ulong")),
    ...
    FUNC("assign_long_long", {
        ...
        SIMPLE_ASSIGN_NODE(
            KEFIR_AST_NODE_BASE(kefir_ast_new_identifier(mem, context->symbols, "sllong")),
    ...
    FUNC("enum", {
        SIMPLE_ASSIGN_NODE(
            KEFIR_AST_NODE_BASE(kefir_ast_new_identifier(mem, context->symbols, "enum")),
