...
static kefir_result_t translate_extension_node(struct kefir_mem *mem, struct kefir_ast_translator_context *context,
    ...
    UNUSED(node);
    UNUSED(context);
    REQUIRE(tag == KEFIR_AST_TRANSLATOR_CONTEXT_EXTENSION_TAG_LVALUE, KEFIR_INTERNAL_ERROR);
    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IROPCODE_PUSHI64, 100));
    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IROPCODE_LOAD64, 100));
    REQUIRE_OK(KEFIR_IRBUILDER_BLOCK_APPENDI64(builder, KEFIR_IROPCODE_LOAD64, 100));
