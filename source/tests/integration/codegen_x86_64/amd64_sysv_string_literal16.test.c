...
kefir_result_t kefir_int_test(struct kefir_mem *mem) {
    ...
    REQUIRE_OK(kefir_ir_module_string_literal(mem, &module, KEFIR_IR_STRING_LITERAL_UNICODE16, literal,
    ...
    const kefir_char16_t literal2[] = u"Hello,\ncruel\tworld!";
    REQUIRE_OK(kefir_ir_module_string_literal(mem, &module, KEFIR_IR_STRING_LITERAL_UNICODE16, literal2,
                                              sizeof(literal2) / sizeof(literal2[0]), &literal_id));
    kefir_irbuilder_block_appendi64(mem, &func->body, KEFIR_IROPCODE_PUSHSTRING, literal_id);

    const kefir_char16_t literal3[] = u"\n\n\n\t\t\t   \t\t\t\v\v\v\n";
    REQUIRE_OK(kefir_ir_module_string_literal(mem, &module, KEFIR_IR_STRING_LITERAL_UNICODE16, literal3,
                                              sizeof(literal3) / sizeof(literal3[0]), &literal_id));
    kefir_irbuilder_block_appendi64(mem, &func->body, KEFIR_IROPCODE_PUSHSTRING, literal_id);

    const kefir_char16_t literal4[] = u"\'\"Hey ho!\"\'\\\"";
    REQUIRE_OK(kefir_ir_module_string_literal(mem, &module, KEFIR_IR_STRING_LITERAL_UNICODE16, literal4,
                                              sizeof(literal4) / sizeof(literal4[0]), &literal_id));
    kefir_irbuilder_block_appendi64(mem, &func->body, KEFIR_IROPCODE_PUSHSTRING, literal_id);

    const kefir_char16_t literal5[] = u"\0\0\0\n\0";
    REQUIRE_OK(kefir_ir_module_string_literal(mem, &module, KEFIR_IR_STRING_LITERAL_UNICODE16, literal5,
