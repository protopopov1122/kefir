...
static kefir_result_t free_vararg_scalar_type(struct kefir_mem *mem, void *payload) {
    ...
    ASSIGN_DECL_CAST(struct vararg_scalar_type *, type,
        payload);
    KEFIR_FREE(mem, type);
    return KEFIR_OK;
}
...
static kefir_result_t vararg_get_int(struct kefir_codegen_amd64 *codegen,
    ...
    ASSIGN_DECL_CAST(struct vararg_scalar_type *, scalar_type,
        payload);
    ASMGEN_LABEL(&codegen->asmgen, "%s",
        identifier);
...
static kefir_result_t vararg_visit_integer(const struct kefir_ir_type *type,
    ...
    ASSIGN_DECL_CAST(struct vararg_getter *, param,
        payload);
    snprintf(buffer, BUF_SIZE - 1, KEFIR_AMD64_SYSV_FUNCTION_VARARG_ARG_SCALAR_LABEL,
        param->sysv_func->func->name, (kefir_int_t) typeentry->typecode);
...
static kefir_result_t vararg_visit_aggregate(const struct kefir_ir_type *type,
    ...
    struct vararg_aggregate_info *info =
        KEFIR_MALLOC(param->mem, sizeof(struct vararg_aggregate_info));
...
static kefir_result_t vararg_visit_builtin_impl(struct kefir_mem *mem,
                                             struct kefir_vector *allocation,
                                             kefir_size_t slot,
                                             const struct kefir_ir_typeentry *typeentry,
                                             struct kefir_codegen_amd64 *codegen,
...
static kefir_result_t vararg_visit_builtin(const struct kefir_ir_type *type,
    ...
    res = vararg_visit_builtin_impl(param->mem, &allocation, iter.slot, typeentry, param->codegen, param->sysv_func, param->identifier);
    REQUIRE_ELSE(res == KEFIR_OK, {
...
kefir_result_t kefir_codegen_amd64_sysv_vararg_instruction(struct kefir_mem *mem,
                                                       struct kefir_codegen_amd64 *codegen,
                                                       struct kefir_codegen_amd64_sysv_module *sysv_module,
                                                       struct kefir_amd64_sysv_function *sysv_func,
