...
const char *KEFIR_AMD64_SYSV_INTEGER_REGISTERS[] = {KEFIR_AMD64_RDI, KEFIR_AMD64_RSI, KEFIR_AMD64_RDX,
                                                    KEFIR_AMD64_RCX, KEFIR_AMD64_R8,  KEFIR_AMD64_R9};
...
const char *KEFIR_AMD64_SYSV_SSE_REGISTERS[] = {KEFIR_AMD64_XMM0, KEFIR_AMD64_XMM1, KEFIR_AMD64_XMM2, KEFIR_AMD64_XMM3,
                                                KEFIR_AMD64_XMM4, KEFIR_AMD64_XMM5, KEFIR_AMD64_XMM6, KEFIR_AMD64_XMM7};
...
const char *KEFIR_AMD64_SYSV_INTEGER_RETURN_REGISTERS[] = {KEFIR_AMD64_RAX, KEFIR_AMD64_RDX};
...
const char *KEFIR_AMD64_SYSV_SSE_RETURN_REGISTERS[] = {KEFIR_AMD64_XMM0, KEFIR_AMD64_XMM1};
...
static kefir_result_t assign_immediate_memory(const struct kefir_ir_type *type, kefir_size_t index,
                                              const struct kefir_ir_typeentry *typeentry, void *payload) {
    UNUSED(type);
    UNUSED(typeentry);
    struct input_allocation *info = (struct input_allocation *) payload;
...
static kefir_result_t assign_immediate_pad(const struct kefir_ir_type *type, kefir_size_t index,
                                           const struct kefir_ir_typeentry *typeentry, void *payload) {
    UNUSED(type);
    UNUSED(typeentry);
    struct input_allocation *info = (struct input_allocation *) payload;
...
static kefir_result_t assign_nested_scalar(const struct kefir_ir_type *type, kefir_size_t index,
    ...
    if (typeentry->typecode == KEFIR_IR_TYPE_BITS) {
        REQUIRE_OK(kefir_amd64_sysv_abi_qwords_next_bitfield(&info->top_allocation->container, dataclass,
                                                             typeentry->param, &allocation->container_reference));
    } else {
        REQUIRE_OK(kefir_amd64_sysv_abi_qwords_next(&info->top_allocation->container, dataclass, layout->size,
...
static kefir_result_t assign_nested_memory(const struct kefir_ir_type *type, kefir_size_t index,
                                           const struct kefir_ir_typeentry *typeentry, void *payload) {
    UNUSED(type);
    UNUSED(index);
    UNUSED(typeentry);
    ...
    allocation->klass = KEFIR_AMD64_SYSV_PARAM_NO_CLASS;
    allocation->index = index;
    REQUIRE_OK(kefir_amd64_sysv_abi_qwords_next(&info->top_allocation->container, KEFIR_AMD64_SYSV_PARAM_MEMORY,
                                                layout->size, layout->alignment, &allocation->container_reference));
    return KEFIR_OK;
}
...
static kefir_result_t union_reset_hook(const struct kefir_ir_type *type, kefir_size_t index,
    ...
    if (info->nested == 0) {
        ...
        REQUIRE_OK(
            kefir_amd64_sysv_abi_qwords_max_position(&info->max_position, &current_position, &info->max_position));
...
static kefir_result_t assign_nested_union(const struct kefir_ir_type *type, kefir_size_t index,
    ...
    REQUIRE_OK(
        kefir_amd64_sysv_abi_qwords_restore_position(&info->top_allocation->container, &nested_info.max_position));
    return KEFIR_OK;
}
...
static kefir_result_t aggregate_disown(struct kefir_mem *mem, struct kefir_amd64_sysv_parameter_allocation *alloc) {
...
static kefir_result_t aggregate_postmerger(struct kefir_mem *mem, struct kefir_amd64_sysv_parameter_allocation *alloc) {
    ...
    if (length > 2 && (((struct kefir_amd64_sysv_abi_qword *) kefir_vector_at(&alloc->container.qwords, 0))->klass !=
                           KEFIR_AMD64_SYSV_PARAM_SSE ||
                       had_nonsseup)) {
        return aggregate_disown(mem, alloc);
    }
...
kefir_result_t kefir_amd64_sysv_parameter_classify(struct kefir_mem *mem, const struct kefir_ir_type *type,
    ...
    for (kefir_size_t i = 0; i < slots; i++) {
        ASSIGN_DECL_CAST(struct kefir_amd64_sysv_parameter_allocation *, entry, kefir_vector_at(allocation, i));
...
static kefir_result_t pad_allocate(const struct kefir_ir_type *type, kefir_size_t index,
                                   const struct kefir_ir_typeentry *typeentry, void *payload) {
    UNUSED(type);
    UNUSED(index);
    UNUSED(typeentry);
...
static kefir_result_t memory_allocate(const struct kefir_ir_type *type, kefir_size_t index,
                                      const struct kefir_ir_typeentry *typeentry, void *payload) {
    UNUSED(index);
    UNUSED(typeentry);
    struct allocation_state *state = (struct allocation_state *) payload;
    ...
    ASSIGN_DECL_CAST(struct kefir_amd64_sysv_parameter_allocation *, alloc,
                     kefir_vector_at(state->allocation, iter.slot));
    REQUIRE(alloc->requirements.integer == 0 && alloc->requirements.sse == 0 && alloc->requirements.sseup == 0 &&
                alloc->requirements.memory.size > 0,
