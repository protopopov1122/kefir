...
kefir_result_t kefir_parser_init(struct kefir_mem *mem, struct kefir_parser *parser,
                                 const struct kefir_parser_ruleset *ruleset, struct kefir_symbol_table *symbols,
    ...
    REQUIRE(ruleset != NULL, KEFIR_SET_ERROR(KEFIR_MALFORMED_ARG, "Expected valid parser ruleset"));
    REQUIRE(stream != NULL, KEFIR_SET_ERROR(KEFIR_MALFORMED_ARG, "Expected valid token stream"));

    parser->ruleset = ruleset;
...
kefir_result_t kefir_parser_shift(struct kefir_mem *mem, struct kefir_parser *parser, kefir_uint_t count) {
    REQUIRE(mem != NULL, KEFIR_SET_ERROR(KEFIR_MALFORMED_ARG, "Expected valid memory allocator"));
    REQUIRE(parser != NULL, KEFIR_SET_ERROR(KEFIR_MALFORMED_ARG, "Expected valid parser"));
    while (count--) {
...
kefir_result_t kefir_parser_next(struct kefir_mem *mem, struct kefir_parser *parser,
    ...
    REQUIRE_OK(KEFIR_PARSER_RULE_MATCH(mem, parser, primary_expression));
