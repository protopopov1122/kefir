...
kefir_result_t kefir_ir_data_set_integer(struct kefir_ir_data *data,
                                     kefir_size_t index,
                                     kefir_int64_t value) {
    REQUIRE(data != NULL, KEFIR_SET_ERROR(KEFIR_MALFORMED_ARG, "Expected valid IR data pointer"));
...
kefir_result_t kefir_ir_data_set_float32(struct kefir_ir_data *data,
                                     kefir_size_t index,
...
kefir_result_t kefir_ir_data_set_float64(struct kefir_ir_data *data,
                                     kefir_size_t index,
...
kefir_result_t kefir_ir_data_set_pointer(struct kefir_ir_data *data,
                                     kefir_size_t index,
...
kefir_result_t kefir_ir_data_set_raw(struct kefir_ir_data *data,
                                 kefir_size_t index,
                                 const void *raw,
                                 kefir_size_t length) {
    REQUIRE(data != NULL, KEFIR_SET_ERROR(KEFIR_MALFORMED_ARG, "Expected valid IR data pointer"));
...
static kefir_result_t finalize_unsupported(const struct kefir_ir_type *type,
                                         kefir_size_t index,
...
static kefir_result_t finalize_pad(const struct kefir_ir_type *type,
                                 kefir_size_t index,
                                 const struct kefir_ir_typeentry *typeentry,
                                 void *payload) {
    UNUSED(type);
    ...
    ASSIGN_DECL_CAST(struct finalize_param *, param,
        payload);
    param->slot++;    
    return KEFIR_OK;
}

static kefir_result_t finalize_scalar(const struct kefir_ir_type *type,
                                    kefir_size_t index,
                                    const struct kefir_ir_typeentry *typeentry,
                                    void *payload) {
    UNUSED(type);
...
static kefir_result_t finalize_struct_union(const struct kefir_ir_type *type,
                                          kefir_size_t index,
                                          const struct kefir_ir_typeentry *typeentry,
                                          void *payload) {
    REQUIRE(payload != NULL, KEFIR_SET_ERROR(KEFIR_INTERNAL_ERROR, "Expected valid payload"));
...
static kefir_result_t finalize_array(const struct kefir_ir_type *type,
                                          kefir_size_t index,
                                          const struct kefir_ir_typeentry *typeentry,
                                          void *payload) {
    REQUIRE(payload != NULL, KEFIR_SET_ERROR(KEFIR_INTERNAL_ERROR, "Expected valid payload"));
...
kefir_result_t kefir_ir_data_finalize(struct kefir_ir_data *data) {
    ...
    REQUIRE_OK(kefir_ir_type_visitor_list_nodes(data->type, &visitor, &param, 0, kefir_ir_type_total_length(data->type)));
    data->finalized = true;
    return KEFIR_OK;
}